...
public class Perl5Substitution extends StringSubstitution
{
    ...
    private static final boolean __isInterpolationCharacter(final char c) {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if ((c != '$' && c != '\\') || n3 != 0) {
                ...
                if (n5 != length) {
                    ...
                    if (c == '$') {
                        _isInterpolationCharacter = (__isInterpolationCharacter(c2) ? 1 : 0);
                    }
                    else if (c == '\\') {
                        if (c2 == 'l') {
                            if (n4 == 0) {
    ...
    void _calcSub(final StringBuffer sb, final MatchResult matchResult) {
        ...
        for (int subOpcodesCount = this._subOpcodesCount, i = 0; i < subOpcodesCount; ++i) {
            ...
            if (n == -2) {
                sb.append(Character.toLowerCase(array[j++]));
                sb.append(array, j, --n3);
                n = 0;
            }
            else if (n == -3) {
                sb.append(Character.toUpperCase(array[j++]));
                sb.append(array, j, --n3);
                n = 0;
            }
            else if (n == -4) {
