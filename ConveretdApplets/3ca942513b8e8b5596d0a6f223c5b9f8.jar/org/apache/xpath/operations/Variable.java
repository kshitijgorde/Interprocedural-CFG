...
public class Variable extends Expression implements PathComponent
{
    ...
    public XObject execute(final XPathContext xctxt, final boolean destructiveOK) throws TransformerException {
        ...
        if (this.m_fromXPathAPI) {
            if (this.m_variableResolver == null) {
                final XPathExpressionException wrappedException = new XPathExpressionException(XPATHMessages.createXPATHMessage("ER_UNDEFINED_VARIABLE_RESOLVER", new Object[] { this.toString() }));
                throw new TransformerException(wrappedException);
            }
            final String localPart = this.m_qname.getLocalName();
            String uri = this.m_qname.getNamespaceURI();
            if (uri == null) {
                uri = "";
            }
            ...
            final javax.xml.namespace.QName varName = new javax.xml.namespace.QName(uri, localPart, prefix);
            final Object val = this.m_variableResolver.resolveVariable(varName);
            if (val != null) {
                return XObject.create(val, xctxt);
            }
            final XPathExpressionException wrappedException2 = new XPathExpressionException(XPATHMessages.createXPATHMessage("ER_UNRESOLVED_VARIABLE", new Object[] { this.toString() }));
    ...
    public String toString() {
        final String uri = this.m_qname.getNamespaceURI();
        if (uri != null) {
            return "{" + uri + "}" + this.m_qname.getLocalName();
        }
