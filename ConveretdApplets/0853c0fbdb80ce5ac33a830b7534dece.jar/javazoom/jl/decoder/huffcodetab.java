...
final class huffcodetab
{
    ...
    public static int huffman_decoder(final huffcodetab huffcodetab, final int[] array, final int[] array2, final int[] array3, final int[] array4, final BitReserve bitReserve) {
        final int n = Integer.MIN_VALUE;
        int n2 = 0;
        int n3 = 1;
        int n4 = n;
        if (huffcodetab.val == null) {
            return 2;
        }
        if (huffcodetab.treelen == 0) {
            array[0] = (array2[0] = 0);
            return 0;
        }
        while (true) {
            while (huffcodetab.val[n2][0] != 0) {
                if (bitReserve.hget1bit() != 0) {
                    while (huffcodetab.val[n2][1] >= 250) {
                        n2 += huffcodetab.val[n2][1];
                    }
                    n2 += huffcodetab.val[n2][1];
                }
                else {
                    while (huffcodetab.val[n2][0] >= 250) {
                        n2 += huffcodetab.val[n2][0];
                    }
                    n2 += huffcodetab.val[n2][0];
                }
                n4 >>>= 1;
                if (n4 == 0 && n2 >= 0) {
                    if (huffcodetab.tablename0 == '3' && (huffcodetab.tablename1 == '2' || huffcodetab.tablename1 == '3')) {
                        array3[0] = (array2[0] >> 3 & 0x1);
                        array4[0] = (array2[0] >> 2 & 0x1);
                        array[0] = (array2[0] >> 1 & 0x1);
                        array2[0] &= 0x1;
                        if (array3[0] != 0 && bitReserve.hget1bit() != 0) {
                            array3[0] = -array3[0];
                        }
                        if (array4[0] != 0 && bitReserve.hget1bit() != 0) {
                            array4[0] = -array4[0];
                        }
                        if (array[0] != 0 && bitReserve.hget1bit() != 0) {
                            array[0] = -array[0];
                        }
                        if (array2[0] != 0 && bitReserve.hget1bit() != 0) {
                            array2[0] = -array2[0];
                        }
                    }
                    else {
                        if (huffcodetab.linbits != 0 && huffcodetab.xlen - 1 == array[0]) {
                            final int n5 = 0;
                            array[n5] += bitReserve.hgetbits(huffcodetab.linbits);
                        }
                        if (array[0] != 0 && bitReserve.hget1bit() != 0) {
                            array[0] = -array[0];
                        }
                        if (huffcodetab.linbits != 0 && huffcodetab.ylen - 1 == array2[0]) {
                            final int n6 = 0;
                            array2[n6] += bitReserve.hgetbits(huffcodetab.linbits);
                        }
                        if (array2[0] != 0 && bitReserve.hget1bit() != 0) {
                            array2[0] = -array2[0];
                        }
            ...
            array[0] = huffcodetab.val[n2][1] >>> 4;
            array2[0] = (huffcodetab.val[n2][1] & 0xF);
            n3 = 0;
            continue;
        }
    }
