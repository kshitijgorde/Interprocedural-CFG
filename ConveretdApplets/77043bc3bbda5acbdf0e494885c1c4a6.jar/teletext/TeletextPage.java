...
public class TeletextPage
{
    ...
    public void fillRectangle(final int n, final int n2, final int n3, final int n4, final boolean b) {
        for (int i = n; i <= n3; ++i) {
            this.drawLine(i, n2, i, n4, b);
        }
    }
    ...
    public int numberHitTest(final int n, final int n2) {
        ...
        int n3 = n;
        while (i > 0) {
            if (!this.isDigit(this.contents[n2][i - 1])) {
                break;
            }
            --i;
        }
        while (n3 < this.getWidth() - 1 && this.isDigit(this.contents[n2][n3 + 1])) {
            ++n3;
        }
        for (int j = i; j <= n3; ++j) {
            sb.append(this.contents[n2][j]);
        }
        int int1;
        try {
