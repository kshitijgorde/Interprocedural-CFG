...
public abstract class BasePeriod extends AbstractPeriod implements ReadablePeriod, Serializable
{
    ...
    protected BasePeriod(final int n, final int n2, final int n3, final int n4, final int n5, final int n6, final int n7, final int n8, PeriodType checkPeriodType) {
        ...
        this.setPeriodInternal(n, n2, n3, n4, n5, n6, n7, n8);
    }
    ...
    public DurationFieldType getFieldType(final int n) {
        return this.iType.getFieldType(n);
    }
    ...
    private void setPeriodInternal(final ReadablePeriod readablePeriod) {
        final int[] iValues = new int[this.size()];
        for (int i = 0; i < readablePeriod.size(); ++i) {
            this.checkAndUpdate(readablePeriod.getFieldType(i), iValues, readablePeriod.getValue(i));
        }
        this.iValues = iValues;
    }
    ...
    private void setPeriodInternal(final int n, final int n2, final int n3, final int n4, final int n5, final int n6, final int n7, final int n8) {
        final int[] iValues = new int[this.size()];
        this.checkAndUpdate(DurationFieldType.years(), iValues, n);
        this.checkAndUpdate(DurationFieldType.months(), iValues, n2);
        this.checkAndUpdate(DurationFieldType.weeks(), iValues, n3);
        this.checkAndUpdate(DurationFieldType.days(), iValues, n4);
        this.checkAndUpdate(DurationFieldType.hours(), iValues, n5);
        this.checkAndUpdate(DurationFieldType.minutes(), iValues, n6);
        this.checkAndUpdate(DurationFieldType.seconds(), iValues, n7);
        this.checkAndUpdate(DurationFieldType.millis(), iValues, n8);
        this.iValues = iValues;
    }
    ...
    protected void mergePeriod(final ReadablePeriod readablePeriod) {
        if (readablePeriod != null) {
            this.iValues = this.mergePeriodInto(this.getValues(), readablePeriod);
        }
    }
    ...
    protected void addPeriod(final ReadablePeriod readablePeriod) {
        if (readablePeriod != null) {
            this.iValues = this.addPeriodInto(this.getValues(), readablePeriod);
        }
    }
    ...
    protected void setValues(final int[] iValues) {
        this.iValues = iValues;
    }
}
