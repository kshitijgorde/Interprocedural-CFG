...
class LayerIIDecoder extends LayerIDecoder implements FrameDecoder
{
    protected void createSubbands() {
        if (super.mode == 3) {
            for (int i = 0; i < super.num_subbands; ++i) {
                super.subbands[i] = new SubbandLayer2(i);
            }
        }
        else if (super.mode == 1) {
            int j;
            for (j = 0; j < super.header.intensity_stereo_bound(); ++j) {
                super.subbands[j] = new SubbandLayer2Stereo(j);
            }
            while (j < super.num_subbands) {
                super.subbands[j] = new SubbandLayer2IntensityStereo(j);
                ++j;
            }
        }
        else {
            for (int k = 0; k < super.num_subbands; ++k) {
                super.subbands[k] = new SubbandLayer2Stereo(k);
            }
        }
    }
    ...
    protected void readScaleFactorSelection() {
        for (int i = 0; i < super.num_subbands; ++i) {
            ((SubbandLayer2)super.subbands[i]).read_scalefactor_selection(super.stream, super.crc);
        }
    }
    ...
    static class SubbandLayer2Stereo extends SubbandLayer2
    {
        ...
        public void read_allocation(final Bitstream bitstream, final Header header, final Crc16 crc16) {
            ...
            super.allocation = bitstream.get_bits(get_allocationlength);
            this.channel2_allocation = bitstream.get_bits(get_allocationlength);
            if (crc16 != null) {
                crc16.add_bits(super.allocation, get_allocationlength);
        ...
        public void read_scalefactor_selection(final Bitstream bitstream, final Crc16 crc16) {
            if (super.allocation != 0) {
                super.scfsi = bitstream.get_bits(2);
                if (crc16 != null) {
                    crc16.add_bits(super.scfsi, 2);
                }
            }
        ...
        public void read_scalefactor(final Bitstream bitstream, final Header header) {
            ...
            if (this.channel2_allocation != 0) {
                switch (this.channel2_scfsi) {
                    case 0: {
                        this.channel2_scalefactor1 = Subband.scalefactors[bitstream.get_bits(6)];
                        this.channel2_scalefactor2 = Subband.scalefactors[bitstream.get_bits(6)];
                        this.channel2_scalefactor3 = Subband.scalefactors[bitstream.get_bits(6)];
                        break;
                    }
                    case 1: {
                        final float n = Subband.scalefactors[bitstream.get_bits(6)];
                    ...
                    case 2: {
                        final float channel2_scalefactor1 = Subband.scalefactors[bitstream.get_bits(6)];
                    ...
                    case 3: {
                        this.channel2_scalefactor1 = Subband.scalefactors[bitstream.get_bits(6)];
                        final float n2 = Subband.scalefactors[bitstream.get_bits(6)];
        ...
        public boolean read_sampledata(final Bitstream bitstream) {
            ...
            if (this.channel2_allocation != 0) {
                if (super.groupingtable[1] != null) {
                    ...
                    final float[] array = super.groupingtable[1];
                    int n2 = 0;
                    int n3 = n;
        ...
        public boolean put_next_sample(final int n, final SynthesisFilter synthesisFilter, final SynthesisFilter synthesisFilter2) {
            ...
            if (this.channel2_allocation != 0 && n != 1) {
                float n2 = this.channel2_samples[super.samplenumber - 1];
                if (super.groupingtable[1] == null) {
                    n2 = (n2 + this.channel2_d[0]) * this.channel2_c[0];
                }
                float n3;
                if (super.groupnumber <= 4) {
                    n3 = n2 * this.channel2_scalefactor1;
                }
                else if (super.groupnumber <= 8) {
                ...
                if (n == 0) {
                    ...
                    synthesisFilter.input_sample(n3, super.subbandnumber);
                }
            }
    ...
    static class SubbandLayer2 extends Subband
    {
        ...
        public void read_scalefactor(final Bitstream bitstream, final Header header) {
            if (this.allocation != 0) {
                switch (this.scfsi) {
                    case 0: {
                        this.scalefactor1 = Subband.scalefactors[bitstream.get_bits(6)];
                        this.scalefactor2 = Subband.scalefactors[bitstream.get_bits(6)];
                        this.scalefactor3 = Subband.scalefactors[bitstream.get_bits(6)];
                        break;
                    }
                    case 1: {
                        final float n = Subband.scalefactors[bitstream.get_bits(6)];
                    ...
                    case 2: {
                        final float scalefactor1 = Subband.scalefactors[bitstream.get_bits(6)];
    ...
    static class SubbandLayer2IntensityStereo extends SubbandLayer2
    {
        ...
        public boolean put_next_sample(final int n, final SynthesisFilter synthesisFilter, final SynthesisFilter synthesisFilter2) {
            if (super.allocation != 0) {
                float n2 = super.samples[super.samplenumber];
                if (super.groupingtable[0] == null) {
                    n2 = (n2 + super.d[0]) * super.c[0];
                }
                if (n == 0) {
                    final float n3 = n2;
                    float n4;
                    float n5;
                    if (super.groupnumber <= 4) {
                        n4 = n2 * super.scalefactor1;
                        n5 = n3 * this.channel2_scalefactor1;
                    }
                    else if (super.groupnumber <= 8) {
                        n4 = n2 * super.scalefactor2;
                        n5 = n3 * this.channel2_scalefactor2;
                    }
                    else {
                        n4 = n2 * super.scalefactor3;
                        n5 = n3 * this.channel2_scalefactor3;
                    }
                    synthesisFilter.input_sample(n4, super.subbandnumber);
                    synthesisFilter2.input_sample(n5, super.subbandnumber);
                }
                else if (n == 1) {
                    float n6;
                    if (super.groupnumber <= 4) {
                        n6 = n2 * super.scalefactor1;
                    }
                    else if (super.groupnumber <= 8) {
                        n6 = n2 * super.scalefactor2;
                    }
                    else {
                        n6 = n2 * super.scalefactor3;
                    }
                    synthesisFilter.input_sample(n6, super.subbandnumber);
                }
                else {
                    float n7;
                    if (super.groupnumber <= 4) {
                    ...
                    synthesisFilter.input_sample(n7, super.subbandnumber);
                }
            }
            return ++super.samplenumber == 3;
        }
    }
}
