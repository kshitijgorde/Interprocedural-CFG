...
{
    public ShellThing(final SApplet applet, final SScalable sScalable, final double n, final double n2, final int n3) {
    ...
    public void paint(final Graphics graphics) {
        ...
        for (int i = 0; i <= super.s; ++i) {
            graphics.drawOval(n + i, n2 + i, 2 * super.w - 2 * i + 1, 2 * super.h - 2 * i + 1);
        }
    }
    ...
    public void paintHighlight(final Graphics graphics) {
        ...
        for (int i = 0; i <= super.s; ++i) {
            graphics.drawOval(n + i, n2 + i, 2 * super.w - 2 * i + 1, 2 * super.h - 2 * i + 1);
            graphics.drawOval(n + i - 1, n2 + i - 1, 2 * super.w - 2 * i + 3, 2 * super.h - 2 * i + 3);
        }
    }
