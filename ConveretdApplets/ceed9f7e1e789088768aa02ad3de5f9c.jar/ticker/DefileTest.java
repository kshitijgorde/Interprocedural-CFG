...
public class DefileTest extends Canvas implements Runnable
{
    Applet tickerappletwithmouseclick;
    ...
    public DefileTest(final TitresDataBase database, final Applet tickerappletwithmouseclick) {
        this.tickerappletwithmouseclick = null;
        ...
        this.tickerappletwithmouseclick = tickerappletwithmouseclick;
        this.addMouseListener(new TickerMouseListener());
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (this.database.getTitres() != null && this.indice < this.database.getTitres().size()) {
                n = this.hCasesTitres.get(this.database.getTitres().elementAt(this.indice)).image.getWidth(this);
            }
        }
        catch (Exception ex) {
    ...
    private class TickerMouseListener implements MouseListener
    {
        private void work(final MouseEvent mouseEvent) {
            final AppletContext appletContext = DefileTest.this.tickerappletwithmouseclick.getAppletContext();
            try {
                final URL url = new URL("http://www.rtfx.com/");
