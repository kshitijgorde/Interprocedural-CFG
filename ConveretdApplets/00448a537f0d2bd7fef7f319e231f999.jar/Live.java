...
public class Live extends Applet implements Runnable
{
    ...
    public Live() {
        this.frames_count = 0;
        this.animation_display_time = 0.05;
    ...
    public void initialize() {
        if (!this.initialized) {
            ...
            if (null == logo_image) {
                ...
                this.print_message(this.graphics_buffer, "LiveGraphics3D 1.60", false);
            ...
            if (null != dependent_variables) {
                ...
                if (!parser2.scan_dependent_variables()) {
                    ...
                    this.print_syntax_error(this.graphics_buffer, parser2);
                    this.evaluator = null;
                    this.repaint();
                }
                else {
            ...
            if (null != parser3.text && parser3.scan_animation() && null != parser3.scanned_frames && 0 < parser3.scanned_frames.size()) {
                ...
                this.preceding_rotation = this.rotation;
                (this.ps3D = this.frames.elementAt(this.current_frame_index)).setPerspective(this.ps3D.initial_length_view_point, this.ps3D.initial_magnification * this.magnification_factor, this.is_stereo, this.stereo_distance);
                ...
                if (null == parser3.text) {
                    ...
                    this.print_syntax_error(this.graphics_buffer, parser3);
                }
                this.ps3D = null;
            }
            parser3 = null;
    ...
    public boolean setFrame(final int new_frame_index) {
        if (new_frame_index < 0 || new_frame_index >= this.frames_count) {
            return false;
        }
        this.setFrameInternal(this.current_frame_index = new_frame_index);
        return this.is_new_dragging = true;
    }
                ...
                if (this.is_new_dragging) {
                    ...
                    if (!this.is_dragging_point) {
                        ...
                        if (0.0 >= point_scale) {
                            ...
                            for (int axis_index2 = 0; axis_index2 < 3; ++axis_index2) {
                                final Expression expression = expressions[axis_index2];
                                if (null != expression) {
                                    old_expression_values[axis_index2] = expression.getCurrentValue();
                            ...
                            if (3 > independent_variables_count) {
                                ...
                                for (int axis_index3 = 0; axis_index3 < 3; ++axis_index3) {
                                    if (null != expressions[axis_index3]) {
                                        vector[0] = 0.0;
                                        vector[2] = (vector[1] = 0.0);
                                        vector[axis_index3] = 1.0;
                                        final double dot_product = normal[axis_index3];
                                        ...
                                        scale_factors[axis_index3] = 1.0 / length;
                                    }
                                }
                            }
                            ...
                            for (int axis_index4 = 0; axis_index4 < 3; ++axis_index4) {
                                final Expression expression2 = expressions[axis_index4];
                                if (null != expression2) {
                                    expression2.setIndependentVariable(vector[axis_index4] / this.ps3D.scalings[axis_index4] + this.ps3D.original_center[axis_index4]);
                                }
                            }
                            if (!this.evaluator.evaluate()) {
                                for (int axis_index5 = 0; axis_index5 < 3; ++axis_index5) {
                                    final Expression expression3 = expressions[axis_index5];
                                    if (null != expression3) {
                                        expression3.setIndependentVariable(old_expression_values[axis_index5]);
                                    }
                                }
                            }
                            this.ps3D.projectPoints(true);
                    ...
                    try {
                        if ((this.is_animating && this.frames_count > 1 && (this.is_dragging || this.is_mouse_here) && !this.is_meta_down) || (this.is_spinning && this.is_mouse_here)) {
                            ...
                            final Thread painter5 = this.painter;
                            Thread.sleep(20L);
                        }
                    }
                    catch (InterruptedException e) {}
                }
                final boolean is_frame_old = System.currentTimeMillis() - this.painted_time >= (long)(1000.0 * this.animation_display_time);
    ...
    public boolean mouseMove(final Event evt, final int x, final int y) {
        this.is_mouse_here = true;
        if (!this.initialized || null == this.painted_ps3D || x < 0 || x > this.applet_width || y < 0 || y > this.applet_height || !this.is_put_on_screen || this.is_dragging_point) {
            return true;
        }
    ...
    public synchronized void drawTextRectangle(final Primitive3D primitive) {
        if (primitive.font_url == Primitive3D.text_dragable_point) {
            ...
            if (0.0f < this.painted_ps3D.point_scale[point_index]) {
                ...
                this.graphics_buffer.drawRect(this.painted_ps3D.left_pixel_xs[point_index] - radius, this.painted_ps3D.pixel_ys[point_index] - radius, 2 * radius - 1, 2 * radius - 1);
                if (this.painted_ps3D.is_stereo) {
                    this.graphics_buffer.drawRect(this.painted_ps3D.pixel_width + this.painted_ps3D.right_pixel_xs[point_index] - radius, this.painted_ps3D.pixel_ys[point_index] - radius, 2 * radius - 1, 2 * radius - 1);
                }
            ...
            if (0.0f < this.painted_ps3D.point_scale[point_index2]) {
                this.graphics_buffer.setColor(Color.white);
                this.graphics_buffer.setXORMode(Color.black);
                this.graphics_buffer.drawRect(this.painted_ps3D.left_pixel_xs[point_index2] + primitive.first_point, this.painted_ps3D.pixel_ys[point_index2] + primitive.second_point, primitive.third_point - 1, primitive.fourth_point - 1);
                if (this.painted_ps3D.is_stereo) {
                    this.graphics_buffer.drawRect(this.painted_ps3D.pixel_width + this.painted_ps3D.right_pixel_xs[point_index2] + primitive.first_point, this.painted_ps3D.pixel_ys[point_index2] + primitive.second_point, primitive.third_point - 1, primitive.fourth_point - 1);
                }
                this.graphics_buffer.setPaintMode();
            }
        }
    ...
    public boolean mouseUp(final Event evt, final int x, final int y) {
        ...
        this.is_meta_down = false;
        this.is_control_down = false;
        if (!this.initialized || null == this.painted_ps3D || !this.is_mouse_here || x < 0 || x > this.applet_width || y < 0 || y > this.applet_height || !this.is_put_on_screen) {
            return true;
        }
        this.mark_hyperlink(x, y);
        return true;
    ...
    void apply_new_offset() {
        for (int frame_index = 0; frame_index < this.frames.size(); ++frame_index) {
            ...
            g3D.preparePrimitives(this.graphics_buffer, this.evaluator, this.center_offset, this.sizes_scales);
        }
        if (null != this.evaluator) {
            this.evaluator.evaluate();
        }
        if (this.frames.size() > 1) {
            for (int frame_index2 = 0; frame_index2 < this.frames.size(); ++frame_index2) {
                final Graphics3D g3D2 = this.frames.elementAt(frame_index2);
                g3D2.projectPoints(true);
            }
        }
    }
        ...
        if (null == this.bg_image) {
            ...
            if (this.bg_is_spherical) {
                ...
                if (start_y > 0) {
                    ...
                    if (this.ps3D.is_stereo) {
                        int x2;
                        for (start_x = (x2 = ((this.ps3D.pixel_width + this.bg_width) / 2 + bg_rotated_x_offset + (int)this.ps3D.pixel_stereo_offset) % this.bg_width - this.bg_width); x2 < this.ps3D.pixel_width; x2 += this.bg_width) {
                            this.second_graphics_buffer.drawImage(second_bg_rotated_image, x2, start_y - this.bg_height, null);
                        }
                    }
                }
                int x;
                for (start_x = (x = ((this.ps3D.pixel_width + this.bg_width) / 2 + this.bg_x_offset - (int)this.ps3D.pixel_stereo_offset) % this.bg_width - this.bg_width); x < this.ps3D.pixel_width; x += this.bg_width) {
                    this.graphics_buffer.drawImage(first_bg_image, x, start_y, null);
                }
                if (this.ps3D.is_stereo) {
                    int x2;
                    for (start_x = (x2 = ((this.ps3D.pixel_width + this.bg_width) / 2 + this.bg_x_offset + (int)this.ps3D.pixel_stereo_offset) % this.bg_width - this.bg_width); x2 < this.ps3D.pixel_width; x2 += this.bg_width) {
                        this.second_graphics_buffer.drawImage(second_bg_image, x2, start_y, null);
                    }
                }
                if (start_y + this.bg_height < this.ps3D.pixel_height) {
                    ...
                    if (this.ps3D.is_stereo) {
                        int x3;
                        for (start_x = (x3 = ((this.ps3D.pixel_width + this.bg_width) / 2 + bg_rotated_x_offset + (int)this.ps3D.pixel_stereo_offset) % this.bg_width - this.bg_width); x3 < this.ps3D.pixel_width; x3 += this.bg_width) {
                ...
                for (int start_y = y = ((this.ps3D.pixel_height + this.bg_height) / 2 + this.bg_y_offset) % this.bg_height - this.bg_height; y < this.ps3D.pixel_height; y += this.bg_height) {
                    ...
                    for (int start_x2 = x2 = ((this.ps3D.pixel_width + this.bg_width) / 2 + this.bg_x_offset - local_pixel_stereo_offset) % this.bg_width - this.bg_width; x2 < this.ps3D.pixel_width; x2 += this.bg_width) {
                        this.graphics_buffer.drawImage(first_bg_image, x2, y, null);
                    }
                    if (this.ps3D.is_stereo) {
                        int x3;
                        for (int start_x2 = x3 = ((this.ps3D.pixel_width + this.bg_width) / 2 + this.bg_x_offset + local_pixel_stereo_offset) % this.bg_width - this.bg_width; x3 < this.ps3D.pixel_width; x3 += this.bg_width) {
                            this.second_graphics_buffer.drawImage(second_bg_image, x3, y, null);
                        }
                    }
                }
            }
    ...
    static {
        Live.empty_string = "";
        Live.enter_plain_string = "LiveGraphics3D 1.60";
        Live.enter_drag_string = "LiveGraphics3D 1.60: Please drag to rotate.";
        Live.enter_drag_animated_string = "LiveGraphics3D 1.60: Drag to rotate; double click to stop/restart.";
        Live.enter_animated_string = "LiveGraphics3D 1.60: Please double click to stop/restart.";
        Live.enter_init_string = "Initializing LiveGraphics3D 1.60. Please wait.";
