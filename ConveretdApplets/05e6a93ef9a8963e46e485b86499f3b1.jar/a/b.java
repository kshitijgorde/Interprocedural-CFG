...
final class b extends az implements aB
{
    private boolean q;
    private Event q;
    private String q;
    ...
    public final boolean handleEvent(final Event q) {
        ...
        final int q3 = this.q.q();
        final int w2;
        final av av = ((w2 = this.q.w(q.x)) >= 0) ? this.q.q(w2) : null;
        bP arg;
        if (q2 >= 0 && q2 < q3) {
            arg = bf.q(this.q).elementAt(q2);
        }
        else {
            arg = null;
        }
        switch (q.id) {
            ...
            case 501: {
                if (q.metaDown()) {
                    ...
                    if (bf.q(this.w) && q2 < bf.w(this.w).size() && arg != null && arg.q instanceof bV) {
                        if (bf.q(this.w).q((Object)arg.q)) {
                            bf.q(this.w).w(arg.q);
                        }
                        else {
                            bf.q(this.w).q((bV)arg.q);
                        }
                    }
                    this.repaint();
                    this.requestFocus();
                    this.q = q;
                    if ((this.q.arg = arg) != null && arg.q && q.clickCount > 1 && q.clickCount != 3 && q2 == w && q2 >= 0) {
    ...
    public b(final bf w, final bf q) {
        this.w = w;
        this.q = false;
        this.q = -1;
        this.q = q;
