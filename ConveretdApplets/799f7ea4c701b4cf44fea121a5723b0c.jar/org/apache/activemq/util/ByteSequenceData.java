...
public final class ByteSequenceData
{
    ...
    public static long readLongBig(final ByteSequence packet) throws IOException {
        ...
        return (read(packet) << 56) + (read(packet) << 48) + (read(packet) << 40) + (read(packet) << 32) + (read(packet) << 24) + (read(packet) << 16) + (read(packet) << 8) + (read(packet) << 0);
    }
    
    public static long readLongLittle(final ByteSequence packet) throws IOException {
        spaceNeeded(packet, 8);
        return (read(packet) << 0) + (read(packet) << 8) + (read(packet) << 16) + (read(packet) << 24) + (read(packet) << 32) + (read(packet) << 40) + (read(packet) << 48) + (read(packet) << 56);
    }
