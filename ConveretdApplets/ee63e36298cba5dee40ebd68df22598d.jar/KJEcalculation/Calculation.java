...
public abstract class Calculation
{
    ...
    public String JavaScriptReport(final String s, final String s2) throws CalculationException {
        this.bWithSchedule = true;
        this.recalculate();
        this.bWithSchedule = false;
        final String replace = replace("**GRAPH2**", s2, replace("**GRAPH**", s, replace("**GRAPH**", String.valueOf(this.sChrome) + " " + s, replace("&amp;NBSP;", "&nbsp;", replace("QQ_HEADER_QQ", this.MHI, replace("CURRENCY_LABEL", this.sCurrency, this.formatReport(replace("&QCPQ;", "&copy;", replace("&NBSP;", "&nbsp;", replace("**Q**", "\"", replace("&QQ", "&#", replace("<START>", "<HTML><HEAD>", this._sMessage))))))))))));
