...
public abstract class LongDigest implements ExtendedDigest
{
    ...
    protected void processWord(final byte[] array, final int n) {
        this.W[this.wOff++] = ((array[n] & 0xFF) << 56 | (array[n + 1] & 0xFF) << 48 | (array[n + 2] & 0xFF) << 40 | (array[n + 3] & 0xFF) << 32 | (array[n + 4] & 0xFF) << 24 | (array[n + 5] & 0xFF) << 16 | (array[n + 6] & 0xFF) << 8 | (array[n + 7] & 0xFF));
        if (this.wOff == 16) {
            this.processBlock();
        }
    }
    
    protected void unpackWord(final long n, final byte[] array, final int n2) {
        array[n2] = (byte)(n >>> 56);
