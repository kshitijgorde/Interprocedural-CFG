...
public final class HexSupport
{
    ...
    public static String toHexFromBytes(final byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; ++i) {
            rc.append(HexSupport.HEX_TABLE[0xFF & bytes[i]]);
        }
        return rc.toString();
    }
    
    public static String toHexFromInt(final int value, boolean trim) {
        final StringBuffer rc = new StringBuffer(HexSupport.INT_OFFSETS.length * 2);
        for (int i = 0; i < HexSupport.INT_OFFSETS.length; ++i) {
            final int b = 0xFF & value >> HexSupport.INT_OFFSETS[i];
            if (!trim || b != 0) {
                rc.append(HexSupport.HEX_TABLE[b]);
                trim = false;
            }
        }
        return rc.toString();
