...
class LPMOFArray extends WorldArray
{
    public float maxhop;
    ...
    public LPMOFArray(final int n, final int n2, final MofPanel p3) {
        super(n, n2);
    ...
    public void Compute() {
        ...
        for (int i = 0; i < this.Width(); ++i) {
            ...
            for (int j = 0; j < this.Length(); ++j) {
                ...
                float n = 0.0f;
                float fmin = 100.0f;
                float n2 = getValue2 + LPMOFArray.pi;
                if (n2 > LPMOFArray.twopi) {
                    n2 -= LPMOFArray.twopi;
                }
                final float n3 = LPMOFArray.twopi - getValue;
                ...
                final int n10 = (int)(n3 / (2.0f * WorldArray.acos(LPMOFArray.r0 / (n9 + LPMOFArray.r0)))) + 1;
                final float n11 = n3 / (2.0f * n10);
                final float n12 = this.m9_1 / WorldArray.cos(WorldArray.fmin(WorldArray.fmax(WorldArray.atan(WorldArray.sin(n11) / ((LPMOFArray.r0 + n9) / LPMOFArray.r0 - WorldArray.cos(n11))), this.min_phi), this.max_phi));
                final float n13 = this.sex_1 - this.sex_2 * WorldArray.pow(WorldArray.cos(WorldArray.fmin(WorldArray.fmax(WorldArray.atan((WorldArray.cos(n11) - LPMOFArray.r0 / (n9 + LPMOFArray.r0)) / WorldArray.sin(n11)), this.min_takeoff), this.max_takeoff)), this.sex_3);
                for (int l = 1; l <= n10; ++l) {
                ...
                float fmin3 = WorldArray.fmin(WorldArray.fmax(fmin * n12, 1.0), 50.0);
                final float n15 = n * WorldArray.pow(WorldArray.fabs(n13), this.sex_4);
                if (fmin3 < WorldArray.fmin(WorldArray.fmax(this.clof_1 + this.clof_2 * n15 + this.clof_3 * WorldArray.pow(n15, this.clof_4), 1.0), 50.0) && this.lp_blackout_flag) {
                    fmin3 = 0.0f;
                }
