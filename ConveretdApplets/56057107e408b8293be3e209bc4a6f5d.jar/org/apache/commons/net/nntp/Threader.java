...
public class Threader
{
    ...
    public Threadable thread(final Threadable[] messages) {
        ...
        final Threadable result = (this.root.child == null) ? null : this.root.child.threadable;
        this.root.flush();
        this.root = null;
        return result;
    }
    ...
    private void buildContainer(final Threadable threadable) {
        ...
        for (int i = 0; i < references.length; ++i) {
            final String refString = references[i];
    ...
    private ThreadContainer findRootSet() {
        ...
        for (final Object key : this.idTable.keySet()) {
            final ThreadContainer c = this.idTable.get(key);
            if (c.parent == null) {
                if (c.next != null) {
    ...
    private void gatherSubjects() {
        ...
        HashMap subjectTable = new HashMap((int)(count * 1.2), 0.9f);
        count = 0;
        ...
        ThreadContainer prev = null;
        for (ThreadContainer c3 = this.root.child, rest = c3.next; c3 != null; c3 = rest, rest = ((rest == null) ? null : rest.next)) {
