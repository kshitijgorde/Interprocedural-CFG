            ...
            if (wireFormat.isStackTraceEnabled()) {
                if (BaseDataStreamMarshaller.STACK_TRACE_ELEMENT_CONSTRUCTOR != null) {
                    ...
                    for (int i = 0; i < ss.length; ++i) {
                        try {
                            ss[i] = BaseDataStreamMarshaller.STACK_TRACE_ELEMENT_CONSTRUCTOR.newInstance(this.tightUnmarshalString(dataIn, bs), this.tightUnmarshalString(dataIn, bs), this.tightUnmarshalString(dataIn, bs), new Integer(dataIn.readInt()));
                        }
                        catch (IOException e) {
                            throw e;
                        }
        ...
        if (dataIn.readBoolean()) {
            ...
            if (wireFormat.isStackTraceEnabled()) {
                if (BaseDataStreamMarshaller.STACK_TRACE_ELEMENT_CONSTRUCTOR != null) {
                    ...
                    for (int i = 0; i < ss.length; ++i) {
                        try {
                            ss[i] = BaseDataStreamMarshaller.STACK_TRACE_ELEMENT_CONSTRUCTOR.newInstance(this.looseUnmarshalString(dataIn), this.looseUnmarshalString(dataIn), this.looseUnmarshalString(dataIn), new Integer(dataIn.readInt()));
                        }
                        catch (IOException e) {
                            throw e;
                        }
