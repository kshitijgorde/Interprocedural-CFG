...
class StaticCodeBook
{
    ...
    StaticCodeBook(final int dim, final int entries, final int[] lengthlist, final int maptype, final int q_min, final int q_delta, final int q_quant, final int q_sequencep, final int[] quantlist, final Object nearest_tree, final Object thresh_tree) {
        this();
        this.dim = dim;
            ...
            for (i = 1; i < this.entries; ++i) {
                ...
                if (_this > _last) {
                    for (int j = _last; j < _this; ++j) {
                        opb.write(i - count, ilog(this.entries - count));
                        count = i;
                    }
                }
            }
        ...
        switch (opb.read(1)) {
            ...
            case 1: {
                ...
                while (i < this.entries) {
                    final int num2 = opb.read(ilog(this.entries - i));
                    if (num2 == -1) {
                        this.clear();
                        return -1;
                    }
            ...
            switch (this.maptype) {
                ...
                case 2: {
                    for (int j = 0; j < this.entries; ++j) {
                        float last = 0.0f;
                        for (int i = 0; i < this.dim; ++i) {
                            float val2 = this.quantlist[j * this.dim + i];
    ...
    static float float32_unpack(final int val) {
        ...
        final float sign = val & Integer.MIN_VALUE;
