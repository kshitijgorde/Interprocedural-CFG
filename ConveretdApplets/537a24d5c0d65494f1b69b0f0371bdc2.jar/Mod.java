...
public class Mod
{
    ...
    Mod(final String s, final Applet applet) {
        try {
            final DataInputStream dataInputStream = new DataInputStream(new URL(applet.getCodeBase(), s).openStream());
    ...
    static final String readText(final DataInputStream dataInputStream, final int n) throws IOException {
    ...
    void readPatterns(final DataInputStream dataInputStream) throws IOException {
        final int n = this.numtracks * 4 * 64;
        this.patterns = new byte[this.numpatterns][];
        for (int i = 0; i < this.numpatterns; ++i) {
            dataInputStream.readFully(this.patterns[i] = new byte[n], 0, n);
        }
    }
