...
public class JCString extends JCVector
{
    ...
    private static void parseToken(final Component component, final JCString jcString, final String s) {
        ...
        if (substring2.equalsIgnoreCase("DEFAULT_FONT")) {
        ...
        else if (substring2.equalsIgnoreCase("DEFAULT_COLOR")) {
            jcString.add(JCString.DEFAULT_COLOR);
        }
        else if (substring2.equalsIgnoreCase("UNDERLINE") || substring2.equalsIgnoreCase("UL")) {
            jcString.add(JCString.UNDERLINE);
        }
        else if (substring2.equalsIgnoreCase("/UNDERLINE") || substring2.equalsIgnoreCase("/UL")) {
            jcString.add(JCString.END_UNDERLINE);
        }
        else if (substring2.equalsIgnoreCase("STRIKETHROUGH") || substring2.equalsIgnoreCase("ST")) {
            jcString.add(JCString.STRIKETHROUGH);
        }
        else if (substring2.equalsIgnoreCase("/STRIKETHROUGH") || substring2.equalsIgnoreCase("/ST")) {
            jcString.add(JCString.END_STRIKETHROUGH);
        }
        else if (substring2.equalsIgnoreCase("HREF")) {
        ...
        else if (substring2.equalsIgnoreCase("ALIGN")) {
            if (substring.equalsIgnoreCase("TOP")) {
                jcString.add(JCString.TOP);
            }
            else if (substring.equalsIgnoreCase("MIDDLE")) {
                jcString.add(JCString.MIDDLE);
            }
            else if (substring.equalsIgnoreCase("BOTTOM")) {
        ...
        else if (substring2.equalsIgnoreCase("RESET")) {
            jcString.add(JCString.RESET);
        }
        else if (substring2.equalsIgnoreCase("COLOR")) {
            if (substring != null) {
            ...
            if (substring != null) {
                jcString.add(JCUtilConverter.toImage(component, substring));
            }
        }
    ...
    private boolean addImage(final Applet applet, final URL url, final String s) {
        ...
        final Image image = applet.getImage(url, s);
    ...
    void drawLine(final Component component, final Graphics graphics, final Font font, final Rectangle rectangle, final int n, final int n2) {
        ...
        for (int n4 = n; n4 < n2 && n4 < this.size(); ++n4) {
            if (super.elementData[n4] instanceof String) {
                ...
                x += stringWidth;
                if (this.url != null) {
                    this.endURL(graphics, x, y);
                }
            }
            else if (super.elementData[n4] instanceof Image) {
                if (this.url != null) {
                    this.startURL(x, rectangle.y);
                }
                graphics.drawImage((Image)super.elementData[n4], x, rectangle.y, null);
                x += ((Image)super.elementData[n4]).getWidth(null);
                if (this.url != null) {
