...
public class Format
{
    ...
    private String fixed_format(final double n) {
        ...
        final long n2 = (long)((this.precision == 0) ? (n + 0.5) : n);
        final String string = s + n2;
        final double n3 = n - n2;
        if (n3 >= 1.0 || n3 < 0.0) {
            return this.exp_format(n);
        }
        return string + this.frac_part(n3);
    }
    ...
    private String frac_part(final double n) {
        ...
        if (this.precision > 0) {
            ...
            s = string2.substring(string2.length() - this.precision, string2.length());
        }
        ...
        if ((this.fmt == 'G' || this.fmt == 'g') && !this.alternate) {
            int n4;
            for (n4 = s.length() - 1; n4 >= 0 && s.charAt(n4) == '0'; --n4) {}
            if (n4 >= 0 && s.charAt(n4) == '.') {
                --n4;
            }
            s = s.substring(0, n4 + 1);
        }
        return s;
    }
