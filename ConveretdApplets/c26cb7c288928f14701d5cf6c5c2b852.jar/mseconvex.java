...
public class mseconvex extends Applet implements Runnable
{
    Thread t;
    boolean suspended;
    String infile;
    ...
    public int blue(final int n, final int n2) {
        return mseconvex.data[n * this.width + n2] & 0xFF;
    }
    
    private void buildImage() {
        this.photo = this.createImage(new MemoryImageSource(this.width, this.height, mseconvex.dataNew, 0, this.width));
    }
    
    private void grabPixels() {
        final PixelGrabber pixelGrabber = new PixelGrabber(this.photo, 0, 0, this.width, this.height, mseconvex.data, 0, this.width);
        try {
    ...
    public int green(final int n, final int n2) {
        return (mseconvex.data[n * this.width + n2] & 0xFF00) >> 8;
    }
    ...
    public void init() {
        ...
        mseconvex.data = new int[this.height * this.width];
        mseconvex.dataNew = new int[this.height * this.width];
        this.grabPixels();
        this.restoreData();
        this.buildImage();
    ...
    private void program() {
        ...
        if (this.idold != this.id) {
            ...
            for (int i = n4; i < height; ++i) {
                for (int j = n3; j < width; ++j) {
                    mseconvex.dataNew[i * this.width + j] = mseconvex.data[i * this.width + j];
                }
            }
            this.idold = this.id;
        }
        ...
        for (int k = n6; k < height2; ++k) {
            for (int l = n5; l < width2; ++l) {
                ...
                float n10 = 1.0f;
                if (n9 < this.radius) {
                    n10 = this.convex - (this.convex - 1.0f) * n9 / this.radius;
                }
                final float n11 = n7 / n10 + n;
                final float n12 = n8 / n10 + n2;
                if (n9 < this.radius) {
                    if (n11 > 0.0f && n11 < this.width - 1 && n12 > 0.0f && n12 < this.height - 1) {
                        ...
                        this.setRGB(k, l, 255, 255, 255);
                    }
                }
                else {
                    mseconvex.dataNew[k * this.width + l] = mseconvex.data[k * this.width + l];
                }
            }
        }
        this.buildImage();
        this.convex += this.convexstep;
        if (this.convex >= this.maxConvex) {
            this.convexstep = -this.convexstep;
        }
        else if (this.convex <= 1.0f) {
            this.convexstep = -this.convexstep;
        }
    }
    
    public int red(final int n, final int n2) {
        return (mseconvex.data[n * this.width + n2] & 0xFF0000) >> 16;
    }
    ...
    private void scroll(final int n, final int n2) {
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                mseconvex.dataNew[(i + n2 + this.height) % this.height * this.width + (j + n + this.width) % this.width] = mseconvex.data[i * this.width + j];
            }
        }
        this.buildImage();
    }
    ...
    public void setRGB(final int n, final int n2, final int n3, final int n4, final int n5) {
        mseconvex.dataNew[n * this.width + n2] = new Color(n3, n4, n5).getRGB();
    }
    ...
    private void zoom(final int n) {
        ...
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                mseconvex.dataNew[i * this.width + j] = mseconvex.data[((i - n3) * 2 / n + this.yc[this.id] + this.height) % this.height * this.width + ((j - n2) * 2 / n + this.xc[this.id] + this.width) % this.width];
            }
        }
        this.buildImage();
    }
