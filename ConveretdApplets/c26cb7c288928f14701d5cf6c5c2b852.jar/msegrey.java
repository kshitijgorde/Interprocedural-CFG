...
public class msegrey extends Applet implements Runnable
{
    Thread t;
    boolean suspended;
    String infile;
    ...
    public int blue(final int n, final int n2) {
        return msegrey.data[n * this.width + n2] & 0xFF;
    }
    
    private void buildImage() {
        this.photo = this.createImage(new MemoryImageSource(this.width, this.height, msegrey.dataNew, 0, this.width));
    }
    
    private void grabPixels() {
        final PixelGrabber pixelGrabber = new PixelGrabber(this.photo, 0, 0, this.width, this.height, msegrey.data, 0, this.width);
        try {
    ...
    public int green(final int n, final int n2) {
        return (msegrey.data[n * this.width + n2] & 0xFF00) >> 8;
    }
    ...
    public void init() {
        ...
        msegrey.data = new int[this.height * this.width];
        msegrey.dataNew = new int[this.height * this.width];
        this.grabPixels();
        this.restoreData();
        this.buildImage();
    ...
    private void program() {
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                if (j >= this.XL[this.id] && j <= this.XH[this.id] && i >= this.YL[this.id] && i <= this.YH[this.id]) {
                    msegrey.dataNew[i * this.width + j] = msegrey.data[i * this.width + j];
                }
                else {
                    final int n = (this.red(i, j) + this.green(i, j) + this.blue(i, j)) / 3;
                    this.setRGB(i, j, n, n, n);
                }
            }
        }
        this.buildImage();
    ...
    public int red(final int n, final int n2) {
        return (msegrey.data[n * this.width + n2] & 0xFF0000) >> 16;
    }
    ...
    private void scroll(final int n, final int n2) {
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                msegrey.dataNew[(i + n2 + this.height) % this.height * this.width + (j + n + this.width) % this.width] = msegrey.data[i * this.width + j];
            }
        }
        this.buildImage();
    }
    ...
    public void setRGB(final int n, final int n2, final int n3, final int n4, final int n5) {
        msegrey.dataNew[n * this.width + n2] = new Color(n3, n4, n5).getRGB();
    }
    ...
    private void zoom(final int n) {
        ...
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                msegrey.dataNew[i * this.width + j] = msegrey.data[((i - n3) * 2 / n + this.yc[this.id] + this.height) % this.height * this.width + ((j - n2) * 2 / n + this.xc[this.id] + this.width) % this.width];
            }
        }
        this.buildImage();
    }
