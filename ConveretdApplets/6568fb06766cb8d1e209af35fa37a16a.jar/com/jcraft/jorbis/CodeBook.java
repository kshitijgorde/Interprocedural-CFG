...
class CodeBook
{
    ...
    int decodevs(final float[] array, final int n, final Buffer buffer, final int n2, final int n3) {
        ...
        switch (n3) {
            ...
            default: {
                System.err.println("CodeBook.decodeves: addmul=" + n3);
                break;
            }
        }
        return decode;
    ...
    int best(final float[] array, final int n) {
        ...
        if (thresh_tree != null) {
            int n3 = 0;
            for (int i = 0, n4 = n * (this.dim - 1); i < this.dim; ++i, n4 -= n) {
                int n5;
                for (n5 = 0; n5 < thresh_tree.threshvals - 1 && array[n4] >= thresh_tree.quantthresh[n5]; ++n5) {}
                n3 = n3 * thresh_tree.quantvals + thresh_tree.quantmap[n5];
            }
        ...
        if (nearest_tree != null) {
            do {
                float n6 = 0.0f;
                final int n7 = nearest_tree.p[n2];
                final int n8 = nearest_tree.q[n2];
                for (int j = 0, n9 = 0; j < this.dim; ++j, n9 += n) {
                    n6 += (float)((this.valuelist[n7 + j] - this.valuelist[n8 + j]) * (array[n9] - (this.valuelist[n7 + j] + this.valuelist[n8 + j]) * 0.5));
                }
                if (n6 > 0.0) {
                    n2 = -nearest_tree.ptr0[n2];
                }
                else {
                    n2 = -nearest_tree.ptr1[n2];
                }
            } while (n2 > 0);
            return -n2;
        }
        ...
        for (int k = 0; k < this.entries; ++k) {
            if (this.c.lengthlist[k] > 0) {
                final float dist = dist(this.dim, this.valuelist, n12, array, n);
                if (n10 == -1 || dist < n11) {
                    n11 = dist;
                    n10 = k;
                }
