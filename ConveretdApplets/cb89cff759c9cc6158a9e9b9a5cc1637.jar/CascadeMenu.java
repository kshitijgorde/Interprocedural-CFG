...
public class CascadeMenu extends Applet implements MouseListener
{
    ...
    public CascadeMenu(final AppletPanel ap) {
        this.lh = -2;
    ...
    public void init() {
        ...
        this.setLayout(null);
        this.itemh = Integer.parseInt(this.ap.getParameter("Item_Height"));
        this.offset = Integer.parseInt(this.ap.getParameter("Item_Offset"));
        if (this.ap.getParameter("Use_animation").toUpperCase().equals("YES")) {
            this.useAnim = true;
        }
        if (this.ap.getParameter("Use_shadow").toUpperCase().equals("YES")) {
            this.useShadow = true;
        }
        this.dicon = this.ap.getParameter("Default_icon");
        this.dtarget = this.ap.getParameter("Default_target");
        this.ms = this.ap.getTree("Item_", true);
        System.out.println(this.ms);
        this.logo = this.ap.getImage(this.ap.getCodeBase(), this.ap.getParameter("bgimage"));
        this.setBackground(new Color(Integer.parseInt(this.ap.getParameter("bgcolor"), 16)));
        this.menuBarHeight = Integer.parseInt(this.ap.getParameter("Menubar_Height"));
        System.out.println(this.getWidth() + " " + this.menuBarHeight);
        System.out.println("*** Go MenuBar ****");
    ...
    public void paint(final Graphics graphics) {
        ...
        if (this.logo.getWidth(this) / 2 > 0) {
    ...
    public void performAction(final Item item) {
        this.ap.showDocument(item.getAction(), item.getTarget());
    }
    
    public void show(final String s) {
        this.ap.showStatus(s);
    }
    ...
    public Color getColor(String s, final int n, final int n2) {
        ...
        try {
            if (n2 == 0) {
                color = new Color(Integer.parseInt(this.ap.getParameter(s + "_bgcolor" + s2), 16));
            }
            else if (n2 == 1) {
                color = new Color(Integer.parseInt(this.ap.getParameter(s + "_fgcolor" + s2), 16));
            }
        }
        catch (Exception ex) {}
        return color;
    ...
    public Font getFont(String s, final int n) {
        ...
        try {
            font = new Font(this.ap.getParameter(s + "_fontstyle" + s2), Integer.parseInt(this.ap.getParameter(s + "_fonttype" + s2)), Integer.parseInt(this.ap.getParameter(s + "_fontsize" + s2)));
        }
        catch (Exception ex) {}
        return font;
    }
    ...
    public void mouseExited(final MouseEvent mouseEvent) {
        ...
        System.out.println("Xco: " + x + "Yco: " + y + " " + this.getHeight() + " " + this.getWidth());
    ...
    private MenuSet getTree(final String s, final boolean b) {
        ...
        while (this.exists(s + n)) {
            ...
            String s2 = this.getParameter(string + "_Icon");
            if (s2 == null || s2.length() < 1) {
                s2 = this.dicon;
            }
            System.out.println(string + "_Icon is " + s2);
            String s3 = this.getParameter(string + "_Target");
            if (s3 == null) {
                s3 = this.dtarget;
            }
            if (s2 != null) {
            ...
            final Item item = new Item(this.getParameter(string + "_Text"), this.makeURL(string), s3, b, image);
            System.out.println("Created item: " + item);
            if (this.exists(s + n + "_1")) {
                item.giveSub(this.getTree(s + n + "_", false));
            }
            ++n;
            set.addItem(item);
        }
    ...
    private URL makeURL(final String s) {
        URL url = null;
        try {
            final String parameter = this.getParameter(s + "_Action");
            if (parameter != null) {
    ...
    private boolean exists(final String s) {
        return this.getParameter(s + "_Text") != null;
    }
    
    private boolean checkRegistrationKey() {
        final int n = 13900;
        final String parameter = this.ap.getParameter("RegistrationKey");
        System.out.println("Key:" + parameter);
        if (parameter == null || parameter.equals("") || parameter.equalsIgnoreCase("Unregistered")) {
            return false;
        }
        final String protocol = this.ap.getCodeBase().getProtocol();
        final String upperCase = this.ap.getCodeBase().getHost().toUpperCase();
