...
public class LogarithmicAxis extends NumberAxis
{
    ...
    public void setAutoRangeNextLogFlag(final boolean flag) {
    ...
    public double switchedPow10(final double val) {
    ...
    public double adjustedLog10(double val) {
        ...
        final double res = Math.log(val) / LogarithmicAxis.LOG10_VALUE;
    ...
    public double adjustedPow10(double val) {
        ...
        double res;
        if (val < 1.0) {
            res = (Math.pow(10.0, val + 1.0) - 10.0) / 9.0;
        }
        else {
            res = Math.pow(10.0, val);
        }
        return negFlag ? (-res) : res;
    }
    ...
    public void autoAdjustRange() {
        ...
        if (plot instanceof ValueAxisPlot) {
            ...
            if (lower > 0.0 && (lowerMargin = this.getLowerMargin()) > 0.0) {
                final double logLower = Math.log(lower) / LogarithmicAxis.LOG10_VALUE;
                double logAbs;
            ...
            if (upper > 0.0 && (upperMargin = this.getUpperMargin()) > 0.0) {
                final double logUpper = Math.log(upper) / LogarithmicAxis.LOG10_VALUE;
                double logAbs2;
                if ((logAbs2 = Math.abs(logUpper)) < 1.0) {
                    logAbs2 = 1.0;
                }
                upper = Math.pow(10.0, logUpper + logAbs2 * upperMargin);
            }
    ...
    public void zoomRange(final double lowerPercent, final double upperPercent) {
        final double startLog = this.switchedLog10(this.getRange().getLowerBound());
        final double lengthLog = this.switchedLog10(this.getRange().getUpperBound()) - startLog;
        Range adjusted;
        if (this.isInverted()) {
            adjusted = new Range(this.switchedPow10(startLog + lengthLog * (1.0 - upperPercent)), this.switchedPow10(startLog + lengthLog * (1.0 - lowerPercent)));
        }
        else {
            adjusted = new Range(this.switchedPow10(startLog + lengthLog * lowerPercent), this.switchedPow10(startLog + lengthLog * upperPercent));
        }
