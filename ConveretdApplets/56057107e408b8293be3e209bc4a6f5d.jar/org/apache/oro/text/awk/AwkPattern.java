...
public final class AwkPattern implements Pattern, Serializable
{
    ...
    AwkPattern(final String expression, final SyntaxTree syntaxTree) {
        ...
        final DFAState dfaState = new DFAState((BitSet)this._U.clone(), this._numStates);
        (this._stateMap = new Hashtable()).put(dfaState._state, dfaState);
    ...
    void _createNewState(final int n, final int n2, final int[] array) {
        final DFAState dfaState = this._stateList.elementAt(n);
        int size = this._nodeList[n2].size();
        this._U.xor(this._U);
        while (size-- > 0) {
            final int position = this._nodeList[n2].elementAt(size)._position;
        ...
        if (!this._stateMap.containsKey(this._U)) {
            ...
            array[n2] = this._stateMap.get(this._U)._stateNumber;
        }
    }
    
    int[] _getStateArray(final int n) {
        return this._Dtrans.elementAt(n);
    }
