...
public class SHA1Digest extends GeneralDigest
{
    ...
    protected void processWord(final byte[] array, final int n) {
        this.X[this.xOff++] = ((array[n] & 0xFF) << 24 | (array[n + 1] & 0xFF) << 16 | (array[n + 2] & 0xFF) << 8 | (array[n + 3] & 0xFF));
        if (this.xOff == 16) {
            this.processBlock();
        }
    }
    
    private void unpackWord(final int n, final byte[] array, int n2) {
        array[n2++] = (byte)(n >>> 24);
        array[n2++] = (byte)(n >>> 16);
        array[n2++] = (byte)(n >>> 8);
        array[n2++] = (byte)n;
    ...
    public int doFinal(final byte[] array, final int n) {
        ...
        this.unpackWord(this.H5, array, n + 16);
        this.reset();
        return 20;
    }
