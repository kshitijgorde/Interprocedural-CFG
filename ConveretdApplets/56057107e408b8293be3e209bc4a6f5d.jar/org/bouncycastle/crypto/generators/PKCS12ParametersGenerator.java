...
public class PKCS12ParametersGenerator extends PBEParametersGenerator
{
    ...
    private byte[] generateDerivedKey(final int n, final int n2) {
        ...
        byte[] array3;
        if (super.salt != null && super.salt.length != 0) {
            array3 = new byte[this.v * ((super.salt.length + this.v - 1) / this.v)];
            for (int j = 0; j != array3.length; ++j) {
                array3[j] = super.salt[j % super.salt.length];
            }
        }
        else {
            array3 = new byte[0];
        ...
        byte[] array4;
        if (super.password != null && super.password.length != 0) {
            array4 = new byte[this.v * ((super.password.length + this.v - 1) / this.v)];
            for (int k = 0; k != array4.length; ++k) {
                array4[k] = super.password[k % super.password.length];
            }
        }
        else {
            array4 = new byte[0];
        ...
        for (int n3 = (n2 + this.u - 1) / this.u, l = 1; l <= n3; ++l) {
            ...
            for (int n4 = 1; n4 != super.iterationCount; ++n4) {
