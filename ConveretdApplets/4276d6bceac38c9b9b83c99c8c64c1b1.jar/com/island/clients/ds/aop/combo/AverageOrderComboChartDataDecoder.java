...
import com.island.clients.ds.aop.Event;
...
public class AverageOrderComboChartDataDecoder implements DataDecoder
{
    protected static final float DECIMAL = 1000.0f;
    protected static final float MINIMUM_PRICE_RANGE = 0.05f;
    ...
    public AverageOrderComboChartDataDecoder(final URL streamer, final int timeLimit) throws IOException {
        this.eventList = new EventList();
        this.streamer = streamer;
        this.timeLimit = timeLimit;
        this.dataList = new DataList(timeLimit);
        this.istream = streamer.openStream();
    }
    ...
    public DataList getOrderChartData(final String symbol, final int maxShares) {
        try {
            if (this.istream.available() > 0) {
                final char type = (char)this.istream.read();
                if (type == 'N') {
                    this.handleNPacket(symbol, maxShares);
                }
                else if (type == 'H') {
                    this.handleHPacket(symbol, maxShares);
                }
                else if (type == 'S') {
                    this.handleSPacket(symbol, maxShares);
                }
                else if (type == 'C') {
                    this.handleCPacket(symbol, maxShares);
                }
                else {
                    final AverageOrderAppletData chartData = new AverageOrderAppletData(symbol, maxShares);
        ...
        final DataList out = this.dataList;
        this.dataList = new DataList(this.timeLimit);
        return out;
    }
    
    protected void handleNPacket(final String symbol, final int maxShares) {
        System.out.print("N");
        this.dataList.add(this.getInvalidSymbolData(symbol, maxShares));
    }
    
    protected void handleHPacket(final String symbol, final int maxShares) {
        System.out.print("H");
        this.dataList.add(this.getHeartBeatData(symbol, maxShares));
    }
    
    protected void handleSPacket(final String symbol, final int maxShares) throws IOException {
        System.out.print("S");
        if (this.book != null) {
            this.readEvents(this.istream);
            while (this.eventList.size > 0) {
                this.updateBook();
                this.dataList.add(this.book.getOrderData(symbol, maxShares));
            }
        }
        else {
    ...
    protected void handleCPacket(final String symbol, final int maxShares) throws IOException {
        System.out.print("C");
    ...
    class Book
    {
        ...
        protected AverageOrderAppletData getOrderData(final String symbol, final int maxShares) {
            ...
            float low = -1.0f;
            float high = -1.0f;
            final float lastPrice = AverageOrderComboChartDataDecoder.this.book.lastPrice / 1000.0f;
            data.setLastPrice(lastPrice);
            final int bidSize = AverageOrderComboChartDataDecoder.this.book.bidSize;
            ...
            if (bidSize > 0) {
                ...
                Order order = AverageOrderComboChartDataDecoder.this.book.bids;
                for (int i = 0; i < bidSize; ++i) {
            ...
            final int askSize = AverageOrderComboChartDataDecoder.this.book.askSize;
            if (askSize > 0) {
                long sum2 = 0L;
                int totShares2 = 0;
                asks = new SimpleOrder.List();
                Order order2 = AverageOrderComboChartDataDecoder.this.book.asks;
                for (int j = 0; j < askSize; ++j) {
            ...
            data.setMatchedShares((long)AverageOrderComboChartDataDecoder.this.book.matchedShares);
            data.setNetChange(AverageOrderComboChartDataDecoder.this.book.netChange / 1000.0f);
            data.setTimeStamp((long)AverageOrderComboChartDataDecoder.this.book.timeStamp);
            data.setUpdateTime(data.getTimeStamp());
            data.setLow(low);
            data.setHigh(high);
            data.setDataType(0);
