...
public class CascadeMenu extends Applet implements MouseListener
{
    ...
    public void init() {
        ...
        if (!this.useVert) {
            ...
            (this.mb = new MenuBar(this.menuBarHeight, this.getHeight(), this.ms, this, this.registered)).setBounds(0, 0, this.menuBarHeight, this.getHeight());
    ...
    public Color getColor(String s, final int n, final int n2) {
        ...
        try {
            if (n2 == 0) {
                color = new Color(Integer.parseInt(this.getParameter(s + "_bgcolor" + s2), 16));
            }
            else if (n2 == 1) {
                color = new Color(Integer.parseInt(this.getParameter(s + "_fgcolor" + s2), 16));
            }
        }
        catch (Exception ex) {}
        return color;
    ...
    public Font getFont(String s, final int n) {
        ...
        try {
            font = new Font(this.getParameter(s + "_fontstyle" + s2), Integer.parseInt(this.getParameter(s + "_fonttype" + s2)), Integer.parseInt(this.getParameter(s + "_fontsize" + s2)));
            final FontMetrics fontMetrics = this.getFontMetrics(font);
            final int fontheight = fontMetrics.getMaxDescent() + fontMetrics.getMaxAscent();
