...
public class DS_Ripple extends Applet implements Runnable
{
    Thread _fld0;
    int _fld2;
    Font _fld3;
    ...
    void _mth1b() {
        ...
        this._fld4e = ((this._fld4e >= 10) ? ((this._fld4e <= 60) ? this._fld4e : 60) : 10);
        String parameter2 = this.getParameter("frequency");
        if (parameter2 == null) {
            parameter2 = "20";
        }
        this._fld5e = Integer.valueOf(parameter2);
        this._fld5e = ((this._fld5e >= 10) ? ((this._fld5e <= 30) ? this._fld5e : 30) : 10);
        String parameter3 = this.getParameter("pause");
        if (parameter3 == null) {
            parameter3 = "1";
        }
        this._fld8e = Integer.valueOf(parameter3);
        this._fld8e = ((this._fld8e >= 0) ? ((this._fld8e <= 8) ? this._fld8e : 8) : 0);
        this._fld8e *= 50;
    ...
    void _mth2b() {
        ...
        for (int i = 0; i < this._fld5; ++i) {
            for (int j = 0; j < this._fld4; ++j) {
                this._fld2e[i * this._fld4 + j] = (int)Math.sqrt(j * j + i * i);
            }
        }
        this._fld9e = (this._fld8e << 1) + (this._fld4e << 1);
        this._fld0f = this._fld9e << 1;
        this._fld7e = new int[this._fld9e];
        for (int k = 0; k < this._fld8e; ++k) {
            this._fld7e[k] = 0;
        }
        for (int l = this._fld8e; l < this._fld4e + this._fld8e; ++l) {
            this._fld7e[l] = l - this._fld8e;
        }
        for (int n = 0; n < this._fld4e; ++n) {
            this._fld7e[this._fld8e + this._fld4e + n] = this._fld4e - n;
        }
        for (int n2 = 0; n2 < this._fld8e; ++n2) {
            this._fld7e[this._fld8e + this._fld4e + this._fld4e + n2] = 0;
        }
    }
    
    void _mth3b() {
        if (this._fld6e > 0) {
            if (this._fld6e < this._fld0f) {
                --this._fld6e;
                return;
            }
            ++this._fld6e;
    ...
    void _mth4b() {
        ...
        this._fld6e %= this._fld9e;
        final int n = this._fld7e[this._fld6e];
        for (int i = 0; i < this._fld5; ++i) {
            this._fld3e[i] = (int)(n * Math.sin(this._fld5e * (i - this._fld6e) * 3.141592653589793 / 180.0));
        }
        this._mth5b();
    ...
    void _mth5b() {
        ...
        for (int i = 0; i < this._fld5; ++i) {
            for (int j = 0; j < this._fld4; ++j) {
                final int n4 = this._fld3e[this._fld2e[Math.abs(i - n2) * this._fld4 + Math.abs(j - n)]];
                final int n5 = j;
                int n6 = i + n4;
                if (n6 > this._fld5 - 1) {
                    n6 = this._fld5 - 1;
                ...
                this._fld7[n3++] = this._fld6[n6 * this._fld4 + n5];
            }
        }
    }
