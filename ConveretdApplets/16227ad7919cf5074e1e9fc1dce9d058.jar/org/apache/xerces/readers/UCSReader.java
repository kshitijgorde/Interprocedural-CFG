...
final class UCSReader extends XMLEntityReader implements StringProducer
{
    ...
    public int addString(final int n, final int n2) {
        ...
        return this.fStringPool.addString((StringPool.StringProducer)this, n, n2);
    }
    ...
    public int addSymbol(final int n, final int n2) {
        ...
        return this.fStringPool.addSymbol((StringPool.StringProducer)this, n, n2, this.getHashcode(n, n2));
    }
    
    public void append(final CharBuffer charBuffer, int i, final int n) {
        while (i < i + n) {
            int char1;
            try {
    ...
    public void callCharDataHandler(int i, final int n, final boolean b) throws Exception {
        while (i < i + n) {
            this.appendCharData(this.getChar(i));
            i += this.fBytesPerChar;
        }
        if (super.fSendCharDataAsCharArray) {
            if (b) {
                super.fEntityHandler.processWhitespace(UCSReader.fCharacters, 0, this.fCharDataLength);
            }
            else {
                super.fEntityHandler.processCharacters(UCSReader.fCharacters, 0, this.fCharDataLength);
            }
        }
        else {
    ...
    public void skipPastName(final char c) throws Exception {
        if ((XMLCharacterProperties.fgCharFlags[this.getChar(super.fCurrentOffset)] & 0x2) == 0x0) {
            return;
        }
        int char1;
        do {
            super.fCurrentOffset += this.fBytesPerChar;
            ++super.fCharacterCounter;
            char1 = this.getChar(super.fCurrentOffset);
            if (c == char1) {
                return;
            }
        } while ((XMLCharacterProperties.fgCharFlags[char1] & 0x4) != 0x0);
    }
    
    public void skipPastNmtoken(final char c) throws Exception {
        for (int n = this.getChar(super.fCurrentOffset); c != n; n = this.getChar(super.fCurrentOffset)) {
    ...
    public boolean scanExpectedName(final char c, final CharArrayRange charArrayRange) throws Exception {
        final int fCurrentOffset = super.fCurrentOffset;
        this.skipPastName(c);
    ...
    public int scanContent(final int n) throws Exception {
        final int fCurrentOffset = super.fCurrentOffset;
        int n2 = this.getChar(super.fCurrentOffset);
        super.fCurrentOffset += this.fBytesPerChar;
        if (n2 < 128) {
            if (n2 == -1) {
                super.fCurrentOffset -= this.fBytesPerChar;
            ...
            if ((XMLCharacterProperties.fgCharFlags[n2] & 0x1) == 0x0 && n2 != 10 && n2 != 13) {
                if (n2 == 60) {
                    ++super.fCharacterCounter;
                    if (!super.fInCDSect) {
        ...
        int n3;
        while (true) {
            n3 = this.getChar(super.fCurrentOffset);
            super.fCurrentOffset += this.fBytesPerChar;
            if (n3 >= 128) {
                break;
            }
            ...
            if ((XMLCharacterProperties.fgCharFlags[n3] & 0x1) == 0x0) {
                if (n3 == 10) {
                    ++super.fLinefeedCounter;
                    super.fCharacterCounter = 1;
                }
