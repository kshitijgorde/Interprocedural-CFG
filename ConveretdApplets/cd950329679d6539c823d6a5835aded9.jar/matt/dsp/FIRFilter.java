...
public class FIRFilter
{
    ...
    public FIRFilter() {
        this.a = new float[FIRFilter.taps];
    }
    ...
    public float[] filter(final float[] ip) {
        final float[] paddedip = new float[ip.length + this.a.length];
        final int nSamples = paddedip.length;
        final float[] x = new float[FIRFilter.taps];
        final float[] op = new float[nSamples];
        for (int i = 0; i < this.a.length; ++i) {
            paddedip[i] = ip[0];
        }
        for (int i = this.a.length; i < paddedip.length; ++i) {
            paddedip[i] = ip[i - this.a.length];
        }
        ...
        for (int k = 1; k < FIRFilter.taps; ++k) {
            x[k] = 0.0f;
        }
        for (int i = 0; i < nSamples; ++i) {
            x[0] = paddedip[i];
            float y = 0.0f;
            for (int j = 0; j < FIRFilter.taps; ++j) {
                y += this.a[j] * x[j];
            }
            op[i] = y;
            for (int j = FIRFilter.taps - 1; j > 0; --j) {
                x[j] = x[j - 1];
            }
        }
        final float[] unpaddedop = new float[ip.length];
        for (int l = this.a.length; l < paddedip.length; ++l) {
            unpaddedop[l - this.a.length] = op[l];
        }
        return unpaddedop;
    }
