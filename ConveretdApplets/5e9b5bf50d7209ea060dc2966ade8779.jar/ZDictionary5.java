...
class ZDictionary5 extends ZDictionary
{
    ZMachine zm;
    String separators;
    int nentries;
    ...
    ZDictionary5(final ZMachine zm, final int dict_address) {
        this.zm = zm;
    ...
    ZDictionary5(final ZMachine zm) {
    ...
    boolean parse_word(final int textloc, final int wordloc, final int wordlength, final int parseloc) {
        return this.parse_word(textloc, wordloc, wordlength, parseloc, true);
    }
    
    boolean parse_word(final int textloc, final int wordloc, final int wordlength, final int parseloc, final boolean flagunknown) {
        int first = 0;
        int last = this.nentries - 1;
        if (this.zm.memory_image[parseloc] == this.zm.memory_image[parseloc + 1]) {
            return true;
        }
        final short[] encword = this.zm.encode_word(wordloc, wordlength, 3);
        final long enclong = (encword[0] & 0xFFFFL) << 32 | (encword[1] & 0xFFFF) << 16 | (encword[2] & 0xFFFF);
        int middle = (last + first) / 2;
        int dictloc;
        long dictlong;
        while (true) {
            dictloc = this.wtable_addr + middle * this.entry_length;
            dictlong = ((this.zm.memory_image[dictloc] & 0xFFL) << 40 | (this.zm.memory_image[dictloc + 1] & 0xFFL) << 32 | (this.zm.memory_image[dictloc + 2] & 0xFFL) << 24 | (this.zm.memory_image[dictloc + 3] & 0xFFL) << 16 | (this.zm.memory_image[dictloc + 4] & 0xFFL) << 8 | (this.zm.memory_image[dictloc + 5] & 0xFFL));
            if (enclong < dictlong) {
                if (first == middle) {
                    break;
                }
                ...
                if (enclong <= dictlong || last == middle) {
                    break;
                }
                first = middle + 1;
        ...
        if (dictloc != 0 || flagunknown) {
            ...
            this.zm.memory_image[parseentry + 3] = (byte)(wordloc - textloc + 2);
        }
    ...
    void tokenise(final int textloc, final int textlength, final int parseloc) {
        this.tokenise(textloc, textlength, parseloc, true);
    }
    
    void tokenise(final int textloc, final int textlength, final int parseloc, final boolean parseunknown) {
