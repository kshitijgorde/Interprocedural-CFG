...
public class SkinnyMethodAdapter implements MethodVisitor, Opcodes
{
    ...
    public SkinnyMethodAdapter(final ClassVisitor cv, final int flags, final String name, final String signature, final String something, final String[] exceptions) {
    ...
    public void aloadMany(final int... args) {
    ...
    public void end() {
        if (SkinnyMethodAdapter.DEBUG) {
            ...
            ((TraceMethodVisitor)this.getMethodVisitor()).print(pw);
            pw.flush();
        }
    ...
    static {
        DEBUG = SafePropertyAccessor.getBoolean("jruby.compile.dump");
    }
}
