    ...
    protected int[] generateWorkingKey(final boolean b, final byte[] array) {
        ...
        for (int j = 0; j < 16; ++j) {
            ...
            for (byte b3 = 0; b3 < 28; ++b3) {
                final byte b4 = (byte)(b3 + DESEngine.totrot[j]);
                if (b4 < 28) {
                    array4[b3] = array3[b4];
                }
                else {
                    array4[b3] = array3[b4 - 28];
                }
            }
            for (byte b5 = 28; b5 < 56; ++b5) {
                final byte b6 = (byte)(b5 + DESEngine.totrot[j]);
                if (b6 < 56) {
                    array4[b5] = array3[b6];
                }
                else {
                    array4[b5] = array3[b6 - 28];
                }
            }
            for (int k = 0; k < 24; ++k) {
                if (array4[DESEngine.pc2[k]]) {
                    ...
                    final int n3 = n;
                    array5[n3] |= DESEngine.bigbyte[k];
                }
                if (array4[DESEngine.pc2[k + 24]]) {
                    final int[] array6 = array2;
                    final int n4 = n2;
                    array6[n4] |= DESEngine.bigbyte[k];
                }
            }
        }
        for (int l = 0; l != 32; l += 2) {
            final int n5 = array2[l];
            final int n6 = array2[l + 1];
            array2[l] = ((n5 & 0xFC0000) << 6 | (n5 & 0xFC0) << 10 | (n6 & 0xFC0000) >>> 10 | (n6 & 0xFC0) >>> 6);
            array2[l + 1] = ((n5 & 0x3F000) << 12 | (n5 & 0x3F) << 16 | (n6 & 0x3F000) >>> 4 | (n6 & 0x3F));
        }
        return array2;
    }
