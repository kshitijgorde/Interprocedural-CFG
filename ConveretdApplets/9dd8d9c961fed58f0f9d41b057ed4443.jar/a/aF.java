...
public abstract class aF extends Panel
{
    public static int q;
    public Image q;
    private r[] q;
    ...
    public final synchronized void q(final r r) {
        try {
            ...
            this.q(this.q[(this.t - 1 + this.y) % this.q.length] = r, this.q.size().width - 12 - aF.q, this.getFontMetrics(aU.w.w()));
            ...
            if ((n2 = this.r - this.w - height - 1) < r.q && (graphics = this.q.getGraphics()) != null) {
                this.q(r, this.r - r.q - this.w + 1, graphics);
            }
            if (n2 > 0) {
                if (this.q.r() == 4) {
                    this.q(-n2);
    ...
    public abstract void q(final r p0, final int p1, final FontMetrics p2);
    
    protected abstract void q(final r p0, final int p1, final Graphics p2);
    ...
    public final r q(final Event event) {
        synchronized (this) {
            if (event.x >= aF.q) {
                int n = event.y + this.w;
                final int n2 = event.x - aF.q;
                for (int i = 0; i < this.t; ++i) {
                    final r q = this.q(i);
                    if (n < q.q) {
                        q.r = -1;
                        ...
                        final int n4;
                        final int n3 = (n4 = n - (3 + fontMetrics.getAscent())) / (fontMetrics = this.getFontMetrics(aU.w.w())).getHeight();
                        if (n4 > 0 && n3 < q.w) {
                            int n5;
    ...
    public final synchronized void q() {
        ...
        final int n = this.q.size().width - 6 - aF.q;
        final FontMetrics fontMetrics = this.getFontMetrics(aU.w.w());
        for (int i = 0; i < this.t; ++i) {
            final r q = this.q(i);
    ...
    public aF(final br br, final boolean w) {
        this.q = 0L;
        this.q = null;
        this.w = 0;
        this.e = 0;
        ...
        (this.q = new bI(1)).setLineIncrement(10);
        layout.setConstraints(this.q, gridBagConstraints);
        this.add(this.q);
    }
}
