...
public final class NodeOptInfo
{
    ...
    public void concatLeftNode(final NodeOptInfo other, final Encoding enc) {
        ...
        if (other.exb.length > 0) {
            if (exbReach) {
                this.exb.concat(other.exb, enc);
                other.exb.clear();
            }
            else if (exmReach) {
                this.exm.concat(other.exb, enc);
                other.exb.clear();
            }
        }
        this.exm.select(other.exb, enc);
        this.exm.select(other.exm, enc);
        if (this.expr.length > 0) {
            if (other.length.max > 0) {
                ...
                if (this.expr.mmd.max == 0) {
                    this.exb.select(this.expr, enc);
                }
                else {
                    this.exm.select(this.expr, enc);
                }
            }
        }
    ...
    public void altMerge(final NodeOptInfo other, final OptEnvironment env) {
        ...
        this.map.altMerge(other.map, env.enc);
