...
public class WorldClock extends Applet implements ItemListener
{
    ...
    public void init() {
        this.backimage = this.getImage(this.getCodeBase(), "inter.jpg");
        ...
        this.list.setBounds(this.size().width - 155, 10, 145, 100);
        ...
        this.dclock.setDigitalTwelveHourMode(false);
        this.dclock.setDigitalNumeralFont(new Font("TimesRoman", 1, 14));
        this.add(this.dclock);
        this.clip = this.getAudioClip(this.getCodeBase(), "1.au");
    }
    
    public void paint(final Graphics graphics) {
        graphics.setColor(this.backcolor);
        graphics.drawImage(this.backimage, 0, 0, this.size().width, this.size().height, this);
        graphics.fill3DRect(7, 5, this.size().width - 15, this.size().height - 11, true);
    }
    ...
    public void itemStateChanged(final ItemEvent itemEvent) {
        ...
        if (this.list.getSelectedItem() == "Baghdad") {
            this.tz = this.data.Baghdad;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Colombo") {
            this.tz = this.data.Colombo;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Dakar") {
            this.tz = this.data.Dakar;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Kabul") {
            this.tz = this.data.Kabul;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Katmandu") {
            this.tz = this.data.Katmandu;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Kuala Lumpur") {
            this.tz = this.data.Kuala_Lumpur;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Riyadh") {
            this.tz = this.data.Riyadh;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Vientiane") {
            this.tz = this.data.Vientiane;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Ankara") {
            this.tz = this.data.Ankara;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Doha") {
            this.tz = this.data.Doha;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Leipzig") {
            this.tz = this.data.Leipzig;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Nicosia") {
            this.tz = this.data.Nicosia;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Beijing(Peking)") {
            this.tz = this.data.Beijing;
            this.clock.setTimeZone(this.tz);
        }
        if (this.list.getSelectedItem() == "Hanoi") {
            this.tz = this.data.Hanoi;
            this.clock.setTimeZone(this.tz);
        }
        this.clip.play();
    ...
    public boolean mouseEnter(final Event event, final int n, final int n2) {
        this.showStatus("Unregistered version... ***World Clock applet*** ...www.zmei-soft.com");
        return true;
    }
