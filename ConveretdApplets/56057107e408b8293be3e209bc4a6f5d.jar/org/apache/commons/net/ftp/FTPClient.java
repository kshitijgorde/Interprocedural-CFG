...
public class FTPClient extends FTP implements Configurable
{
    ...
    private void __parsePassiveModeReply(String reply) throws MalformedServerReplyException {
        ...
        final StringBuffer host = new StringBuffer(24);
        int lastIndex = 0;
        ...
        lastIndex = index + 1;
        index = reply.indexOf(44, lastIndex);
        final String octet1 = reply.substring(lastIndex, index);
        final String octet2 = reply.substring(index + 1);
        try {
    ...
    protected Socket _openDataConnection_(final int command, final String arg) throws IOException {
        ...
        if (this.__dataConnectionMode == 0) {
            final ServerSocket server = this._socketFactory_.createServerSocket(0, 1, this.getLocalAddress());
            ...
            this.__parsePassiveModeReply(this._replyLines.elementAt(0));
    ...
    public String printWorkingDirectory() throws IOException {
        ...
        return this.__parsePathname(this._replyLines.elementAt(0));
    }
    ...
    public String getSystemName() throws IOException {
        if (this.__systemName == null && FTPReply.isPositiveCompletion(this.syst())) {
            this.__systemName = this._replyLines.elementAt(0).substring(4);
        }
    ...
    public FTPFile[] listFiles(final String parserKey, final String pathname) throws IOException {
        final FTPListParseEngine engine = this.initiateListParsing(parserKey, pathname);
    ...
    private FTPListParseEngine initiateListParsing(final FTPFileEntryParser parser, final String pathname) throws IOException {
        ...
        final Socket socket;
        if ((socket = this._openDataConnection_(26, pathname)) == null) {
            return engine;
        }
    ...
    public FTPFile[] listFiles(final FTPFileListParser parser, final String pathname) throws IOException {
        final Socket socket;
        if ((socket = this._openDataConnection_(26, pathname)) == null) {
            return new FTPFile[0];
        }
        final FTPFile[] results = parser.parseFileList(socket.getInputStream(), this.getControlEncoding());
        socket.close();
        this.completePendingCommand();
        return results;
    }
    
    public FTPFile[] listFiles(final FTPFileListParser parser) throws IOException {
        return this.listFiles(parser, null);
    }
    
    public FTPFileList createFileList(final FTPFileEntryParser parser) throws IOException {
        return this.createFileList(null, parser);
    }
    
    public FTPFileList createFileList(final String pathname, final FTPFileEntryParser parser) throws IOException {
        final Socket socket;
        if ((socket = this._openDataConnection_(26, pathname)) == null) {
            return null;
        }
        final FTPFileList list = FTPFileList.create(socket.getInputStream(), parser);
        socket.close();
        this.completePendingCommand();
        return list;
    }
