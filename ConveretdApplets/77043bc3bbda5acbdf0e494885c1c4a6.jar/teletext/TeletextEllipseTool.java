...
public class TeletextEllipseTool extends TeletextTool implements TeletextToolInterface
{
    Color background;
    Color foreground;
    boolean dragActive;
    ...
    public void implementMouseReleased(final MouseEvent mouseEvent) {
        if (this.dragActive) {
            ...
            this.dragActive = false;
            this.getPageCanvas().getPage().drawEllipseInRectangle(this.x1, this.y1, this.x2, this.y2, true);
            this.renderCanvas();
            this.repaintCanvas();
        }
    }
    ...
    public void paint(final Graphics graphics) {
        if (this.dragActive) {
            ...
            for (int i = -1; i < 2; ++i) {
                for (int j = -1; j < 2; ++j) {
                    graphics.drawOval(graphPixelCenter[0] + i, graphPixelCenter[1] + j, graphPixelCenter2[0] - graphPixelCenter[0] + i, graphPixelCenter2[1] - graphPixelCenter[1] + j);
                }
            }
            graphics.setColor(this.foreground);
            graphics.drawOval(graphPixelCenter[0], graphPixelCenter[1], graphPixelCenter2[0] - graphPixelCenter[0], graphPixelCenter2[1] - graphPixelCenter[1]);
        }
    }
