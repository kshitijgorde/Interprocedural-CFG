...
public abstract class SshIO
{
    ...
    public SshIO() {
        ...
        this.identification_string_sent = "SSH-1.5-Java Ssh 1.1 (16/09/99) leo@mud.de, original by Cedric Gourio (javassh@france-mail.com)\r";
            ...
            switch (this.phase) {
                case 0: {
                    ...
                    if (b == 10) {
                        ...
                        this.position = 0;
                        final byte[] data = SshMisc.createString(this.identification_string_sent);
                        final byte packet_type = 17;
                        final SshPacket fistLine = this.createPacket(packet_type, data);
                        return fistLine;
                    }
                    continue;
                }
    ...
    private byte[] handlePacket(final byte packetType, final byte[] packetData) throws IOException {
        ...
        switch (packetType) {
            ...
            case 2: {
                ...
                for (int j = 0; j < Md5_hostKey.length; ++j) {
                    String hex = "";
                    final int[] v = { (Md5_hostKey[j] & 0xF0) >> 4, Md5_hostKey[j] & 0xF };
                    for (int k = 0; k < 1; ++k) {
                        switch (v[k]) {
                            case 10: {
                                hex += "a";
    ...
    private byte[] Send_SSH_CMSG_REQUEST_PTY() throws IOException {
        ...
        for (int i = 0; i < 4; ++i) {
            data[offset++] = XPixels[i];
        }
        for (int i = 0; i < 4; ++i) {
            data[offset++] = YPixels[i];
        }
