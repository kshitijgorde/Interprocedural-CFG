...
public class JMSSink implements MessageListener
{
    ...
    public JMSSink(final String s, final String s2, final String s3, final String s4) {
        try {
            ...
            topicConnection.createTopicSession(false, 1).createSubscriber((Topic)initialContext.lookup(s2)).setMessageListener(this);
        }
        catch (Exception t) {
    ...
    public void onMessage(final Message message) {
        try {
            ...
            JMSSink.logger.error("Exception thrown while processing incoming message.", t);
        }
    }
