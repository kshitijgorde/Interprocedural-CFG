...
public class NuSiteProxyViewer extends NuApplet implements SessionListener
{
        ...
        try {
            synchronized (this.session) {
                ...
                if (this.session.idStr != null) {
                    ...
                    this.report("Unable to connect to camera. " + this.session.cameraState() + " " + this.session.stateFailureMessage() + " reply: " + r.inspect(";"));
                    if (!this.hasError()) {
    ...
    protected void switchDocument(final String doc) throws Exception {
        if (this.hasError()) {
    ...
    protected void snapPicture(final boolean invert) throws Exception {
        ...
        Debug.println("Calling Snap: " + url.toString());
    ...
    protected boolean handleMessage(final int id, final Object arg) {
        ...
        switch (id) {
            ...
            case 1015: {
                if (this.videoCanvas == null || super.quit) {
                    break;
                }
                boolean recon = false;
                if (this.mjpeg != null) {
                ...
                this.stopStreaming();
                this.videoCanvas.setErrorStr((String)arg);
                break;
            }
            case 1016: {
    ...
    protected URLConnection doCommand(final String path) throws IOException {
        final URLConnection c = this.getConnection(path);
        return c;
    }
    ...
    public void paint(final Graphics g) {
        if (!super.vis) {
            super.vis = true;
            Debug.println("paint setting vis to true.");
            super.paint(g);
        }
        if (this.background != null) {
