...
public class Terminal extends Plugin implements FilterPlugin, VisualTransferPlugin, ClipboardOwner, Runnable
{
    ...
    private void configure(final PluginConfig cfg) {
        ...
        if ((tmp = cfg.getProperty("Terminal", super.id, "resize")) != null) {
            if (tmp.equals("font")) {
                final vt320 terminal = this.terminal;
                final vt320 terminal2 = this.terminal;
                terminal.setResizeStrategy(2);
            }
            else if (tmp.equals("screen")) {
                final vt320 terminal3 = this.terminal;
                final vt320 terminal4 = this.terminal;
                terminal3.setResizeStrategy(1);
            }
            else {
                final vt320 terminal5 = this.terminal;
                final vt320 terminal6 = this.terminal;
                terminal5.setResizeStrategy(0);
            }
        }
