...
public class XConnection
{
    ...
    public void close(final SQLDocument sqldoc) throws SQLException {
        for (int size = this.m_OpenSQLDocuments.size(), x = 0; x < size; ++x) {
            final SQLDocument d = this.m_OpenSQLDocuments.elementAt(x);
            if (d == sqldoc) {
                d.close();
