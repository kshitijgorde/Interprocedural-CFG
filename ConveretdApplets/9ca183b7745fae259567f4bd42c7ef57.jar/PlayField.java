...
final class PlayField extends Canvas implements Runnable, KeyListener
{
    ...
    public void paint(final Graphics graphics) {
        ...
        for (int j = 0; j < 8; ++j) {
            if (this.data.playerFire[j].isAlive()) {
                this.bufferGraphics.drawImage(this.data.playerFire[j].getFace(), this.data.playerFire[j].getXco(), this.data.playerFire[j].getYco(), this);
            }
        }
        for (int k = 0; k < this.data.getEnemyFire(); ++k) {
            if (this.data.enemyFire[k].isAlive()) {
                this.bufferGraphics.drawImage(this.data.enemyFire[k].getFace(), this.data.enemyFire[k].getXco(), this.data.enemyFire[k].getYco(), this);
            }
        }
        if (this.data.isWaiting() || this.starting) {
