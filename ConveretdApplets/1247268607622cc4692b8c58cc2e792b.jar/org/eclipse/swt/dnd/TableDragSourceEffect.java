...
public class TableDragSourceEffect extends DragSourceEffect
{
    ...
    public TableDragSourceEffect(final Table table) {
        super(table);
    ...
    Image getDragSourceImage(final DragSourceEvent dragSourceEvent) {
        ...
        if (OS.SendMessage(table.handle, 4098, 1, 0) != 0) {
            ...
            if (!OS.IsWinCE && OS.WIN32_VERSION >= OS.VERSION(4, 10) && (table.getStyle() & 0x4000000) != 0x0) {
                OS.SetLayout(createCompatibleDC3, 9);
            }
            ...
            for (final TableItem tableItem : selection) {
                final Rectangle bounds = tableItem.getBounds(0);
                final int sendMessage = OS.SendMessage(table.handle, 4129, table.indexOf(tableItem), new POINT());
            ...
            return this.dragSourceImage = Image.win32_new(table.getDisplay(), 0, createCompatibleBitmap);
        }
        return null;
    }
}
