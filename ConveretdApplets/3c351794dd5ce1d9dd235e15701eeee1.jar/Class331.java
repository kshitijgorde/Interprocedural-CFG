// 
// Decompiled by Procyon v0.5.30
// 

final class Class331
{
    byte[] aByteArray2775;
    byte[] aByteArray2776;
    int anInt2777;
    int anInt2778;
    int anInt2779;
    boolean[] aBooleanArray2780;
    int[] anIntArray2781;
    boolean[] aBooleanArray2782;
    byte[] aByteArray2783;
    int[][] anIntArrayArray2784;
    int anInt2785;
    int anInt2786;
    byte aByte2787;
    int anInt2788;
    int anInt2789;
    int[][] anIntArrayArray2790;
    int[] anIntArray2791;
    static Class105 aClass105_2792;
    int anInt2793;
    int[] anIntArray2794;
    byte[] aByteArray2795;
    byte[] aByteArray2796;
    int anInt2797;
    int anInt2798;
    byte[] aByteArray2799;
    static int anInt2800;
    int anInt2801;
    int anInt2802;
    static Class98_Sub46_Sub8 aClass98_Sub46_Sub8_2803;
    byte[][] aByteArrayArray2804;
    int anInt2805;
    int[][] anIntArrayArray2806;
    int[] anIntArray2807;
    int anInt2808;
    int anInt2809;
    static int[] anIntArray2810;
    static Class117 aClass117_2811;
    
    static final Class110[] method3723(final int n) {
        try {
            if (n != 256) {
                return null;
            }
            return new Class110[] { Class358.aClass110_3030, Class314.aClass110_2688, ha.aClass110_941 };
        }
        catch (RuntimeException ex) {
            throw Class64_Sub27.method667(ex, "uf.A(" + n + ')');
        }
    }
    
    public static void method3724(final int n) {
        try {
            Class331.aClass117_2811 = null;
            Class331.aClass105_2792 = null;
            if (n < -63) {
                Class331.anIntArray2810 = null;
                Class331.aClass98_Sub46_Sub8_2803 = null;
            }
        }
        catch (RuntimeException ex) {
            throw Class64_Sub27.method667(ex, "uf.B(" + n + ')');
        }
    }
    
    Class331() {
        this.anIntArrayArray2784 = new int[6][258];
        this.anIntArray2781 = new int[16];
        this.aBooleanArray2780 = new boolean[16];
        this.aByteArray2795 = new byte[256];
        this.anIntArray2791 = new int[6];
        this.aByteArray2776 = new byte[18002];
        this.aByteArray2796 = new byte[4096];
        this.aByteArray2783 = new byte[18002];
        this.anIntArrayArray2790 = new int[6][258];
        this.aBooleanArray2782 = new boolean[256];
        this.anIntArray2794 = new int[256];
        this.anIntArray2807 = new int[257];
        this.anInt2808 = 0;
        this.anInt2801 = 0;
        this.anIntArrayArray2806 = new int[6][258];
        this.aByteArrayArray2804 = new byte[6][258];
    }
    
    static {
        Class331.aClass105_2792 = new Class105("", 17);
    }
}
