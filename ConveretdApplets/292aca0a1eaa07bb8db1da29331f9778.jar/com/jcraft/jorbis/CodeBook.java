...
class CodeBook
{
    ...
    int best(final float[] a, final int step) {
        final EncodeAuxNearestMatch nt = this.c.nearest_tree;
        final EncodeAuxThreshMatch tt = this.c.thresh_tree;
        int ptr = 0;
        if (tt != null) {
            int index = 0;
            for (int k = 0, o = step * (this.dim - 1); k < this.dim; ++k, o -= step) {
                int i;
                for (i = 0; i < tt.threshvals - 1 && a[o] >= tt.quantthresh[i]; ++i) {}
        ...
        if (nt != null) {
            do {
                float c = 0.0f;
                final int p = nt.p[ptr];
                final int q = nt.q[ptr];
                for (int j = 0, o2 = 0; j < this.dim; ++j, o2 += step) {
                    c += (float)((this.valuelist[p + j] - this.valuelist[q + j]) * (a[o2] - (this.valuelist[p + j] + this.valuelist[q + j]) * 0.5));
                }
                if (c > 0.0) {
                    ptr = -nt.ptr0[ptr];
                }
