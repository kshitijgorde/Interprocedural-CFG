...
class Aboutdialog extends Dialog
{
    ...
    public void paint(final Graphics graphics) {
        ...
        graphics.drawString("Sunview Applet -- " + this.theVersion, 20, 20);
        graphics.drawString("© 1998-2003 Juergen Giesen", 20, 35);
        graphics.drawString("For private not commercial use only.", 20, 50);
        graphics.drawString("E-Mail: jgiesen@t-online.de", 20, 70);
        graphics.drawString("Home Page: http://www.GeoAstro.de", 20, 85);
        for (int i = 0; i < 6; ++i) {
    ...
    public boolean action(final Event event, final Object o) {
        if (event.target instanceof Button) {
            if (event.target == this.moreButton) {
                ...
                this.str[1] = "Sunview Applet is shareware.";
                this.str[2] = "";
