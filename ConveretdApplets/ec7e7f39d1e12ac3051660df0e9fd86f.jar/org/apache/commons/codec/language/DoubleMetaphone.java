...
public class DoubleMetaphone implements StringEncoder
{
    ...
    public String doubleMetaphone(String value, final boolean alternate) {
        ...
        while (!result.isComplete() && index <= value.length() - 1) {
            switch (value.charAt(index)) {
                ...
                case 'U':
                case 'Y': {
                    index = this.handleAEIOUY(value, result, index);
                    continue;
                }
                case 'B': {
                    result.append('P');
    ...
    private int handleAEIOUY(final String value, final DoubleMetaphoneResult result, final int index) {
        if (index == 0) {
            result.append('A');
        }
        return index + 1;
    ...
    private boolean conditionL0(final String value, final int index) {
        return (index == value.length() - 3 && contains(value, index - 1, 4, "ILLO", "ILLA", "ALLE")) || ((contains(value, index - 1, 2, "AS", "OS") || contains(value, value.length() - 1, 1, "A", "O")) && contains(value, index - 1, 4, "ALLE"));
    }
