...
public class X509Name extends ASN1Encodable
{
    ...
    private DERObjectIdentifier decodeOID(final String s, final Hashtable hashtable) {
        if (Strings.toUpperCase(s).startsWith("OID.")) {
            return new DERObjectIdentifier(s.substring(4));
        }
        if (s.charAt(0) >= '0' && s.charAt(0) <= '9') {
            return new DERObjectIdentifier(s);
        }
        final DERObjectIdentifier derObjectIdentifier = hashtable.get(Strings.toLowerCase(s));
    ...
    public Vector getValues(final DERObjectIdentifier derObjectIdentifier) {
    ...
    private void appendValue(final StringBuffer sb, final Hashtable hashtable, final DERObjectIdentifier derObjectIdentifier, final String s) {
    ...
    public String toString(final boolean b, final Hashtable hashtable) {
        final StringBuffer sb = new StringBuffer();
        final Vector vector = new Vector<StringBuffer>();
        int n = 1;
        StringBuffer sb2 = null;
        for (int i = 0; i < this.ordering.size(); ++i) {
            if (this.added.elementAt(i)) {
                sb2.append('+');
                this.appendValue(sb2, hashtable, (DERObjectIdentifier)this.ordering.elementAt(i), (String)this.values.elementAt(i));
            }
            else {
