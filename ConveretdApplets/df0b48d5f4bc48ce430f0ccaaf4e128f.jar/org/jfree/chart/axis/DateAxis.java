...
public class DateAxis extends ValueAxis implements Cloneable, Serializable
{
    ...
    public void setTimeZone(final TimeZone zone) {
        if (!this.timeZone.equals(zone)) {
            this.timeZone = zone;
            this.setStandardTickUnits(createStandardDateTickUnits(zone));
        ...
        while (tickDate.before(upperDate)) {
            if (!this.isHiddenValue(tickDate.getTime())) {
                ...
                switch (unit.getUnit()) {
                    ...
                    case 1: {
                        tickDate = this.calculateDateForPosition(new Month(tickDate, this.timeZone), this.tickMarkPosition);
                        continue;
                    }
                    case 0: {
                        tickDate = this.calculateDateForPosition(new Year(tickDate, this.timeZone), this.tickMarkPosition);
                        continue;
                    }
                    default: {
                        continue;
    ...
    public boolean equals(final Object obj) {
        ...
        return ObjectUtilities.equal(this.tickUnit, that.tickUnit) && ObjectUtilities.equal(this.dateFormatOverride, that.dateFormatOverride) && ObjectUtilities.equal(this.tickMarkPosition, that.tickMarkPosition) && ObjectUtilities.equal(this.timeline, that.timeline) && super.equals(obj);
    }
