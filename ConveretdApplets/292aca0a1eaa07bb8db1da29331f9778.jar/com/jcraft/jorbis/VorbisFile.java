...
public class VorbisFile
{
    ...
    void prefetch_all_headers(final Info first_i, final Comment first_c, final int dataoffset) {
        final Page og = new Page();
    ...
    int open(final InputStream is, final byte[] initial, final int ibytes) {
        return this.open_callbacks(is, initial, ibytes);
    }
    
    int open_callbacks(final InputStream is, final byte[] initial, final int ibytes) {
        this.datasource = is;
        this.oy.init();
        if (initial != null) {
    ...
    public float time_total(final int i) {
        ...
        return this.pcmlengths[i] / this.vi[i].rate;
    }
    ...
    public float time_tell() {
        ...
        return time_total + (this.pcm_offset - pcm_total) / this.vi[link].rate;
    }
    ...
    public static void main(final String[] arg) {
        try {
            final VorbisFile foo = new VorbisFile(arg[0]);
            final int links = foo.streams();
            System.out.println("links=" + links);
            final Comment[] comment = foo.getComment();
            final Info[] info = foo.getInfo();
    ...
    class SeekableInputStream extends InputStream
    {
        ...
        SeekableInputStream(final String file) throws FileNotFoundException {
            this.raf = null;
