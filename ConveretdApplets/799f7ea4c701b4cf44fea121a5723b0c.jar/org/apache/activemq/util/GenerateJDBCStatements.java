    ...
    public static void main(final String[] args) throws Exception {
        ...
        for (int j = 0; j < methods.length; ++j) {
            if (sPattern.matcher(methods[j].getName()).find() && setMethods.contains(methods[j].getName().replace("get", "set"))) {
                System.out.println("<property name=\"" + methods[j].getName().substring(3, 4).toLowerCase() + methods[j].getName().substring(4) + "\" value=\"" + returnStatement(methods[j].invoke(s, (Object[])null)) + "\" />");
            }
        }
        final Pattern sPattern2 = Pattern.compile("get.*Statment$");
        for (int k = 0; k < methods.length; ++k) {
            if (sPattern2.matcher(methods[k].getName()).find()) {
                System.out.println("<property name=\"" + methods[k].getName().substring(3, 4).toLowerCase() + methods[k].getName().substring(4) + "\" value=\"" + returnStatement(methods[k].invoke(s, (Object[])null)) + "\" />");
            }
        }
