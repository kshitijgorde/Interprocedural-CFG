...
public class addsymbolframe extends Frame implements FocusListener, ActionListener, MouseListener
{
    Dialog warning;
    Button fok;
    Button ok;
    ...
    public addsymbolframe(final String ssy, final useHandleInterface mainclass) {
        ...
        this.idx = new Checkbox[12];
        this.dowbox = new Checkbox();
        this.djtbox = new Checkbox();
        ...
        this.stocks = "";
        this.indices = "";
        this.addedSymbols = new Vector();
        this.oldIndices = new Hashtable();
        this.oldsymbol = ssy;
    ...
    public void setSymbols() {
        String indices = "";
        if (this.oldIndices != null && this.oldIndices.size() > 0) {
            this.oldIndices = new Hashtable();
        }
        if (this.idxSymbols.length > 0) {
            for (int i = 0; i < this.idxSymbols.length - 1; ++i) {
                if (this.idx[i].getState()) {
                    ...
                    this.oldIndices.put(this.idxSymbols[i], this.idxSymbols[i]);
                }
            }
            if (this.idx[this.idxSymbols.length - 1].getState()) {
                indices = indices + this.idxSymbols[this.idxSymbols.length - 1] + ",";
                this.oldIndices.put(this.idxSymbols[this.idxSymbols.length - 1], this.idxSymbols[this.idxSymbols.length - 1]);
            }
        }
        String stocks = "";
        ...
        this.mainclass.setSymbol(stocks, indices);
    ...
    public String splitTwoRowSymbols(final String symbols, final boolean stock) {
        final StringTokenizer tok = new StringTokenizer(symbols, ",");
    ...
    public void actionPerformed(final ActionEvent ev) {
        if (ev.getSource() == this.ok) {
            ...
            this.setSymbols();
            this.dispose();
            this.addedSymbols.removeAllElements();
        }
            ...
            if (this.addedSymbols != null && this.addedSymbols.size() > 0) {
                for (int j = 0; j < this.addedSymbols.size(); ++j) {
                    final String nSym = this.addedSymbols.get(j);
                    this.list.remove(nSym);
                }
                this.addedSymbols.removeAllElements();
            }
            for (int k = 0; k < this.idxSymbols.length; ++k) {
                this.idx[k].setState(false);
                if (this.oldIndices.containsKey(this.idxSymbols[k])) {
                    this.idx[k].setState(true);
                }
            }
            this.hide();
        }
    ...
    public void mouseClicked(final MouseEvent me) {
        this.hide();
        this.mainclass.showBrowser("http://www.quotemedia.com", "_self");
    }
