...
public abstract class ValueAxis extends Axis implements Cloneable, PublicCloneable, Serializable
{
    ...
    protected float[] calculateAnchorPoint(final ValueTick tick, final double cursor, final Rectangle2D dataArea, final RectangleEdge edge) {
        ...
        if (edge == RectangleEdge.TOP) {
            ...
            result[1] = (float)(cursor - this.getTickLabelInsets().bottom - 2.0);
        }
        else if (edge == RectangleEdge.BOTTOM) {
            result[0] = (float)this.translateValueToJava2D(tick.getValue(), dataArea, edge);
            result[1] = (float)(cursor + this.getTickLabelInsets().top + 2.0);
        }
        else if (edge == RectangleEdge.LEFT) {
            result[0] = (float)(cursor - this.getTickLabelInsets().left - 2.0);
            result[1] = (float)this.translateValueToJava2D(tick.getValue(), dataArea, edge);
        }
        else if (edge == RectangleEdge.RIGHT) {
            result[0] = (float)(cursor + this.getTickLabelInsets().right + 2.0);
            result[1] = (float)this.translateValueToJava2D(tick.getValue(), dataArea, edge);
        }
        return result;
    }
    ...
    protected AxisState drawTickMarksAndLabels(final Graphics2D g2, final double cursor, final Rectangle2D plotArea, final Rectangle2D dataArea, final RectangleEdge edge) {
        ...
        for (final ValueTick tick : ticks) {
            if (this.isTickLabelsVisible()) {
                ...
                RefineryUtilities.drawRotatedString(tick.getText(), g2, anchorPoint[0], anchorPoint[1], tick.getTextAnchor(), tick.getRotationAnchor(), tick.getAngle());
            }
            if (this.isTickMarksVisible()) {
                final float xx = (float)this.translateValueToJava2D(tick.getValue(), dataArea, edge);
                Line2D mark = null;
    ...
    public AxisSpace reserveSpace(final Graphics2D g2, final Plot plot, final Rectangle2D plotArea, final RectangleEdge edge, AxisSpace space) {
        ...
        if (this.isTickLabelsVisible()) {
            g2.setFont(this.getTickLabelFont());
            final List ticks = this.refreshTicks(g2, new AxisState(), plotArea, plotArea, edge);
    ...
    public boolean equals(final Object obj) {
        ...
        final ValueAxis axis = (ValueAxis)obj;
        if (super.equals(obj)) {
            final boolean b1 = this.positiveArrowVisible == axis.positiveArrowVisible;
            final boolean b2 = this.negativeArrowVisible == axis.negativeArrowVisible;
            final boolean b3 = this.inverted == axis.inverted;
            final boolean b4 = ObjectUtils.equal(this.range, axis.range);
            final boolean b5 = this.autoRange == axis.autoRange;
            final boolean b6 = this.autoRangeMinimumSize == axis.autoRangeMinimumSize;
            final boolean b7 = Math.abs(this.upperMargin - axis.upperMargin) < 1.0E-6;
            final boolean b8 = Math.abs(this.lowerMargin - axis.lowerMargin) < 1.0E-6;
            final boolean b9 = Math.abs(this.fixedAutoRange - axis.fixedAutoRange) < 1.0E-6;
            final boolean b10 = this.autoTickUnitSelection == axis.autoTickUnitSelection;
            final boolean b11 = ObjectUtils.equal(this.standardTickUnits, axis.standardTickUnits);
            final boolean b12 = this.verticalTickLabels == axis.verticalTickLabels;
    ...
    public void setMinimumAxisValue(final double min) {
    ...
    public void setMaximumAxisValue(final double max) {
    ...
    public double translateJava2DtoValue(final float java2DValue, final Rectangle2D dataArea, final RectangleEdge edge) {
        return this.translateJava2DToValue(java2DValue, dataArea, edge);
    }
