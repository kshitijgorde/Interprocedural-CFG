...
public class JValueLabel
{
    ...
    public static NFLabel loadValueLabel(final JGraph graph, final String s, final int n, final Vector vector, NFLabel loadParams, final int n2) throws NFParamException {
        if (graph.param.changed(s + "ValueLabelStyle")) {
            final Vector vector2 = (Vector)graph.param.get(s + "ValueLabelStyle");
        ...
        if (graph.param.changed(s + "ValueLabel")) {
            loadParams = NFLabel.loadParams(graph.param, graph.param.get(s + "ValueLabel"));
        }
        if (loadParams != null && graph.param.changed(s + "ValueLabelBox")) {
            loadParams.setRegion(NFRegion.loadParams(graph.param, graph.param.get(s + "ValueLabelBox")));
        }
        return loadParams;
    }
