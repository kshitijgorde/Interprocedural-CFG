...
public class VorbisFile
{
    ...
    public VorbisFile(final String s) throws JOrbisException {
        ...
        try {
            seekableInputStream = new SeekableInputStream(s);
        }
        catch (Exception ex) {
    ...
    private int get_prev_page(final Page page) {
    ...
    void prefetch_all_headers(final Info info, final Comment comment, final int n) {
        final Page page = new Page();
        ...
        if (inputStream instanceof SeekableInputStream) {
            ...
            try {
                if (n2 == 0) {
                    ...
                    System.out.println("seek: " + n2 + " is not supported");
                }
            }
            catch (Exception ex) {}
            return 0;
    ...
    int open(final InputStream inputStream, final byte[] array, final int n) {
        return this.open_callbacks(inputStream, array, n);
    }
    
    int open_callbacks(final InputStream datasource, final byte[] array, final int n) {
    ...
    public float time_total(final int n) {
        ...
        return this.pcmlengths[n] / this.vi[n].rate;
    }
    ...
    public float time_tell() {
        ...
        return time_total + (this.pcm_offset - pcm_total) / this.vi[i].rate;
    }
    ...
    public static void main(final String[] array) {
        try {
            final VorbisFile vorbisFile = new VorbisFile(array[0]);
            final int streams = vorbisFile.streams();
            System.out.println("links=" + streams);
            final Comment[] comment = vorbisFile.getComment();
            final Info[] info = vorbisFile.getInfo();
    ...
    class SeekableInputStream extends InputStream
    {
        ...
        SeekableInputStream(final String s) throws FileNotFoundException, IOException {
            this.raf = null;
