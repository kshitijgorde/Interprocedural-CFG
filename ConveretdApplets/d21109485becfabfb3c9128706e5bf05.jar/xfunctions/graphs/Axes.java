...
public class Axes extends Drawable
{
    ...
    private int xLabel_x;
    private int xLabel_y;
    private int yLabel_x;
    private int yLabel_y;
    private Font font;
    private int ascent;
    private int descent;
    ...
    public void setXAxisPosition(final int xAxisPosition) {
        if (xAxisPosition == 0 || xAxisPosition == 1 || xAxisPosition == 4 || xAxisPosition == 5) {
    ...
    public void setYAxisPosition(final int yAxisPosition) {
        if (yAxisPosition == 2 || yAxisPosition == 3 || yAxisPosition == 4 || yAxisPosition == 5) {
    ...
    public Axes() {
        ...
        this.lightAxesColor = new Color(150, 150, 255);
    ...
    public void draw(final Graphics graphics, final CoordinateRect coordinateRect) {
        if (this.needsReset) {
            ...
            this.setup(graphics.getFontMetrics(this.font), coordinateRect.getXmin(), coordinateRect.getXmax(), coordinateRect.getYmin(), coordinateRect.getYmax(), coordinateRect.getLeft(), coordinateRect.getTop(), coordinateRect.getWidth(), coordinateRect.getHeight(), coordinateRect.getGap());
            this.needsReset = false;
        }
        graphics.setFont(this.font);
        this.doDraw(graphics, coordinateRect.getXmin(), coordinateRect.getXmax(), coordinateRect.getYmin(), coordinateRect.getYmax(), coordinateRect.getLeft(), coordinateRect.getTop(), coordinateRect.getWidth(), coordinateRect.getHeight(), coordinateRect.getGap());
    }
    ...
    void setup(final FontMetrics fontMetrics, final double n, final double n2, final double n3, final double n4, final int n5, final int n6, final int n7, final int n8, final int n9) {
        ...
        for (double n12 = 1.5; n12 < 4.0 && fontMetrics.stringWidth(Utilities.realToString(n11 + fudgeStart)) + this.digitWidth > n11 / (n2 - n) * (n7 - 2 * n9); n11 = this.fudge(n12 * (n2 - n) / n10), n12 += 0.5) {}
        ...
        while (n13 < 50 && n14 <= n2 && n5 + n9 + (n7 - 2 * n9) * (n14 - n) / (n2 - n) + fontMetrics.stringWidth(Utilities.realToString(n14)) / 2 <= n15) {
            array[n13] = n14;
            ++n13;
            n14 += n11;
        }
        ...
        while (n13 < 50 && n16 >= n && n5 + n9 + (n7 - 2 * n9) * (n16 - n) / (n2 - n) - fontMetrics.stringWidth(Utilities.realToString(n16)) / 2 >= n17) {
            array[n13] = n16;
            ++n13;
            n16 -= n11;
        }
        ...
        for (int i = 0; i < n13; ++i) {
            ...
            this.xTickLabels[i] = Utilities.realToString(array[i]);
        ...
        int n24 = 0;
        for (int j = 0; j < n19; ++j) {
            this.yTickLabels[j] = Utilities.realToString(array[j]);
    ...
    double fudgeStart(final double n, final double n2) {
