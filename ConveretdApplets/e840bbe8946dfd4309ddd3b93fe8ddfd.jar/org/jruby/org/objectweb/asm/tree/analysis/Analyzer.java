...
public class Analyzer implements Opcodes
{
    ...
    public Frame[] analyze(final String s, final MethodNode methodNode) throws AnalyzerException {
        ...
        for (int i = 0; i < methodNode.tryCatchBlocks.size(); ++i) {
            ...
            for (int index2 = this.insns.indexOf(tryCatchBlockNode.end), j = index; j < index2; ++j) {
                List<TryCatchBlockNode> list = (List<TryCatchBlockNode>)this.handlers[j];
                if (list == null) {
        ...
        while (this.top > 0) {
            ...
            final Frame frame3 = this.frames[n2];
            ...
            try {
                ...
                final List list3 = this.handlers[n2];
                if (list3 == null) {
                    continue;
                }
                for (int n6 = 0; n6 < list3.size(); ++n6) {
                    ...
                    if (this.newControlFlowExceptionEdge(n2, index9)) {
                        frame2.init(frame3);
                        frame2.clearStack();
    ...
    private void findSubroutine(int n, final Subroutine subroutine, final List list) throws AnalyzerException {
        while (n >= 0 && n < this.n) {
            ...
            if (value instanceof JumpInsnNode) {
                ...
                for (int i = tableSwitchInsnNode.labels.size() - 1; i >= 0; --i) {
                    this.findSubroutine(this.insns.indexOf((AbstractInsnNode)tableSwitchInsnNode.labels.get(i)), subroutine, list);
                }
            }
            else if (value instanceof LookupSwitchInsnNode) {
                final LookupSwitchInsnNode lookupSwitchInsnNode = (LookupSwitchInsnNode)value;
                ...
                for (int j = lookupSwitchInsnNode.labels.size() - 1; j >= 0; --j) {
                    this.findSubroutine(this.insns.indexOf((AbstractInsnNode)lookupSwitchInsnNode.labels.get(j)), subroutine, list);
                }
            }
            final List list2 = this.handlers[n];
            if (list2 != null) {
    ...
    private void merge(final int n, final Frame frame, final Frame frame2, final Subroutine subroutine, final boolean[] array) throws AnalyzerException {
        ...
        if (frame3 == null) {
            ...
            merge = frame3.merge(frame2, array);
        }
