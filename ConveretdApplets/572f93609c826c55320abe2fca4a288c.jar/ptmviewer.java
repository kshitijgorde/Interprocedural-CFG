...
public class ptmviewer extends Applet
{
    ...
    public void init() {
        ...
        final String parameter2 = this.getParameter("order");
        if (parameter2 != null) {
    ...
    void loadQTVR() {
        this.pv.percent[0] = 0;
        this.pv.repaint();
        final byte[] file_read = this.pv.file_read(this.filename, this.pv.percent);
        if (file_read == null) {
            this.pv.fatal = true;
            return;
        }
    ...
    void displayObjectQTVR(final byte[] array) {
        ...
        final Image[] get_jpeg_images = this.get_jpeg_images(array);
        if (get_jpeg_images == null || get_jpeg_images.length == 0) {
    ...
    void insert_cyltile(final int n, final int[] array, final int n2, final int[][] array2, final byte[] array3) {
        ...
        if (this.y_LU == null) {
            ...
            final double n4 = length2 / 6.283185307179586;
            final double n5 = n4 * 256.0;
            final double n6 = length / 2.0 + 0.5;
            final int n7 = n2 * 128;
            for (int i = 0; i < length; ++i) {
                final int n8 = (int)(n5 * Math.tan((i - n6) / n4)) + n7;
                this.w1[i] = (n8 & 0xFF);
        ...
        for (int j = 0; j < length; ++j) {
            for (int k = 0, n10 = 0; k < n3; ++k, n10 += n2) {
                ...
                int n15 = (n12 >> 16 & 0xFF) * this.w0[j] + (n13 >> 16 & 0xFF) * this.w1[j] >> 8;
                if (n15 > 255) {
                    n15 = 255;
                }
                if (n15 < 0) {
                ...
                int n16 = (n12 >> 8 & 0xFF) * this.w0[j] + (n13 >> 8 & 0xFF) * this.w1[j] >> 8;
                if (n16 > 255) {
                    n16 = 255;
                }
                if (n16 < 0) {
                ...
                int n17 = (n12 & 0xFF) * this.w0[j] + (n13 & 0xFF) * this.w1[j] >> 8;
                if (n17 > 255) {
                    n17 = 255;
                }
                if (n17 < 0) {
    ...
    void displayCubicQTVR(final byte[] array) {
        ...
        final Image[] get_jpeg_images = this.get_jpeg_images(array);
        if (get_jpeg_images == null || get_jpeg_images.length != 6) {
            System.out.println("Could not read QTVR-file.");
            this.pv.fatal = true;
            return;
        }
        final String[] array2 = { "{code=ptzoom.class}{fov=90.2}{pan=180}", "{code=ptzoom.class}{fov=90.2}{pan=-90}", "{code=ptzoom.class}{fov=90.2}{pan=0}", "{code=ptzoom.class}{fov=90.2}{pan=90}", "{code=ptzoom.class}{fov=90.2}{pan=180}{tilt=90}", "{code=ptzoom.class}{fov=90.2}{pan=180}{tilt=-90}" };
        ...
        for (int j = 0; j < 6; ++j) {
            this.pv.percent[0] = 80 + j * 4;
            this.pv.repaint();
            final ptzoom ptzoom = new ptzoom(this.pv, get_jpeg_images[j], get_tiff_hsimages[j], array2[j]);
            get_tiff_hsimages[j] = null;
            this.pv.app_properties.addElement(array2[j]);
            this.pv.applets.put(array2[j], ptzoom);
            ptzoom.init();
            ptzoom.start();
        }
    }
    ...
    Image[] get_jpeg_images(final byte[] array) {
        ...
        for (int l = 0; l < vector.size(); ++l) {
            ...
            final int n4 = vector2.elementAt(l) - intValue;
        ...
        for (int n6 = 0; n6 < vector3.size(); ++n6) {
            ...
            final int n7 = image.getWidth(null) * image.getHeight(null);
            if (n7 > n5) {
                n5 = n7;
            }
        }
    ...
    void ordertiles(final Vector vector, final String s) {
        ...
        for (int i = 0; i < vector2.size(); ++i) {
            final String arg = this.pv.getArg(i, s, ',');
            if (arg != null) {
    ...
    byte[][] get_tiff_hsimages(final byte[] array, final int n) {
        ...
        for (int j = 0; j < array.length; ++j) {
            final int readtiffHeader = this.readtiffHeader(array, j);
            if (readtiffHeader > 0) {
    ...
    final int getInt(final byte[] array) throws IndexOutOfBoundsException {
        final int n = array[this.tiff_fp++] & 0xFF;
        final int n2 = array[this.tiff_fp++] & 0xFF;
        final int n3 = array[this.tiff_fp++] & 0xFF;
        final int n4 = array[this.tiff_fp++] & 0xFF;
    ...
    int readtiffHeader(final byte[] array, final int n) {
        ...
        int int1;
        try {
            final int short1 = this.getShort(array);
            if (short1 == 18761) {
    ...
    boolean readtiffdirectory(final byte[] array, final int tiff_fp) {
        this.tiff_fp = tiff_fp;
        int short1;
        try {
        ...
        for (int i = 0; i < short1; ++i) {
            ...
            switch (short2) {
                case 256: {
                    this.tiff_width = value;
                    break;
                }
                case 257: {
                    this.tiff_height = value;
                    break;
                }
                case 278: {
                    this.tiff_rps = value;
                ...
                case 279: {
                    this.tiff_sbc = new int[int1];
                    if (int1 == 1) {
                        this.tiff_sbc[0] = value;
                        break;
                    }
                    final int tiff_fp2 = this.tiff_fp;
                    this.tiff_fp = this.tiff_start + value;
                    for (int j = 0; j < int1; ++j) {
                        try {
                            this.tiff_sbc[j] = this.getInt(array);
                        }
                        catch (IndexOutOfBoundsException ex3) {
                            return false;
                        }
                    }
                    this.tiff_fp = tiff_fp2;
                    break;
                }
                case 273: {
                    this.tiff_so = new int[int1];
                    if (int1 == 1) {
                        this.tiff_so[0] = value;
                        break;
                    }
                    final int tiff_fp3 = this.tiff_fp;
                    this.tiff_fp = this.tiff_start + value;
                    for (int k = 0; k < int1; ++k) {
                        try {
                            this.tiff_so[k] = this.getInt(array);
                        }
                        catch (IndexOutOfBoundsException ex4) {
                            return false;
                        }
                    }
                    this.tiff_fp = tiff_fp3;
                    break;
                }
                case 258: {
                    if (value != 8) {
    ...
    byte[] readtiffdata(final byte[] array) {
        final byte[] array2 = new byte[this.tiff_width * this.tiff_height];
        final int n = this.tiff_rps * this.tiff_width;
        for (int i = 0; i < this.tiff_so.length; ++i) {
            final int n2 = i * n;
            if (n2 >= array2.length) {
                return null;
            }
            ...
            final int n3 = this.tiff_start + this.tiff_so[i];
            if (n3 >= array.length) {
                return null;
            }
            final int n4 = n3 + this.tiff_sbc[i];
            if (n4 > array.length) {
                return null;
            }
    ...
    void set_qtvr_properties(final Properties properties) {
        ...
        final String property = properties.getProperty("width");
        if (property != null) {
            doubleValue = Double.valueOf(property);
        }
        final String property2 = properties.getProperty("height");
        if (property2 != null) {
            doubleValue2 = Double.valueOf(property2);
        }
        final String property3 = properties.getProperty("fov");
        if (property3 != null) {
        ...
        final String property4 = properties.getProperty("pan");
        if (property4 != null) {
            this.pv.yaw = 180.0 - Double.valueOf(property4);
        }
        final String property5 = properties.getProperty("tilt");
        if (property5 != null) {
            this.pv.pitch = Double.valueOf(property5);
        }
        for (int i = 0; i <= this.pv.hotspots.size(); ++i) {
            final String property6 = properties.getProperty("hotspot" + (i + 1));
            if (property6 != null && i >= this.pv.hotspots.size()) {
                final String property7 = properties.getProperty("target" + (i + 1));
                if (property7 != null) {
    ...
    boolean check_qtvr(final byte[] array) {
        final byte[] array2 = { -1, -40, -1, -32, 0, 16, 74, 70, 73, 70, 0 };
        if (array[4] != 109 || array[5] != 111 || array[6] != 111 || array[7] != 118) {
            return false;
        }
        for (int n = array.length - array2.length, i = 0; i < n; ++i) {
            if (array[i] == array2[0] && array[i + 1] == array2[1] && array[i + 2] == array2[2] && array[i + 3] == array2[3] && array[i + 4] == array2[4] && array[i + 5] == array2[5] && array[i + 6] == array2[6] && array[i + 7] == array2[7] && array[i + 8] == array2[8] && array[i + 9] == array2[9] && array[i + 10] == array2[10]) {
