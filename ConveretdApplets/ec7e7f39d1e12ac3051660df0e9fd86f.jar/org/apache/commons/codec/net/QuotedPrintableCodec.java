...
public class QuotedPrintableCodec implements BinaryEncoder, BinaryDecoder, StringEncoder, StringDecoder
{
    ...
    private static final void encodeQuotedPrintable(final int b, final ByteArrayOutputStream buffer) {
        buffer.write(QuotedPrintableCodec.ESCAPE_CHAR);
    ...
    public static final byte[] decodeQuotedPrintable(final byte[] bytes) throws DecoderException {
        ...
        for (int i = 0; i < bytes.length; ++i) {
            final int b = bytes[i];
            if (b == QuotedPrintableCodec.ESCAPE_CHAR) {
                try {
    ...
    static {
        ...
        QuotedPrintableCodec.ESCAPE_CHAR = 61;
        QuotedPrintableCodec.TAB = 9;
        QuotedPrintableCodec.SPACE = 32;
        for (int i = 33; i <= 60; ++i) {
        ...
        QuotedPrintableCodec.PRINTABLE_CHARS.set(QuotedPrintableCodec.TAB);
        QuotedPrintableCodec.PRINTABLE_CHARS.set(QuotedPrintableCodec.SPACE);
    }
}
