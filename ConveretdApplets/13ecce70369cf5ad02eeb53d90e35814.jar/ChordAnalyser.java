...
public class ChordAnalyser
{
    ...
    private final int FLAT_NOTE = 0;
    private final int SHARP_NOTE = 1;
    ...
    public int[] getMidiValues() {
        if (this.szResult == null) {
            return null;
        }
        int n = 0;
        int n2 = 0;
        final int n3 = 48;
        for (int i = 0; i < this.szResult.length(); ++i) {
            if (this.szResult.charAt(i) == ',') {
                ++n;
            }
        }
        final String[] array = new String[n];
        final int[] array2 = new int[n];
        final StringTokenizer stringTokenizer = new StringTokenizer(this.szResult);
        while (stringTokenizer.hasMoreTokens()) {
            final String nextToken = stringTokenizer.nextToken();
            array[n2++] = nextToken.substring(0, nextToken.length() - 1);
        }
        for (int j = 0; j < n; ++j) {
            for (int k = 0; k < 12; ++k) {
                if (array[j].equals(this.noteNamesSharp[k]) || array[j].equals(this.noteNamesFlat[k])) {
                    if (j == 0) {
                        array2[j] = n3 + k;
                    }
                    else if (n3 + k < array2[j - 1]) {
                        if (n3 + k + 12 > array2[j - 1]) {
                            array2[j] = n3 + k + 12;
                        }
                        else {
                            array2[j] = n3 + k + 24;
    ...
    public void findReverseChords(final String s) {
        ...
        int n2 = 0;
        boolean b = true;
        if (s != null && !s.equals("") && !s.equals("--------") && !s.equals("no matching note found, ")) {
            for (int i = 0; i < s.length(); ++i) {
                if (s.charAt(i) == ',') {
                    ++n;
                }
    ...
    private String parseFormulaMask(final int n, final String s, final String[] array, final int n2) {
        if (s != null) {
            int i = 0;
            while (i < 12) {
                if (s.equals(array[i])) {
                    ...
                    if (n2 == 1) {
                        return this.noteNamesSharp[n3];
                    }
                    return this.noteNamesFlat[n3];
                }
                else {
                    ++i;
                }
    ...
    private String getRealNote(final int n, final String s) {
        if (!this.parseFormulaMask(n, s, this.sharpNoteNames, 1).equals("ZERO")) {
            return this.parseFormulaMask(n, s, this.sharpNoteNames, 1);
        }
        if (!this.parseFormulaMask(n, s, this.flatNoteNames, 0).equals("ZERO")) {
            return this.parseFormulaMask(n, s, this.flatNoteNames, 0);
        }
        if (!this.parseFormulaMask(n, s, this.addNoteNames, 1).equals("ZERO")) {
            return this.parseFormulaMask(n, s, this.addNoteNames, 1);
        }
        if (!this.parseFormulaMask(n, s, this.addFlatNoteNames, 0).equals("ZERO")) {
            return this.parseFormulaMask(n, s, this.addFlatNoteNames, 0);
        }
    ...
    public String getNoteFromSharpArr(final int n) {
        if (n >= 0 && n <= 11) {
    ...
    public String getNoteFromFlatArr(final int n) {
        if (n >= 0 && n <= 11) {
            return this.noteNamesFlat[n];
        }
        return "";
    }
