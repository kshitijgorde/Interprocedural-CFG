...
public class TextLine
{
    ...
    public void parseText(final Graphics lg) {
        ...
        for (int j = 0; j < this.list.size(); ++j) {
            final TextState textState7 = this.list.elementAt(j);
    ...
    public void draw(final Graphics graphics, final int n, int n2, final int n3, final int n4) {
        int n5 = this.getWidth(graphics);
        n2 += (n4 + this.getHeight(graphics)) / 2;
        if (n5 < n3) {
            this.draw(graphics, n, n2);
        }
        else {
            final String string = ">" + this.text;
            while (n5 > n3 && this.text.length() > 2) {
                this.text = this.text.substring(0, this.text.length() - 1);
                this.parse = true;
                n5 = this.getWidth(graphics);
            }
            if (this.text.charAt(this.text.length() - 1) >= '0') {
                this.text += '.';
            }
            else {
                this.text = this.text.substring(0, this.text.length() - 1) + '.';
    ...
    public void draw(final Graphics graphics, final int n, final int n2) {
        ...
        for (int i = 0; i < this.list.size(); ++i) {
            final TextState textState = this.list.elementAt(i);
            if (textState.f != null) {
