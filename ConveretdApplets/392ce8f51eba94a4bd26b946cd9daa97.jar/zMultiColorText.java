...
public class zMultiColorText extends Applet implements Runnable
{
    Thread mythread;
    Image offI;
    Graphics offG;
    ...
    public void go() {
        while (true) {
            this.showStatus("Multi Color Text Applet by Plamen Gelev-Unregistered");
            this.i = 0;
            this.xpos = 20;
            try {
                Thread.sleep(100L);
                for (int j = 0; j < this.s.length(); ++j) {
                    this.r = (int)(Math.random() * 255.0);
                    this.g = (int)(Math.random() * 255.0);
                    this.b = (int)(Math.random() * 255.0);
                    ...
                    this.offG.drawString(this.s.substring(j, j + 1), 20 + this.i, this.size().height / 2 + 10);
