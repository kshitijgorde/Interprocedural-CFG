...
public class InstructionFinder
{
    ...
    public final Iterator search(final String pattern, final InstructionHandle from, final CodeConstraint constraint) {
        ...
        try {
            final RE regex = new RE(search);
            final ArrayList matches = new ArrayList();
            while (start < this.il_string.length() && regex.match(this.il_string, start)) {
                final int startExpr = regex.getParenStart(0);
                final int endExpr = regex.getParenEnd(0);
                final int lenExpr = regex.getParenLength(0);
