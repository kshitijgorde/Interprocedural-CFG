...
public class HttpStatus
{
    ...
    static {
        REASON_PHRASES = new String[][] { new String[0], new String[3], new String[8], new String[8], new String[25], new String[8] };
    ...
    public static String getStatusText(final int statusCode) {
        final int classIndex = statusCode / 100;
        final int codeIndex = statusCode - classIndex * 100;
        if (classIndex < 1 || classIndex > HttpStatus.REASON_PHRASES.length - 1 || codeIndex < 0 || codeIndex > HttpStatus.REASON_PHRASES[classIndex].length) {
            return null;
        }
        return HttpStatus.REASON_PHRASES[classIndex][codeIndex];
    }
    
    private static void addStatusCodeMap(final int statusCode, final String reasonPhrase) {
        final int classIndex = statusCode / 100;
