...
public class PeriodType implements Serializable
{
    ...
    private static final Map cTypes;
    ...
    public static synchronized PeriodType forFields(final DurationFieldType[] array) {
        ...
        final Map cTypes = PeriodType.cTypes;
        if (PeriodType.cTypes.isEmpty()) {
        ...
        final PeriodType periodType = new PeriodType(null, array, null);
        ...
        if (list.size() > 0) {
            cTypes.put(periodType, list);
            throw new IllegalArgumentException("PeriodType does not support fields: " + list);
        }
        final PeriodType periodType3 = cTypes.get(new PeriodType(null, periodType2.iTypes, null));
        if (periodType3 != null) {
            cTypes.put(periodType, periodType3);
            return periodType3;
        }
        cTypes.put(periodType, periodType2);
        return periodType2;
    }
    ...
    private PeriodType withFieldRemoved(final int n, final String s) {
        final int n2 = this.iIndices[n];
        if (n2 == -1) {
            return this;
        }
        ...
        return new PeriodType(this.getName() + s, array, array2);
    }
