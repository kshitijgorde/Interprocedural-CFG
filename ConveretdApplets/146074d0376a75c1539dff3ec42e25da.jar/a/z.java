...
final class z extends D implements dT
{
    private boolean q;
    private Event q;
    private String q;
    ...
    public final boolean handleEvent(final Event q) {
        ...
        final int q3 = this.q.q();
        final int w2;
        final y y = ((w2 = this.q.w(q.x)) >= 0) ? this.q.q(w2) : null;
        A arg;
        if (q2 >= 0 && q2 < q3) {
            arg = a.w.q(this.q).elementAt(q2);
        }
        else {
            arg = null;
        }
        switch (q.id) {
            ...
            case 501: {
                if (q.metaDown()) {
                    ...
                    if (a.w.q(this.w) && q2 < a.w.w(this.w).size() && arg != null && arg.q instanceof eo) {
                        if (a.w.q(this.w).q((Object)arg.q)) {
                            a.w.q(this.w).w(arg.q);
                        }
                        else {
                            a.w.q(this.w).q((eo)arg.q);
                        }
                    }
                    this.repaint();
                    if (q.clickCount < 2) {
