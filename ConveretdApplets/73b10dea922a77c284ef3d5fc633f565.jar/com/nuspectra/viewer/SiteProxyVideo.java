...
public class SiteProxyVideo extends MJPEGStream
{
    ...
    public SiteProxyVideo(final NuApplet inApplet, final VideoCanvas vc, final ControlSession cs) throws Exception {
        super(inApplet, vc);
    ...
    void slowStream() throws Exception {
        while (!super.quit) {
            ...
            if (len > 0) {
                ...
                String n = new String(this.header);
                if (n.indexOf("SP_ERROR=") == 0) {
                    while (s.available() > 0) {
                        final int ch = s.read();
                        if (ch == 0 || ch == 10) {
                            break;
