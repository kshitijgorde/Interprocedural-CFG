...
public class NFGraph extends Panel implements Observer, NFDwellObserver
{
    ...
    public boolean dwellDisplay(final boolean b, final NFActiveLabel nfActiveLabel) {
        if (b) {
            NFDebug.print(16L, "NFGraph: DwellOn  <" + nfActiveLabel.label + "> param=" + nfActiveLabel.selectedItemParam + " index=" + nfActiveLabel.selectedItemIndex);
        }
        else {
            NFDebug.print(16L, "NFGraph: DwellOff  <" + nfActiveLabel.label + "> param=" + nfActiveLabel.selectedItemParam + " index=" + nfActiveLabel.selectedItemIndex);
            this.display();
        }
        return false;
    }
    ...
    public boolean dwellPress(final Event event, final int n, final int n2, final NFActiveLabel nfActiveLabel) {
        if (nfActiveLabel.url != null && nfActiveLabel.target != null && nfActiveLabel.target.equals("LOADPARAMS")) {
            NFDebug.print(16L, "NFGraph: Dwell LoadParams from <" + nfActiveLabel.url + ">");
            try {
        ...
        NFDebug.print(16L, "NFGraph: DwellPress at " + n + "," + n2);
        return false;
    }
    ...
    protected void addBannerButton() {
        ...
        (this.bannerButton = new Panel()).setLayout(new FlowLayout(1));
    ...
    static {
        NFGraph.loadLogicalFont = false;
        if (NFUtil.getJDKVersion() >= 1.2 && !NFGraph.loadLogicalFont) {
            NFOffscreenImage.getAllFonts();
            NFGraph.loadLogicalFont = true;
        }
        NFGlobal.getVendor();
    }
}
