...
public abstract class NFDataChart extends NFGraph implements NFAxisMap
{
    ...
    public NFDataChart(final Applet applet, final int n, final int n2, final int n3, final int n4) {
        ...
        this.b = null;
        this.axisThickness = NFDataChart.AXIS_THICKNESS_DEFAULT;
        this.c = 0;
        this.d = 0;
        this.e = 0;
    ...
    protected void drawLineValueLabel(final Graphics graphics, final NFDataSeries nfDataSeries, final int n, final int n2, final String s) {
        NFValueLabel.drawPointValueLabel((this.lineValueLabel == null) ? super.dwell.getLabel() : this.lineValueLabel, graphics, nfDataSeries, n, n2, s);
    }
    
    protected void defineLineValueLabel(final NFParam nfParam) {
        NFValueLabel.definePointValueLabel(nfParam, "Line", 0);
    }
    ...
    protected void loadLineValueLabel(final NFParam nfParam, final int n) throws NFParamException {
        this.lineValueLabel = NFValueLabel.loadValueLabel(this, "Line", n, this.dataSeries, this.lineValueLabel, 0);
    }
    ...
    protected synchronized void loadParams() throws Exception {
        ...
        if (super.param.changed("AxisThickness")) {
            super.graphChanged = true;
            super.layoutChanged = true;
            this.axisThickness = NFUtil.getNumber(super.param.get("AxisThickness"), NFDataChart.AXIS_THICKNESS_DEFAULT);
        }
    ...
    protected void loadLineSymbol(final NFParam nfParam) throws Exception {
        this.loadDataSeriesSymbol(nfParam, "LineSymbol", 2, NFDataChart.defaultLineSymbol);
    }
    ...
    protected void loadLineStyle(final NFParam nfParam) throws Exception {
        ...
        for (int i = 0; i < vector.size(); ++i) {
            ...
            dataSeries.line = this.loadLine(vector2, dataSeries.line, NFDataChart.defaultLineStyle);
    ...
    protected void handleLayoutChange() {
        ...
        for (int i = 0; i < NFDataChart.axisParameters.length; ++i) {
            this.setChangedIfLoaded(NFDataChart.axisParameters[i]);
        }
    }
    ...
    protected void cleanLayoutParameters() {
        for (int i = 0; i < NFDataChart.axisParameters.length; ++i) {
            if (this.beenLoaded(NFDataChart.axisParameters[i])) {
                this.setLoaded(NFDataChart.axisParameters[i], false);
            }
        }
    }
    
    static {
        NFDataChart.AXIS_THICKNESS_DEFAULT = 0;
        ...
        NFDataChart.defaultLineSymbol = null;
        NFDataChart.defaultLineStyle = null;
        NFDataChart.defaultLineSymbol = new NFGraphSymbol();
        NFDataChart.defaultLineSymbol.type = 0;
        NFDataChart.defaultLineSymbol.size = 1;
        NFDataChart.defaultLineSymbol.style = 1;
        (NFDataChart.defaultLineStyle = new NFLine(null)).setStyle(0);
        NFDataChart.defaultLineStyle.setThickness(1);
        NFDataChart.defaultLineStyle.setColor(Color.black);
    }
}
