    ...
    protected void runTask(final List tokens) throws Exception {
        try {
            ...
            for (final String destName : tokens) {
                ...
                final List addMsgs = AmqMessagesUtil.getMessages(this.getBrokerUrl(), dest, this.queryAddObjects);
                if (this.querySubObjects.size() > 0) {
                    final List subMsgs = AmqMessagesUtil.getMessages(this.getBrokerUrl(), dest, this.querySubObjects);
