...
{
    static final long serialVersionUID = -3119457291398933381L;
    ...
    protected void doit(final HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (action.trim().equals("Clear Alert")) {
            try {
                ManageThresholdMonitorServlet.log.debug("Clearing Alert for monitor: " + monitorObjectName.toString());
        ...
        if (action.trim().equals("Remove Monitor")) {
            try {
                ManageThresholdMonitorServlet.log.debug("removing monitor: " + monitorObjectName.toString());
        ...
        ManageThresholdMonitorServlet.log.debug(threshold);
        final boolean enabled = req.getParameter("enabled") != null;
        ManageThresholdMonitorServlet.log.debug("Enabled: " + enabled);
        final boolean persisted = req.getParameter("persisted") != null;
        ManageThresholdMonitorServlet.log.debug("Persisted: " + persisted);
        final String period = req.getParameter("period").trim();
        ManageThresholdMonitorServlet.log.debug(period);
        long timePeriod = 0L;
        try {
        ...
        ManageThresholdMonitorServlet.log.debug(compare);
    ...
    static {
        log = Logger.getLogger(ManageThresholdMonitorServlet.class);
    }
}
