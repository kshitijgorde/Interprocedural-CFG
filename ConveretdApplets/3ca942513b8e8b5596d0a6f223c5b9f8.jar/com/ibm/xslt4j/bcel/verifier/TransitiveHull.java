    ...
    public void update(final String classname) {
        ...
        if (vr == VerificationResult.VR_OK) {
            ...
            for (int j = 0; j < jc.getMethods().length; ++j) {
                vr = v.doPass3a(j);
                if (vr != VerificationResult.VR_OK) {
                    System.out.println(String.valueOf(v.getClassName()) + ", Pass 3a, method " + j + " ['" + jc.getMethods()[j] + "']:\n" + vr);
                }
                vr = v.doPass3b(j);
                if (vr != VerificationResult.VR_OK) {
                    System.out.println(String.valueOf(v.getClassName()) + ", Pass 3b, method " + j + " ['" + jc.getMethods()[j] + "']:\n" + vr);
                }
            }
        }
        --this.indent;
