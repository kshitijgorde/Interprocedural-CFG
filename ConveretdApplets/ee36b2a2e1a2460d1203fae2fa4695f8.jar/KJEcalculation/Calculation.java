...
public abstract class Calculation
{
    ...
    public String JavaScriptReport(final String s, final String s2) throws CalculationException {
        ...
        this.recalculate();
        this.bWithSchedule = false;
        final String replace = replace("**GRAPH2**", s2, replace("**GRAPH**", s, replace("**GRAPH**", String.valueOf("<script>\n//function toggleChrome() {\n  var is_chrome = navigator.userAgent.toLowerCase().indexOf('chrome') > -1;  \n  if (is_chrome) {\n    thisDiv = document.getElementById(\"GraphCalculator1\");\n    if (thisDiv) {thisDiv.style.display =\"show\";}\n    thisDiv = document.getElementById(\"GraphCalculator2\");\n    if (thisDiv) {thisDiv.style.display = \"show\";}\n  }\n//}\n</script>\n") + " " + s, replace("&amp;NBSP;", "&nbsp;", replace("QQ_HEADER_QQ", this.MHI, replace("CURRENCY_LABEL", this.sCurrency, this.formatReport(replace("&QCPQ;", "&copy;", replace("&NBSP;", "&nbsp;", replace("**Q**", "\"", replace("&QQ", "&#", replace("<START>", "<HTML><HEAD>", this._sMessage))))))))))));
