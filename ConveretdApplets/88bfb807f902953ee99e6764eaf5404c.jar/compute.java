...
class compute
{
    public int computeLat(final int n, final double n2) {
        final double n3 = 0.017453292519943295;
        return (int)Math.round(Math.atan(-Math.cos(n * n3) / Math.tan(n2 * n3)) / n3);
    }
    ...
    public String azDirection(final double n) {
        final String[] array = { "N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW" };
        final int n2 = (int)((n - 11.25) / 22.5);
        if (n < 11.25 || n > 348.75) {
            return "N";
        }
        return array[n2 + 1];
