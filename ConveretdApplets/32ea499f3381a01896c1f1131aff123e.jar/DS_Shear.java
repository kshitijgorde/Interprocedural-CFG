...
public class DS_Shear extends Applet implements Runnable
{
    Thread _fld0;
    int _fld1;
    Font _fld3;
        ...
        if (this._fld9 != null) {
            if (this._fld4b == 0) {
                if (!this._fld1f) {
                    ...
                    graphics.drawImage(this._fld9, 0, 0, this);
                }
            }
    ...
    void _mth1b() {
        this._fld1e = this._mth0b("bgcolor", new Color(0)).getRGB();
        String parameter = this.getParameter("speed");
        if (parameter == null) {
            parameter = "2";
        }
        this._fld0e = Integer.valueOf(parameter);
        this._fld0e = ((this._fld0e >= 1) ? ((this._fld0e <= 4) ? this._fld0e : 4) : 1);
        if (this._fld0e == 3) {
            this._fld0e = 2;
        }
        ...
        else if (parameter2.equalsIgnoreCase("HORIZONTAL")) {
            this._fld2e = true;
        }
        else {
            this._fld2e = false;
    ...
    void _mth2b() {
        ...
        for (int i = 0; i < 64; ++i) {
            this._fld5e[i] = -i;
            this._fld5e[64 + i] = -63 + i;
            this._fld5e[128 + i] = i;
            this._fld5e[192 + i] = 63 - i;
        }
        this._fld4e = 128;
    }
    ...
    void _mth4b() {
        ...
        this._fld4e = (this._fld4e + 256) % 256;
        this._fld3e = this._fld5e[this._fld4e];
        if (this._fld2e) {
            this._mth5b();
        }
        else {
    ...
    void _mth5b() {
        ...
        for (int i = -n2; i < n2; ++i) {
            for (int j = -n; j < n; ++j) {
                final int n4 = j + (i * this._fld3e >> 4);
                if (n4 >= -n && n4 < n) {
                    this._fld7[n3++] = this._fld6[(i + n2) * this._fld4 + (n4 + n)];
                }
                else {
                    this._fld7[n3++] = fld1e;
                }
    ...
    void _mth6b() {
        ...
        for (int i = -n2; i < n2; ++i) {
            for (int j = -n; j < n; ++j) {
                final int n4 = i + (j * this._fld3e >> 4);
                if (n4 >= -n2 && n4 < n2) {
                    this._fld7[n3++] = this._fld6[(n4 + n2) * this._fld4 + (j + n)];
                }
                else {
                    this._fld7[n3++] = fld1e;
                }
    ...
    void _mth7b() {
        ...
        if (host.length() > 0 && this._fld6e.length() > 9) {
            ...
            this._fld6e.getBytes(1, n + 1, this._fld2f, 0);
            this._fld3f = new byte[n2];
            this._fld6e.getBytes(0, n2, this._fld3f, 0);
            int n3 = n % 7;
            final int n4 = n % 3;
            for (int i = 0; i < n; ++i) {
            ...
            if (this._fld3f[0] == this._fld3f[n >> 1] && this._fld3f[1 + n] == this._fld3f[1] && this._fld3f[1 + n + 1] == this._fld3f[n >> 1] && this._fld3f[1 + n + 2] == (byte)(97 + n5) && this._fld3f[1 + n + 3] == 45 && this._fld3f[1 + n + 4] == (byte)(122 - n6) && this._fld3f[1 + n + 5] == (byte)(110 + n5) && this._fld3f[1 + n + 6] == this._fld3f[1] && this._fld3f[1 + n + 7] == this._fld3f[n] && (host.equalsIgnoreCase(new String(this._fld2f, 0)) || host.equalsIgnoreCase(new String(array, 0)))) {
                this._fld1f = true;
            }
        }
        try {
            this._fld0f = new URL("http://" + this._fld6b);
        }
        catch (MalformedURLException ex) {
            this._fld0f = null;
        }
    ...
    public final boolean mouseDown(final Event event, final int n, final int n2) {
        if (!this._fld1f) {
            this.getAppletContext().showDocument(this._fld0f, "_blank");
        }
        else if (this._fld7e != null) {
            if (this._fld8e != null) {
                this.getAppletContext().showDocument(this._fld7e, this._fld8e);
            }
            else {
                this.getAppletContext().showDocument(this._fld7e);
            }
        }
        return true;
    }
    ...
    void _mth8b() {
        ...
        if (--fld9c > 0) {
            this._fld9b = true;
            this._fld9c = fld9c;
            this._fld4f = new String[this._fld9c];
            this._fld5f = new Color[this._fld9c];
            this._fld6f = new Color[this._fld9c];
            this._fld7f = new Font[this._fld9c];
            this._fld8f = new FontMetrics[this._fld9c];
            this._fld9f = new String[this._fld9c];
            this._fld0g = new int[this._fld9c];
            this._fld1g = new int[this._fld9c];
            for (int i = 0; i < this._fld9c; ++i) {
                this._fld4f[i] = this.getParameter("overtext" + String.valueOf(i + 1));
                this._fld5f[i] = this._mth0b("overtextcol" + String.valueOf(i + 1), new Color(16777215));
                this._fld6f[i] = this._mth0b("overtextcols" + String.valueOf(i + 1), new Color(0));
                final String parameter = this.getParameter("overtexty" + String.valueOf(i + 1));
                if (parameter == null) {
                    this._fld1g[i] = 10;
                }
                else {
                    this._fld1g[i] = Integer.parseInt(parameter);
                }
                ...
                this._fld7f[i] = new Font(parameter2, n, int1);
                this._fld8f[i] = this._fld2c.getFontMetrics(this._fld7f[i]);
                this._fld9f[i] = this.getParameter("overTextType" + String.valueOf(i + 1));
                if (this._fld9f[i] == null) {
                    this._fld9f[i] = "scrollleft";
                }
                ...
                if (parameter5 == null) {
                    ...
                    this._fld0g[i] = Integer.valueOf(parameter5);
                    if (this._fld0g[i] < 1 || this._fld0g[i] > 4) {
                        this._fld0g[i] = 2;
                    }
                }
            }
    ...
    void _mth9b() {
        this._fld2d = this._fld8f[this._fld8c].stringWidth(this._fld4f[this._fld8c]);
        this._fld3d = this._fld8f[this._fld8c].getHeight();
        if (this._fld9f[this._fld8c].equalsIgnoreCase("scrolldown")) {
            this._fld0d = this._fld4 - this._fld2d >> 1;
            this._fld1d = 0;
            return;
        }
        if (this._fld9f[this._fld8c].equalsIgnoreCase("scrollup")) {
            this._fld0d = this._fld4 - this._fld2d >> 1;
            this._fld1d = this._fld5 + this._fld3d;
            return;
        }
        if (this._fld9f[this._fld8c].equalsIgnoreCase("scrollright")) {
            this._fld0d = -this._fld2d;
            this._fld1d = this._fld1g[this._fld8c] + (this._fld3d >> 1) + (this._fld3d >> 3);
            return;
        }
    ...
    void _mth0c(final Graphics graphics) {
        graphics.setFont(this._fld7f[this._fld8c]);
        graphics.setColor(this._fld6f[this._fld8c]);
        graphics.drawString(this._fld4f[this._fld8c], this._fld0d + 1, this._fld1d + 1);
        graphics.setColor(this._fld5f[this._fld8c]);
        graphics.drawString(this._fld4f[this._fld8c], this._fld0d, this._fld1d);
        if (this._fld9f[this._fld8c].equalsIgnoreCase("scrolldown")) {
            this._fld1d += this._fld0g[this._fld8c];
        }
        else if (this._fld9f[this._fld8c].equalsIgnoreCase("scrollup")) {
            this._fld1d -= this._fld0g[this._fld8c];
        }
        else if (this._fld9f[this._fld8c].equalsIgnoreCase("scrollright")) {
            this._fld0d += this._fld0g[this._fld8c];
        }
        else {
            this._fld0d -= this._fld0g[this._fld8c];
        }
