...
public abstract class aG extends Panel
{
    public static int q;
    public Image q;
    private s[] q;
    ...
    public final synchronized void q(final s s) {
        try {
            int q = 0;
            int r = -1;
            ...
            this.q(this.q[(this.t - 1 + this.y) % this.q.length] = s, this.q.size().width - 12 - aG.q, this.getFontMetrics(aT.w.w()));
            ...
            if ((n2 = this.r - this.w - height - 1) < s.q && (graphics = this.q.getGraphics()) != null) {
                this.q(s, this.r - s.q - this.w + 1, graphics);
            }
            if (n2 > 0) {
                if (this.q.r() == 4) {
                    this.q(-n2);
    ...
    public abstract void q(final s p0, final int p1, final FontMetrics p2);
    
    protected abstract void q(final s p0, final int p1, final Graphics p2);
    
    public final synchronized s q(final int n) {
    ...
    public final s q(final Event event) {
        synchronized (this) {
            if (event.x >= aG.q) {
                int n = event.y + this.w;
                final int n2 = event.x - aG.q;
                for (int i = 0; i < this.t; ++i) {
                    final s q = this.q(i);
                    if (n < q.q) {
                        q.r = -1;
                        ...
                        final int n4;
                        final int n3 = (n4 = n - (3 + fontMetrics.getAscent())) / (fontMetrics = this.getFontMetrics(aT.w.w())).getHeight();
                        if (n4 > 0 && n3 < q.w) {
                            int n5;
    ...
    public final synchronized void q() {
        ...
        final int n = this.q.size().width - 6 - aG.q;
        final FontMetrics fontMetrics = this.getFontMetrics(aT.w.w());
        for (int i = 0; i < this.t; ++i) {
            final s q = this.q(i);
    ...
    public aG(final bq bq, final boolean w) {
        this.q = 0L;
        this.q = null;
        this.w = 0;
        this.e = 0;
        ...
        (this.q = new bH(1)).setLineIncrement(10);
        layout.setConstraints(this.q, gridBagConstraints);
        this.add(this.q);
    }
}
