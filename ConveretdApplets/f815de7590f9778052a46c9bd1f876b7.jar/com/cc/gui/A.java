...
public class A extends E
{
    ...
    private static final double x = 1.0471975511965976;
    ...
    public A(final B t, final Component component) {
        super(component);
        this.z = 100.0;
        this.w = 0.0;
        this.v = null;
    ...
    private int O() {
        final int m = this.M();
        return (int)(m + (this.B().height - 2 * m - 20) * this.w / this.z);
    }
    ...
    private double G(final int n) {
        final int m = this.M();
        return (n - m) * this.z / (this.B().height - 2 * m - 20);
    }
    ...
    private void A(final double n, final boolean b) {
        final double min = Math.min(this.z, Math.max(n, 0.0));
        if (Math.abs(this.w - min) < 1.0E-6) {
            return;
        }
    ...
    public void E(final Point point) {
        if (this.N().contains(point)) {
            this.v = new Integer(point.y - this.N().y);
        }
        else {
            this.v = null;
        }
