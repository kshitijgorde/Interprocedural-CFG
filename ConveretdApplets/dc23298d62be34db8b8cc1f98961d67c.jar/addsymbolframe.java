...
public class addsymbolframe extends Frame implements FocusListener, ActionListener, MouseListener, Runnable
{
    Dialog warning;
    Button fok;
    Button ok;
    ...
    String oldsymbol;
    String newsymbol;
    useHandleInterface mainclass;
    String dow;
    Vector existingListSymbols;
    Vector existingIndices;
    
    public addsymbolframe(final String ssy, final useHandleInterface mainclass) {
        ...
        this.idx = new Checkbox[13];
        this.idxSymbols = new String[] { "^DJI", "^DJT", "^COMPX", "^NDX", "^NYA", "^RUO", "^SPX", ".SP100", ".SP400", ".SP600", "^TSX", "^CND" };
        this.idxNames = new String[] { "Dow Jones Ind Avg", "DJ Transportation", "NASDAQ Comp", "NASDAQ 100", "NYSE Comp", "Russell 2K", "S&P 500", "S&P 100", "S&P Midcap 400", "S&P Smallcap 600", "S&P TSX", "NASDAQ Canada" };
        this.olddata = null;
        this.framethread = null;
        this.newsymbol = "";
        this.existingListSymbols = new Vector();
        this.existingIndices = new Vector();
        this.oldsymbol = ssy;
        this.mainclass = mainclass;
        for (int i = 0; i < this.idx.length; ++i) {
            this.idx[i] = new Checkbox();
        }
        ...
        buttonpanel.add(l5);
        final int noofIndices = this.idx.length;
        final Panel indexes = new Panel();
        final Panel indexLabels = new Panel();
        indexes.setLayout(new GridLayout(noofIndices, 1, 0, 0));
        indexLabels.setLayout(new GridLayout(noofIndices, 1, 0, 0));
        for (int j = 0; j < this.idxNames.length; ++j) {
    ...
    public Vector getExistingSymbol() {
        final Vector sym = new Vector();
        for (int i = 0; i < this.list.countItems(); ++i) {
            sym.add(this.list.getItem(i));
        }
        return sym;
    }
    
    public void setExistingListSymbols(final Vector existingListSymbols) {
        this.existingListSymbols = existingListSymbols;
    }
    
    public Vector getExistingIndices() {
        final Vector indSym = new Vector();
        if (this.idxSymbols.length > 0) {
            for (int i = 0; i < this.idxSymbols.length; ++i) {
                if (this.idx[i].getState()) {
                    indSym.add(this.idxSymbols[i]);
                }
            }
        }
        return indSym;
    ...
    public void setExistingIndices(final Vector existingIndices) {
    ...
    public void setSymbols() {
        ...
        this.newsymbol = stocks;
        if (!indices.equals("")) {
            this.newsymbol = this.newsymbol + "," + indices;
        }
        this.olddata = this.mainclass.getDataVector();
    ...
    public void run() {
        int n = 0;
        this.mainclass.setCookie(this.newsymbol);
        this.mainclass.dorepainting();
        this.mainclass.setSymbol(this.newsymbol);
        this.mainclass.getQuoteFromServer();
        while (this.framethread != null) {
            ...
            ++n;
            final Vector newdata = this.mainclass.getDataVector();
            if ((this.oldsymbol.equals(this.newsymbol) && newdata == this.olddata) || (!this.oldsymbol.equals(this.newsymbol) && newdata != this.olddata) || n > 15) {
                this.mainclass.reloaddata();
                this.framethread = null;
            }
        }
    }
    ...
    public void actionPerformed(final ActionEvent ev) {
        if (ev.getSource() == this.ok) {
            ...
            this.symbol.setText("");
            if (this.existingListSymbols != null) {
                this.list.removeAll();
                final Iterator it = this.existingListSymbols.iterator();
                while (it.hasNext()) {
                    this.list.add(it.next());
                }
            }
            if (this.existingIndices != null && this.idxSymbols.length > 0) {
                for (int j = 0; j < this.idxSymbols.length; ++j) {
                    if (this.idx[j].getState() && this.existingIndices.contains(this.idxSymbols[j])) {
                        this.idx[j].setState(true);
                    }
                    else {
                        this.idx[j].setState(false);
                    }
                }
            }
            this.hide();
            ...
            this.mainclass.openLookup();
        }
    }
    
    public void mouseClicked(final MouseEvent me) {
        this.hide();
        this.mainclass.showBrowser("http://www.quotemedia.com", "_top");
    }
