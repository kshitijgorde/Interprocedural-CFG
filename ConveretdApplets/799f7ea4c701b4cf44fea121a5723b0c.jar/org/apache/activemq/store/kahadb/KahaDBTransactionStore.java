...
public class KahaDBTransactionStore implements TransactionStore
{
        ...
        if (txid != null) {
            if (!txid.isXATransaction() && this.theStore.isConcurrentStoreAndDispatchTransactions()) {
                ...
                if (tx != null) {
                    ...
                    if (doneSomething) {
                        final KahaTransactionInfo info = this.getTransactionInfo(txid);
                        this.theStore.store(((KahaCommitCommandBase<JournalCommand<?>>)new KahaCommitCommand()).setTransactionInfo(info), true, null, null);
                    }
                }
                ...
                synchronized (this) {
                    this.theStore.store(((KahaCommitCommandBase<JournalCommand<?>>)new KahaCommitCommand()).setTransactionInfo(info2), true, preCommit, postCommit);
                }
            }
        }
        else {
    ...
    public void rollback(final TransactionId txid) throws IOException {
        if (txid.isXATransaction() || !this.theStore.isConcurrentStoreAndDispatchTransactions()) {
            final KahaTransactionInfo info = this.getTransactionInfo(txid);
            this.theStore.store(((KahaRollbackCommandBase<JournalCommand<?>>)new KahaRollbackCommand()).setTransactionInfo(info), false, null, null);
        }
        else {
            this.inflightTransactions.remove(txid);
        }
    ...
    Future<Object> asyncAddQueueMessage(final ConnectionContext context, final MessageStore destination, final Message message) throws IOException {
        if (message.getTransactionId() == null) {
    ...
    Future<Object> asyncAddTopicMessage(final ConnectionContext context, final MessageStore destination, final Message message) throws IOException {
        if (message.getTransactionId() == null) {
        ...
        tx.add(new AddMessageCommand(context) {
            ...
            public Future run(final ConnectionContext ctx) throws IOException {
    ...
    private KahaTransactionInfo getTransactionInfo(final TransactionId txid) {
        return this.theStore.createTransactionInfo(txid);
    }
