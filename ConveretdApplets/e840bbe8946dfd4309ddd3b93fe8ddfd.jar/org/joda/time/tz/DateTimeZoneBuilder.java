...
public class DateTimeZoneBuilder
{
    ...
    public static DateTimeZone readFrom(final InputStream inputStream, final String s) throws IOException {
    ...
    static long readMillis(final DataInput dataInput) throws IOException {
        ...
        switch (unsignedByte >> 6) {
            ...
            case 2: {
                return (unsignedByte << 58 >> 26 | dataInput.readUnsignedByte() << 24 | dataInput.readUnsignedByte() << 16 | dataInput.readUnsignedByte() << 8 | dataInput.readUnsignedByte()) * 1000L;
            }
            case 3: {
    ...
    private static DateTimeZone buildFixedZone(final String s, final String s2, final int n, final int n2) {
        if ("UTC".equals(s) && s.equals(s2) && n == 0 && n2 == 0) {
            return DateTimeZone.UTC;
        }
        return new FixedDateTimeZone(s, s2, n, n2);
    }
    
    public DateTimeZoneBuilder() {
        this.iRuleSets = new ArrayList(10);
    }
    ...
    public DateTimeZoneBuilder addCutover(final int n, final char c, final int n2, final int n3, final int n4, final boolean b, final int n5) {
        final OfYear ofYear = new OfYear(c, n2, n3, n4, b, n5);
        if (this.iRuleSets.size() > 0) {
            this.iRuleSets.get(this.iRuleSets.size() - 1).setUpperLimit(n, ofYear);
        }
    ...
    public DateTimeZone toDateTimeZone(final String s, final boolean b) {
        ...
        final ArrayList list = new ArrayList<Transition>();
    ...
    private boolean addTransition(final ArrayList list, final Transition transition) {
    ...
    public void writeTo(final String s, final OutputStream outputStream) throws IOException {
        if (outputStream instanceof DataOutput) {
            this.writeTo(s, (DataOutput)outputStream);
        }
        else {
            this.writeTo(s, (DataOutput)new DataOutputStream(outputStream));
        }
    }
    ...
    private static final class OfYear
    {
        ...
        OfYear(final char iMode, final int iMonthOfYear, final int iDayOfMonth, final int iDayOfWeek, final boolean iAdvance, final int iMillisOfDay) {
            if (iMode != 'u' && iMode != 'w' && iMode != 's') {
                throw new IllegalArgumentException("Unknown mode: " + iMode);
            }
            this.iMode = iMode;
    ...
    private static final class Recurrence
    {
        ...
        Recurrence renameAppend(final String s) {
            return this.rename((this.iNameKey + s).intern());
        }
    }
    ...
    private static final class RuleSet
    {
        ...
        RuleSet(final RuleSet set) {
            ...
            this.iRules = new ArrayList(set.iRules);
    ...
    private static final class PrecalculatedZone extends DateTimeZone
    {
        ...
        static PrecalculatedZone create(final String s, final boolean b, final ArrayList list, DSTZone dstZone) {
            final int size = list.size();
            if (size == 0) {
                throw new IllegalArgumentException();
            }
            ...
            for (int k = 0; k < array4.length - 1; ++k) {
                ...
                if (n != n2 && n3 == n4 && s2.equals(s3) && period.getYears() == 0 && period.getMonths() > 4 && period.getMonths() < 8 && s2.equals(array5[2]) && s2.equals(array5[4])) {
                    System.out.println("Fixing duplicate name key - " + s3);
                    System.out.println("     - " + new DateTime(array[k], instanceUTC) + " - " + new DateTime(array[k + 1], instanceUTC));
                    if (n > n2) {
                        array4[k] = (s2 + "-Summer").intern();
                    }
                    else if (n < n2) {
                        array4[k + 1] = (s3 + "-Summer").intern();
                        ++k;
                    }
                }
            }
        ...
        public String getNameKey(final long n) {
            ...
            final int binarySearch = Arrays.binarySearch(iTransitions, n);
            if (binarySearch >= 0) {
                return this.iNameKeys[binarySearch];
            }
            final int n2 = ~binarySearch;
            if (n2 < iTransitions.length) {
                if (n2 > 0) {
                ...
                return this.iTailZone.getNameKey(n);
            }
        }
        ...
        public int getOffset(final long n) {
            ...
            if (n2 < iTransitions.length) {
                ...
                return this.iTailZone.getOffset(n);
            }
        }
        ...
        public int getStandardOffset(final long n) {
            ...
            if (n2 < iTransitions.length) {
                ...
                return this.iTailZone.getStandardOffset(n);
            }
        }
        ...
        public long nextTransition(long n) {
            final long[] iTransitions = this.iTransitions;
            final int binarySearch = Arrays.binarySearch(iTransitions, n);
            final int n2 = (binarySearch >= 0) ? (binarySearch + 1) : (~binarySearch);
            if (n2 < iTransitions.length) {
                return iTransitions[n2];
            }
            if (this.iTailZone == null) {
                return n;
            }
            final long n3 = iTransitions[iTransitions.length - 1];
            if (n < n3) {
                n = n3;
            }
            return this.iTailZone.nextTransition(n);
        }
        ...
        public long previousTransition(final long n) {
            ...
            if (binarySearch >= 0) {
                ...
                if (n2 < iTransitions.length) {
                    if (n2 > 0) {
                        final long n3 = iTransitions[n2 - 1];
                ...
                if (this.iTailZone != null) {
                    final long previousTransition = this.iTailZone.previousTransition(n);
                    if (previousTransition < n) {
                        return previousTransition;
                    }
                }
                final long n4 = iTransitions[n2 - 1];
        ...
        public void writeTo(final DataOutput dataOutput) throws IOException {
            ...
            for (int k = 0; k < length; ++k) {
                ...
                final String s = this.iNameKeys[k];
                int l = 0;
                while (l < size) {
                    if (array[l].equals(s)) {
                        if (size < 256) {
