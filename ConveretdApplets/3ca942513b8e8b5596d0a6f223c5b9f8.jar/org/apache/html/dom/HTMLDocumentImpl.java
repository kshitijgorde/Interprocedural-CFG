...
public class HTMLDocumentImpl extends DocumentImpl implements HTMLDocument
{
    ...
    private static void populateElementType(final String s, final String s2) {
        try {
            HTMLDocumentImpl._elementTypesHTML.put(s, ObjectFactory.findProviderClass("org.apache.html.dom." + s2, ((HTMLDocumentImpl.class$org$apache$html$dom$HTMLDocumentImpl == null) ? (HTMLDocumentImpl.class$org$apache$html$dom$HTMLDocumentImpl = class$("org.apache.html.dom.HTMLDocumentImpl")) : HTMLDocumentImpl.class$org$apache$html$dom$HTMLDocumentImpl).getClassLoader(), true));
        }
        catch (Exception ex2) {
            final RuntimeException ex = new RuntimeException("HTM019 OpenXML Error: Could not find or execute class " + s2 + " implementing HTML element " + s + "\n" + s2 + "\t" + s);
        }
    }
