...
public class ScrollerII extends Applet implements MouseMotionListener, MouseListener, Runnable
{
    private int c;
    private int e;
    private int f;
    ...
    public void run() {
        ...
        final int n = (this.getParameter("speed") == null) ? 20 : (20 - Integer.parseInt(this.getParameter("speed")));
        int n2 = 0;
        try {
        ...
        if (this.i) {
            ...
            graphics2.drawString(s3, width / 2 - graphics2.getFontMetrics().stringWidth(s3) / 2, height / 2 + 4);
            this.repaint();
        }
    }
    ...
    private boolean a() {
        ...
        for (int j = 0; j < ScrollerII.\u00c0.length; ++j) {
            if (ScrollerII.\u00c0[j].indexOf("/") == -1) {
                if (ScrollerII.\u00c0[j].equals(s)) {
                    return true;
                }
                if (ScrollerII.\u00c0[j].equals(hostName)) {
                    return true;
                }
                if (ScrollerII.\u00c0[j].equals(hostAddress)) {
                    return true;
                }
            }
            else {
                ScrollerII.\u00c0[j].substring(0, ScrollerII.\u00c0[j].indexOf("/"));
                final String substring = ScrollerII.\u00c0[j].substring(ScrollerII.\u00c0[j].indexOf("/"));
                if (ScrollerII.\u00c0[j].startsWith(s) && this.getDocumentBase().toString().indexOf(substring) > 0) {
                    return true;
                }
                if (ScrollerII.\u00c0[j].startsWith(hostName) && this.getDocumentBase().toString().indexOf(substring) > 0) {
                    return true;
                }
                if (ScrollerII.\u00c0[j].startsWith(hostAddress) && this.getDocumentBase().toString().indexOf(substring) > 0) {
                    return true;
                }
            }
        }
    ...
    public void init() {
        this.setBackground(this.k = ((this.getParameter("bgcolor") == null) ? Color.white : Color.decode(this.getParameter("bgcolor"))));
        this.a.start();
    }
