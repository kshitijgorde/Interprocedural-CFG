...
public class rtidx1 extends Applet implements Runnable
{
    ...
    public boolean mouseMove(final Event event, final int n, final int n2) {
        ...
        else if (n2 < 15 && this.GMode == 4 && n >= this.size().width - 6 - this.vMet.stringWidth(this.aID[2 - this.sele * 2 + this.LMode]) - 4 && n < this.size().width - 6) {
            this.setCursor(new Cursor(12));
        }
        else {
            this.setCursor(new Cursor(0));
    ...
    int readASP(final String s, final int n, final int[][] array) {
        this.m_tnsi = 0;
        this.m_vv = 0;
        try {
            ...
            final byte[] array2 = new byte[openConnection.getContentLength()];
            int procData = 0;
            final byte[] array3 = new byte[2048];
            int read;
            while (-1 != (read = inputStream.read(array3))) {
                System.arraycopy(array3, 0, array2, procData, read);
                procData = this.procData(array2, procData + read, array);
            }
            inputStream.close();
        }
        catch (Exception ex) {}
    ...
    protected int procData(final byte[] array, int n, final int[][] array2) {
        int n2 = 0;
        int n3 = -1;
        int n4 = 0;
        for (int i = 0; i < n; ++i) {
            ...
            switch (array[i]) {
                ...
                case 44: {
                    ...
                    if (n5 > 0) {
                        ...
                        if (s.length() > 0) {
                            array2[this.m_tnsi][this.m_vv] = Integer.parseInt(s);
                            ++this.m_vv;
                        }
                    }
                    n2 = 0;
                ...
                case 10: {
                    ...
                    if (n6 > 0) {
                        String s2;
                        if (n2 != 0) {
                            s2 = new String(array, n3, n6 - 2);
                        }
                        else {
                            s2 = new String(array, n3, n6 - 1);
                        }
                        if (s2.length() > 0) {
                            array2[this.m_tnsi][this.m_vv] = Integer.parseInt(s2);
                            this.m_vv = 0;
                            ++this.m_tnsi;
                        }
                    }
                ...
                default: {
                    if (n2 != 0) {
                        ...
                        if (n7 > 0) {
                            ...
                            if (s3.length() > 0) {
                                array2[this.m_tnsi][this.m_vv] = Integer.parseInt(s3);
                            }
                        }
                        n2 = 0;
                        n3 = i;
    ...
    public void cpaint(final int n, final int[] array, final int[] array2, final int n2, final int[][] array3) {
        ...
        if (this.GMode == 4) {
            ...
            if (this.showlogo != 0) {
                ...
                this.dBuf.fill3DRect(this.size().width - stringWidth - 9, 0, stringWidth + 3, 15, true);
                this.dBuf.setColor(this.cColor[2]);
                this.dBuf.drawString(this.aID[2 - this.sele * 2 + this.LMode], this.size().width - stringWidth - 7, this.vMet.getAscent() + 2);
            }
        }
    ...
    public void init() {
        ...
        this.tStr = this.parse("\u5609\u5be6,\u5104,\u6307\u6578,\u6f32\u8dcc,\u91cf,JUST,bn,IDX,CHG,VOL", ",");
        ...
        if (this.getParameter("ShowLOGO") != null) {
            this.showlogo = Integer.parseInt(this.getParameter("ShowLOGO"));
        }
        else {
            this.showlogo = 1;
        }
