...
public abstract class CalculatorApplet extends JApplet implements ActionListener
{
    ...
    public String sJavaScriptReport(final String s) {
        ...
        try {
            ...
            if (s.equals("NOGRAPH")) {
                s2 = this.CC.JavaScriptReport("", "", "");
            }
            else {
                s2 = this.CC.JavaScriptReport(this.getGraphApplet(0), this.getGraphApplet(1), this.getGraphApplet(2));
            }
        }
        catch (Exception ex) {
        ...
        int n = s2.indexOf("<BODY");
        if (n < 0) {
            n = s2.indexOf("<body");
        }
        int index = s2.indexOf(">", n);
        if (index < 0) {
            index = 0;
        }
        int n2 = s2.indexOf("</BODY", index);
        if (n2 < 0) {
            n2 = s2.indexOf("</body", index);
        }
        if (n2 < 0) {
            n2 = 0;
        }
        ...
        return s2.substring(index + 1, n2);
    }
