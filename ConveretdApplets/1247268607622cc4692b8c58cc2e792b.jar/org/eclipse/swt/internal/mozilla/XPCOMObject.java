...
public class XPCOMObject
{
    ...
    static {
        ...
        XPCOMObject.IsSolaris = (lowerCase.startsWith("sunos") || lowerCase.startsWith("solaris"));
        OS_OFFSET = (XPCOMObject.IsSolaris ? 2 : 0);
        XPCOMObject.Callbacks = new Callback[80 + XPCOMObject.OS_OFFSET][12];
        XPCOMObject.ObjectMap = new Hashtable();
    }
    
    public XPCOMObject(final int[] array) {
        final int[] array2 = new int[array.length + XPCOMObject.OS_OFFSET];
        synchronized (XPCOMObject.Callbacks) {
            for (int i = 0; i < array.length; ++i) {
                if (XPCOMObject.Callbacks[i + XPCOMObject.OS_OFFSET][array[i]] == null) {
                    XPCOMObject.Callbacks[i + XPCOMObject.OS_OFFSET][array[i]] = new Callback(this.getClass(), "callback" + i, array[i] + 1, true, -2147467259);
                }
                array2[i + XPCOMObject.OS_OFFSET] = XPCOMObject.Callbacks[i + XPCOMObject.OS_OFFSET][array[i]].getAddress();
                if (array2[i + XPCOMObject.OS_OFFSET] == 0) {
                    SWT.error(3);
                }
            }
        }
        // monitorexit(XPCOMObject.Callbacks)
        final int malloc = C.malloc(C.PTR_SIZEOF * (array.length + XPCOMObject.OS_OFFSET));
        C.memmove(malloc, array2, C.PTR_SIZEOF * (array.length + XPCOMObject.OS_OFFSET));
        C.memmove(this.ppVtable = C.malloc(C.PTR_SIZEOF), new int[] { malloc }, C.PTR_SIZEOF);
        XPCOMObject.ObjectMap.put(new LONG(this.ppVtable), this);
    }
    
    static int callback0(final int[] array) {
        final XPCOMObject value = XPCOMObject.ObjectMap.get(new LONG(array[0]));
        if (value == null) {
            return -2147467259;
        }
    ...
    public void dispose() {
        final int[] array = { 0 };
        C.memmove(array, this.ppVtable, C.PTR_SIZEOF);
        C.free(array[0]);
        C.free(this.ppVtable);
        XPCOMObject.ObjectMap.remove(new LONG(this.ppVtable));
        this.ppVtable = 0;
    }
