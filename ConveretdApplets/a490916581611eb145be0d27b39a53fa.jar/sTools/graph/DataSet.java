...
public class DataSet
{
    ...
    public void append(final double[] array, final int n) throws Exception {
        ...
        if (n > 1 || this.stride != 2 || this.length < 6) {
            this.range(this.stride);
        }
        else {
    ...
    public void draw_data(final Graphics graphics, final Rectangle rectangle) {
        if (this.xaxis != null) {
    ...
    protected void draw_polygon(final Graphics graphics, final Rectangle rectangle) {
        int n = 0;
        int n2 = 0;
        int n3 = 0;
        int n4 = 0;
        ...
        final int n5 = clipBounds.x + clipBounds.width;
        final int y = clipBounds.y;
        final int n6 = clipBounds.y + clipBounds.height;
        if (this.data == null || this.data.length < this.stride) {
            return;
        }
        final int[] array = new int[this.data.length + 4];
        final int[] array2 = new int[this.data.length + 4];
        int n7 = 0;
        final int n8 = (int)(rectangle.y + (1.0 - (0 - this.ymin) / this.yrange) * rectangle.height);
        int inside;
        if ((inside = (this.inside(this.data[0], this.data[1]) ? 1 : 0)) != 0) {
            ...
            if (n < x || n > n5 || n2 < y || n2 > n6) {
                inside = 0;
            }
        }
        for (int i = this.stride; i < this.length; i += this.stride) {
            int inside2 = this.inside(this.data[i], this.data[i + 1]) ? 1 : 0;
            if (inside2 != 0 || inside != 0) {
                ...
                if (n3 < x || n3 > n5 || n4 < y || n4 > n6) {
                    inside2 = 0;
                }
            }
            if (inside == 0 && inside2 != 0) {
                n = (int)(rectangle.x + (this.data[i - this.stride] - this.xmin) / this.xrange * rectangle.width);
                n2 = (int)(rectangle.y + (1.0 - (this.data[i - this.stride + 1] - this.ymin) / this.yrange) * rectangle.height);
            }
            if (inside != 0 || inside2 != 0) {
                array[n7] = n;
                array2[n7] = n2;
        ...
        array2[n7] = n8;
        ++n7;
        graphics.fillPolygon(array, array2, n7);
    }
    
    protected void draw_lines(final Graphics graphics, final Rectangle rectangle) {
        int n = 0;
        int n2 = 0;
        int n3 = 0;
        int n4 = 0;
