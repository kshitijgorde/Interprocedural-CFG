...
public abstract class bk extends Panel
{
    public static int q;
    public Image q;
    private A[] q;
    ...
    public final synchronized void q(final A a) {
        try {
            int w = 0;
            int r = -1;
            ...
            this.q(this.t - 1, a);
            this.q(a, this.q.size().width - 12 - bk.q, this.getFontMetrics(bC.w.w()));
            ...
            if ((n2 = this.r - this.w - height - 1) < a.w && (graphics = this.q.getGraphics()) != null) {
                this.q(a, this.r - a.w - this.w + 1, graphics);
            }
            if (n2 > 0) {
                if (this.q.r() == 4) {
                    this.q(-n2);
                    ...
                    int n3 = (int)Math.ceil(n2 / Math.pow(3.0, 4 - this.q.r()));
                    int n4 = n2 / n3;
                    long n5 = 5L;
                    for (int k = 0; k < n4; ++k) {
                        if (k != 0) {
    ...
    public abstract void q(final A p0, final int p1, final FontMetrics p2);
    
    protected abstract void q(final A p0, final int p1, final Graphics p2);
    
    public final synchronized A q(final int n) {
    ...
    public final A q(final Event event) {
        synchronized (this) {
            if (event.x >= bk.q) {
                int n = event.y + this.w;
                final int n2 = event.x - bk.q;
                for (int i = 0; i < this.t; ++i) {
                    final A q = this.q(i);
                    if (n < q.w) {
                        q.t = -1;
                        ...
                        final int n4;
                        final int n3 = (n4 = n - (3 + fontMetrics.getAscent())) / (fontMetrics = this.getFontMetrics(bC.w.w())).getHeight();
                        if (n4 > 0 && n3 < q.e) {
                            int n5;
    ...
    public final synchronized void q() {
        ...
        final int n = this.q.size().width - 6 - bk.q;
        final FontMetrics fontMetrics = this.getFontMetrics(bC.w.w());
        for (int i = 0; i < this.t; ++i) {
            final A q = this.q(i);
    ...
    private final void q(int q, final A a) {
    ...
    public bk(final cs cs, final boolean w) {
        this.q = 0L;
        this.q = null;
        this.w = 0;
        this.e = 0;
        ...
        (this.q = new cO(1)).setLineIncrement(10);
        layout.setConstraints(this.q, gridBagConstraints);
        this.add(this.q);
    }
}
