...
public class Nbr extends JTextField implements KeyListener, FocusListener
{
    ...
    static {
        ...
        Nbr.INPUT_ERROR_SUFFIX = ".";
        Nbr.RANGE_LOW_ERROR_MSG = " must be greater than or equal to ";
        Nbr.RANGE_HIGH_ERROR_MSG = " must be less than or equal to ";
        Nbr.RANGE_SUFFIX_MSG = ".";
    }
    
    public Nbr(final String s, final String s2, final double n, final double n2, final int n3, final int n4, final Applet applet) {
        this(s, s2, n, n2, n3, n4, applet, false);
    }
    
    public Nbr(String parameter, String parameter2, final double dDoubleRangeLow, final double dDoubleRangeHigh, final int nDecimals, final int nFormat, final Applet applet, final boolean bz) {
        this._nDecimals = 0;
        this._nFormat = 5;
        this.myApplet = null;
        this.sChange = "";
    ...
    public void keyReleased(final KeyEvent keyEvent) {
        if (keyEvent.getKeyCode() != 16 && keyEvent.getKeyCode() != 27) {
    ...
    public boolean keypressOk(final char c) {
        return this.isEditable() && (c < 'a' || c > 'z') && (c < 'A' || c > 'Z') && c != '!' && c != '@' && c != '#' && c != '^' && c != '&' && c != '*' && c != '_' && c != '+' && c != '=' && c != '{' && c != '}' && c != '[' && c != ']' && c != '|' && c != '\\' && c != ';' && c != ':' && c != '\'' && c != '\"' && c != '<' && c != '>' && c != '?' && c != '~' && c != '/' && c != '`' && c != '~' && (c != '.' || this._nDecimals > 0) && (c != '-' || this._dDoubleRangeLow < 0.0);
    }
    ...
    public double toDouble() throws NumberFormatException {
        ...
        try {
            ...
            throw new NumberFormatException(String.valueOf(Nbr.INPUT_ERROR_MSG) + this._sName + Nbr.INPUT_ERROR_SUFFIX);
        }
        if (round < this._dDoubleRangeLow && (!this.bZ || round != 0.0)) {
            throw new NumberFormatException(String.valueOf(this._sName) + Nbr.RANGE_LOW_ERROR_MSG + Fmt.fmt(this._nFormat, this._dDoubleRangeLow, this._nDecimals) + Nbr.RANGE_SUFFIX_MSG);
        }
        if (round > this._dDoubleRangeHigh) {
            throw new NumberFormatException(String.valueOf(this._sName) + Nbr.RANGE_HIGH_ERROR_MSG + Fmt.fmt(this._nFormat, this._dDoubleRangeHigh, this._nDecimals) + Nbr.RANGE_SUFFIX_MSG);
        }
