...
public final class PackageManager
{
    ...
    public void load(final String s) {
        ...
        final ModifiableConfiguration globalConfig = this.booter.getGlobalConfig();
        final Iterator propertyKeys = globalConfig.findPropertyKeys(s);
        while (propertyKeys.hasNext()) {
            final String s2 = propertyKeys.next();
            if (s2.endsWith(".Module")) {
                this.addModule(globalConfig.getConfigProperty(s2));
            }
        }
        Log.debug("Loaded a total of " + this.modules.size() + " modules under prefix: " + s);
    }
    ...
    private boolean loadModule(final ModuleInfo moduleInfo, final ArrayList list, final ArrayList list2, final boolean b) {
        try {
            final Module module = (Module)this.getClass().getClassLoader().loadClass(moduleInfo.getModuleClass()).newInstance();
    ...
    {
        public void insertConfiguration(final HierarchicalConfiguration hierarchicalConfiguration) {
