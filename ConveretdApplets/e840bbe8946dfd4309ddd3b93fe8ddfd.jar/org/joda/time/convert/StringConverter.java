...
class StringConverter extends AbstractConverter implements InstantConverter, PartialConverter, DurationConverter, PeriodConverter, IntervalConverter
{
    ...
    public long getDurationMillis(final Object o) {
        ...
        long n3;
        if (n > 0) {
            n3 = Long.parseLong(substring.substring(0, n));
            String s3 = substring.substring(n + 1);
            if (s3.length() != 3) {
        ...
        if (n3 < 0L) {
            return FieldUtils.safeAdd(FieldUtils.safeMultiply(n3, 1000), -n2);
        }
        return FieldUtils.safeAdd(FieldUtils.safeMultiply(n3, 1000), n2);
    }
    ...
    public void setInto(final ReadWritablePeriod readWritablePeriod, final Object o, final Chronology chronology) {
        ...
        final int into = standard.parseInto(readWritablePeriod, s, 0);
        if (into < s.length()) {
            if (into < 0) {
                standard.withParseType(readWritablePeriod.getPeriodType()).parseMutablePeriod(s);
            }
            throw new IllegalArgumentException("Invalid format: \"" + s + '\"');
        }
    }
    ...
    public Class getSupportedType() {
        return (StringConverter.class$java$lang$String == null) ? (StringConverter.class$java$lang$String = class$("java.lang.String")) : StringConverter.class$java$lang$String;
    }
