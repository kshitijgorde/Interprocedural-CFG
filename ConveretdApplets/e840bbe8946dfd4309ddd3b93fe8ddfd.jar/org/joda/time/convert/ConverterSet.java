...
class ConverterSet
{
    ...
    Converter select(Class iType) throws IllegalStateException {
        final Entry[] iSelectEntries = this.iSelectEntries;
        final int length = iSelectEntries.length;
        int n = (iType == null) ? 0 : (iType.hashCode() & length - 1);
        Entry entry;
        while ((entry = iSelectEntries[n]) != null) {
            if (entry.iType == iType) {
        ...
        final Converter selectSlow = selectSlow(this, iType);
        final Entry entry2 = new Entry(iType, selectSlow);
    ...
    ConverterSet add(final Converter converter, final Converter[] array) {
        ...
        for (int i = 0; i < length; ++i) {
            final Converter converter2 = iConverters[i];
