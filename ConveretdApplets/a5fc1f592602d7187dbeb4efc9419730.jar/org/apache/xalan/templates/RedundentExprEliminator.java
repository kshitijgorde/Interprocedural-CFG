...
public class RedundentExprEliminator extends XSLTVisitor
{
    ...
    protected void eleminateRedundent(final ElemTemplateElement psuedoVarRecipient, final Vector paths) {
        ...
        if (RedundentExprEliminator.DIAGNOSE_NUM_PATHS_REDUCED) {
    ...
    protected void eleminateSharedPartialPaths(final ElemTemplateElement psuedoVarRecipient, final Vector paths) {
        ...
        if (null != list) {
            if (RedundentExprEliminator.DIAGNOSE_MULTISTEPLIST) {
                list.diagnose();
            }
    ...
    protected MultistepExprHolder matchAndEliminatePartialPaths(final MultistepExprHolder testee, final MultistepExprHolder head, final boolean isGlobal, final int lengthToTest, final ElemTemplateElement varScope) {
        ...
        if (null != matchedPaths) {
            ...
            final ElemVariable var = this.createPsuedoVarDecl(root, newIter, isGlobal);
            if (RedundentExprEliminator.DIAGNOSE_MULTISTEPLIST) {
    ...
    protected LocPathIterator changePartToRef(final QName uniquePsuedoVarName, final WalkingIterator wi, final int numSteps, final boolean isGlobal) {
    ...
    protected int findAndEliminateRedundant(final int start, final int firstOccuranceIndex, final ExpressionOwner firstOccuranceOwner, final ElemTemplateElement psuedoVarRecipient, final Vector paths) throws DOMException {
        ...
        if (RedundentExprEliminator.DEBUG) {
        ...
        if (null != head) {
            ...
            final ElemVariable var = this.createPsuedoVarDecl(root, sharedIter, isGlobal);
            if (RedundentExprEliminator.DIAGNOSE_MULTISTEPLIST) {
                System.err.println("Created var: " + var.getName() + (isGlobal ? "(Global)" : ""));
            }
            final QName uniquePsuedoVarName = var.getName();
            while (null != head) {
                final ExpressionOwner owner3 = head.m_exprOwner;
                if (RedundentExprEliminator.DIAGNOSE_MULTISTEPLIST) {
                    this.diagnoseLineNumber(owner3.getExpression());
                }
                this.changeToVarRef(uniquePsuedoVarName, owner3, paths, root);
                head = head.m_next;
            }
    ...
    protected int oldFindAndEliminateRedundant(final int start, final int firstOccuranceIndex, final ExpressionOwner firstOccuranceOwner, final ElemTemplateElement psuedoVarRecipient, final Vector paths) throws DOMException {
        ...
        for (int j = start; j < n; ++j) {
            ...
            if (null != owner2) {
                ...
                if (isEqual) {
                    ...
                    if (!foundFirst) {
                        foundFirst = true;
                        final ElemVariable var = this.createPsuedoVarDecl(psuedoVarRecipient, lpi, isGlobal);
                        if (null == var) {
                            return 0;
                        }
        ...
        if (0 == numPathsFound && paths == this.m_absPaths) {
            final ElemVariable var2 = this.createPsuedoVarDecl(psuedoVarRecipient, lpi, true);
            if (null == var2) {
                return 0;
            }
            uniquePsuedoVarName = var2.getName();
            this.changeToVarRef(uniquePsuedoVarName, firstOccuranceOwner, paths, psuedoVarRecipient);
    ...
    protected ElemVariable createPsuedoVarDecl(final ElemTemplateElement psuedoVarRecipient, final LocPathIterator lpi, final boolean isGlobal) throws DOMException {
        final QName uniquePsuedoVarName = new QName("http://xml.apache.org/xalan/psuedovar", "#" + RedundentExprEliminator.m_uniquePsuedoVarID);
        ++RedundentExprEliminator.m_uniquePsuedoVarID;
        if (isGlobal) {
            return this.createGlobalPsuedoVarDecl(uniquePsuedoVarName, (StylesheetRoot)psuedoVarRecipient, lpi);
        }
        return this.createLocalPsuedoVarDecl(uniquePsuedoVarName, psuedoVarRecipient, lpi);
    }
    
    protected ElemVariable createGlobalPsuedoVarDecl(final QName uniquePsuedoVarName, final StylesheetRoot stylesheetRoot, final LocPathIterator lpi) throws DOMException {
    ...
    protected ElemVariable createLocalPsuedoVarDecl(final QName uniquePsuedoVarName, final ElemTemplateElement psuedoVarRecipient, final LocPathIterator lpi) throws DOMException {
    ...
    static {
        RedundentExprEliminator.m_uniquePsuedoVarID = 1;
        RedundentExprEliminator.DEBUG = false;
        RedundentExprEliminator.DIAGNOSE_NUM_PATHS_REDUCED = false;
        RedundentExprEliminator.DIAGNOSE_MULTISTEPLIST = false;
    }
