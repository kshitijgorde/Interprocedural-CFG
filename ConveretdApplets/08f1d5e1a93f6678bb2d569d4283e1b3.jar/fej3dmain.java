...
public class fej3dmain extends Applet implements ActionListener, KeyListener, MouseListener, MouseMotionListener
{
    ...
    public void framestart(final int n) {
        if (this.w != null) {
            ...
            this.w = new fej3dfrm(this, "StereoPhotoViewer", "Ver0.31", n);
    ...
    public void showhelp() {
        try {
            this.getAppletContext().showDocument(new URL(this.getCodeBase(), "helpwe.htm"), "_blank");
        }
        catch (Exception ex) {}
    }
    ...
    public InputStream inputstm() {
        ...
        String parameter = this.getParameter("filelist");
        if (parameter == null) {
            parameter = "filelste.txt";
        }
        try {
            openStream = new URL(this.getDocumentBase(), parameter).openStream();
        }
        catch (Exception ex) {}
        return openStream;
    }
