...
public class ElemNumber extends ElemTemplateElement
{
    ...
    String getCountString(final TransformerImpl transformer, final int sourceNode) throws TransformerException {
        ...
        if (null != this.m_valueExpr) {
            ...
            final double d_count = Math.floor(countObj.num() + 0.5);
    ...
    private void getFormattedNumber(final TransformerImpl transformer, final int contextNode, final char numberType, final int numberWidth, final long listElement, final FastStringBuffer formattedNumber) throws TransformerException {
        ...
        final CharArrayWrapper alphaCountTable = null;
    ...
    protected String int2singlealphaCount(final long val, final CharArrayWrapper table) {
        final int radix = table.getLength();
        if (val > radix) {
    ...
    protected void int2alphaCount(long val, final CharArrayWrapper aTable, final FastStringBuffer stringBuf) {
        final int radix = aTable.getLength();
        final char[] table = new char[radix];
        int i;
        for (i = 0; i < radix - 1; ++i) {
    ...
    protected String tradAlphaCount(long val, final XResourceBundle thisBundle) {
        ...
        final char[] buf = new char[100];
        int charPos = 0;
        final IntArrayWrapper groups = (IntArrayWrapper)thisBundle.getObject("numberGroups");
        final StringArrayWrapper tables = (StringArrayWrapper)thisBundle.getObject("tables");
        ...
        if (numbering.equals("multiplicative-additive")) {
            ...
            final LongArrayWrapper multiplier = (LongArrayWrapper)thisBundle.getObject("multiplier");
            final CharArrayWrapper zeroChar = (CharArrayWrapper)thisBundle.getObject("zero");
            int i;
            ...
            while (i < multiplier.getLength()) {
                if (val < multiplier.getLong(i)) {
                    ...
                    final long mult = val / multiplier.getLong(i);
                    ...
                    while (k < groups.getLength()) {
                        ...
                        if (mult / groups.getInt(k) <= 0L) {
                            ...
                            final CharArrayWrapper THEletters = (CharArrayWrapper)thisBundle.getObject(tables.getString(k));
                            ...
                            final char multiplierChar = ((CharArrayWrapper)thisBundle.getObject("multiplierChar")).getChar(i);
        ...
        for (int count = 0; count < groups.getLength(); ++count) {
            if (val / groups.getInt(count) > 0L) {
                final CharArrayWrapper theletters = (CharArrayWrapper)thisBundle.getObject(tables.getString(count));
                table = new char[theletters.getLength() + 1];
                int l;
                for (l = 0; l < theletters.getLength(); ++l) {
                    table[l + 1] = theletters.getChar(l);
                }
                table[0] = theletters.getChar(l - 1);
