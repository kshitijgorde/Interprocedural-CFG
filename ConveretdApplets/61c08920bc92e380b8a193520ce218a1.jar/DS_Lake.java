...
public class DS_Lake extends Applet implements Runnable
{
    Thread _fld0;
    int _fld1;
    Font _fld3;
    ...
    public final boolean mouseMove(final Event event, final int n, final int lh) {
        this.lh = lh;
        return this._fld9d = true;
    }
    ...
    void _mth1b() {
        String parameter = this.getParameter("lakeheight");
        if (parameter == null) {
            parameter = "64";
        }
        this._fld0e = Integer.valueOf(parameter);
        this._fld0e = ((this._fld0e >= 1) ? ((this._fld0e <= 256) ? this._fld0e : 256) : 1);
        if (this._fld0e > this._fld5) {
            this._fld0e = this._fld5;
        }
        this._fld0e = this._fld5 - this._fld0e;
        ...
        if (parameter4 != null) {
            this._fld5e = parameter4;
            final String parameter5 = this.getParameter("reglink");
            if (parameter5 != null) {
                try {
                    this._fld6e = new URL("http://" + parameter5);
                }
                catch (MalformedURLException ex) {
                    this._fld6e = null;
                }
        ...
        switch (this._fld1e) {
            case 1: {
                for (int i = 0; i < this._fld5; ++i) {
                    for (int j = 0; j < this._fld4; ++j) {
                        this._fld3e[i * this._fld4 + j] = this._fld6[(this._fld5 - 1 - i) * this._fld4 + j];
                    }
                }
                break;
            }
            case 2: {
                for (int k = 0; k < this._fld5; ++k) {
                    for (int l = 0; l < this._fld4; ++l) {
                        final int n = this._fld6[(this._fld5 - 1 - k) * this._fld4 + l];
                        final int n2 = (int)Math.round(0.2125 * (n >> 16 & 0xFF) + 0.7154 * (n >> 8 & 0xFF) + 0.0721 * (n & 0xFF));
                        this._fld3e[k * this._fld4 + l] = (0xFF000000 | n2 >> 1 << 8 | n2);
                    }
                }
                break;
            }
            case 3: {
                for (int n3 = 0; n3 < this._fld5; ++n3) {
                    for (int n4 = 0; n4 < this._fld4; ++n4) {
                        final int n5 = this._fld6[(this._fld5 - 1 - n3) * this._fld4 + n4];
                        final int n6 = (int)Math.round(0.2125 * (n5 >> 16 & 0xFF) + 0.7154 * (n5 >> 8 & 0xFF) + 0.0721 * (n5 & 0xFF));
                        this._fld3e[n3 * this._fld4 + n4] = (0xFF000000 | n6 << 8 | n6);
                    }
                }
                break;
            }
            case 4: {
                for (int n7 = 0; n7 < this._fld5; ++n7) {
                    for (int n8 = 0; n8 < this._fld4; ++n8) {
                        final int n9 = this._fld6[(this._fld5 - 1 - n7) * this._fld4 + n8];
                        this._fld3e[n7 * this._fld4 + n8] = (0xFF000000 | (n9 >> 16 & 0xFF) >> 1 << 16 | (n9 >> 8 & 0xFF) >> 1 << 8 | (n9 & 0xFF) >> 1);
                    }
                }
                break;
            }
    ...
    void _mth3b() {
        ...
        this._fld4e = (this._fld4e + 1) % this._fld2e;
        this._mth4b();
        this._fld9 = this.createImage(this._fld8);
    }
    
    void _mth4b() {
        final int n = this._fld4 * this.lh;
        final int n2 = this._fld5 - this.lh - 1;
        int n3 = n2 * this._fld4;
        for (int i = 0; i < n; ++i) {
            this._fld7[i] = this._fld6[n3++];
        }
        final double n4 = 6.283185307179586 * this._fld4e / this._fld2e;
        int n5 = n;
        int n6 = 0;
        for (int j = 0; j <= n2; ++j) {
            final int n7 = (int)(this._fld5 / 14.0 * (j + 28.0) * Math.sin(this._fld5 / 14 * (this._fld5 - j) / (j + 1) + n4) / this._fld5);
            if (j <= -n7) {
                for (int k = 0; k < this._fld4; ++k) {
                    this._fld7[n5] = this._fld3e[n6++];
                    ++n5;
                }
            }
            else if (j + n7 < n2) {
                final int n8 = (j + n7) * this._fld4;
                for (int l = 0; l < this._fld4; ++l) {
                    this._fld7[n5] = this._fld3e[n8 + l];
                    ++n5;
                }
            }
            else {
                final int n9 = n2 * this._fld4;
                for (int n10 = 0; n10 < this._fld4; ++n10) {
                    this._fld7[n5] = this._fld3e[n9 + n10];
                    ++n5;
                }
            }
        }
