...
public class msestretch extends Applet implements Runnable
{
    Thread t;
    boolean suspended;
    String infile;
    ...
    public int blue(final int n, final int n2) {
        return msestretch.data[n * this.width + n2] & 0xFF;
    }
    
    private void buildImage() {
        this.photo = this.createImage(new MemoryImageSource(this.width, this.height, msestretch.dataNew, 0, this.width));
    }
    
    private void grabPixels() {
        final PixelGrabber pixelGrabber = new PixelGrabber(this.photo, 0, 0, this.width, this.height, msestretch.data, 0, this.width);
        try {
    ...
    public int green(final int n, final int n2) {
        return (msestretch.data[n * this.width + n2] & 0xFF00) >> 8;
    }
    ...
    public void init() {
        ...
        msestretch.data = new int[this.height * this.width];
        msestretch.dataNew = new int[this.height * this.width];
        this.grabPixels();
        this.restoreData();
        this.buildImage();
    ...
    private void program() {
        ...
        final int n5 = this.XH[this.id] - this.XL[this.id];
        final int n6 = this.YH[this.id] - this.YL[this.id];
        final float n7 = ((int)(10000.0 * Math.random()) % 31 + 70) / 100.0f;
        final float n8 = ((int)(10000.0 * Math.random()) % 31 + 70) / 100.0f;
        final float n9 = ((int)(10000.0 * Math.random()) % 31 + 70) / 100.0f;
        final float n10 = ((int)(10000.0 * Math.random()) % 31 + 70) / 100.0f;
        if (this.id != this.idold) {
            for (int i = this.YL[this.idold]; i <= this.YH[this.idold]; ++i) {
                for (int j = this.XL[this.idold]; j <= this.XH[this.idold]; ++j) {
                    msestretch.dataNew[i * this.width + j] = msestretch.data[i * this.width + j];
                }
            }
            this.idold = this.id;
        }
        for (int k = this.YL[this.id]; k <= this.YH[this.id]; ++k) {
            for (int l = this.XL[this.id]; l <= this.XH[this.id]; ++l) {
                final float n11 = n7 + (l - this.XL[this.id]) * (n8 - n7) / n5;
                final float n12 = n9 + (k - this.YL[this.id]) * (n10 - n9) / n6;
                final float n13 = (l - n) / n11 + n;
                final float n14 = (k - n2) / n12 + n2;
                if (n13 > 0.0f && n13 < this.width - 1 && n14 > 0.0f && n14 < this.height - 1) {
                    final float n15 = n13 - (int)n13;
                    final float n16 = n14 - (int)n14;
                    this.setRGB(k, l, (int)((1.0f - n15) * (1.0f - n16) * this.red((int)n14, (int)n13) + (1.0f - n15) * n16 * this.red((int)n14 + 1, (int)n13) + n15 * (1.0f - n16) * this.red((int)n14, (int)n13 + 1) + n15 * n16 * this.red((int)n14 + 1, (int)n13 + 1)), (int)((1.0f - n15) * (1.0f - n16) * this.green((int)n14, (int)n13) + (1.0f - n15) * n16 * this.green((int)n14 + 1, (int)n13) + n15 * (1.0f - n16) * this.green((int)n14, (int)n13 + 1) + n15 * n16 * this.green((int)n14 + 1, (int)n13 + 1)), (int)((1.0f - n15) * (1.0f - n16) * this.blue((int)n14, (int)n13) + (1.0f - n15) * n16 * this.blue((int)n14 + 1, (int)n13) + n15 * (1.0f - n16) * this.blue((int)n14, (int)n13 + 1) + n15 * n16 * this.blue((int)n14 + 1, (int)n13 + 1)));
                }
                else {
                    msestretch.dataNew[k * this.width + l] = msestretch.data[k * this.width + l];
                }
            }
        }
        this.buildImage();
    ...
    public int red(final int n, final int n2) {
        return (msestretch.data[n * this.width + n2] & 0xFF0000) >> 16;
    }
    ...
    private void scroll(final int n, final int n2) {
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                msestretch.dataNew[(i + n2 + this.height) % this.height * this.width + (j + n + this.width) % this.width] = msestretch.data[i * this.width + j];
            }
        }
        this.buildImage();
    }
    ...
    public void setRGB(final int n, final int n2, final int n3, final int n4, final int n5) {
        msestretch.dataNew[n * this.width + n2] = new Color(n3, n4, n5).getRGB();
    }
    ...
    private void zoom(final int n) {
        ...
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                msestretch.dataNew[i * this.width + j] = msestretch.data[((i - n3) * 2 / n + this.yc[this.id] + this.height) % this.height * this.width + ((j - n2) * 2 / n + this.xc[this.id] + this.width) % this.width];
            }
        }
        this.buildImage();
    }
