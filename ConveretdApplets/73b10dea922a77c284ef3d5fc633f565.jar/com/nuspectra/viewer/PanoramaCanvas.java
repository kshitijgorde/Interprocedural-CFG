...
public class PanoramaCanvas extends Canvas implements SessionListener
{
    ...
    private void init(Image image) {
        ...
        this.wvMapper = new WvMapper(this.hardScope, this.size);
        int w = 0;
        int h = 0;
        if (image != null) {
        ...
        if (w <= 0 || h <= 0) {
            image = null;
            final int amtby = this.hardScope.getDegRangeX();
            this.size.width = ((this.hardScope.getDegRangeX() < 12000) ? 240 : 480);
            if (amtby != 0) {
                this.size.height = this.size.width * this.hardScope.getDegRangeY() / amtby;
            }
        }
        else {
            this.size.width = w;
        ...
        (this.wvMapper = new WvMapper(this.viewScope, this.size)).mapRect(this.hardScope, this.hardRect);
        this.wvMapper.mapRect(this.softScope, this.softRect);
        this.wvMapper.mapRect(this.viewScope, this.viewRect);
        this.debugScopes("hard");
    ...
    private void debugScopes(final String s) {
        if (this.maxDebug) {
            this.println("SCOPES *** " + s);
            this.println("wvMapper=" + this.wvMapper);
