...
public class elevaz0985 extends Applet
{
    String versStr;
    final char deg = '°';
    public compute comp;
    ...
    public void init() {
        ...
        this.writeChoice.addItem("60 Min");
        this.writeChoice.addItem("15 Min");
        this.writeChoice.addItem("5 Min");
    ...
    public double RightAscension(final double T) {
        ...
        final double e = 0.016708617 - 4.2037E-5 * T - 1.236E-7 * T * T;
        final double n = M + C;
        this.R = 1.000001018 * (1.0 - e * e) / (1.0 + e * Math.cos(0.017453292519943295 * n));
        this.THETA = theta;
        double eps = this.EPS(T);
        this.trueDELTA = Math.asin(Math.sin(0.017453292519943295 * eps) * Math.sin(0.017453292519943295 * theta)) / 0.017453292519943295;
    ...
    public void doData(final int interval) {
        int n = 0;
        for (int i = 0; i <= 24; ++i) {
            ...
            double dec = this.DELTA;
            double elev = this.comp.sun_elev(Jd, this.latitude, -this.longitude, dec, RA / 15.0);
            double az = this.comp.azimuth(Jd, RA / 15.0, dec, this.latitude, this.longitude);
            String str2 = String.valueOf(Math.round(100.0 * elev) / 100.0);
            ...
            String str3 = String.valueOf(Math.round(100.0 * az) / 100.0);
            ...
            double appElev = elev + this.comp.refract(elev);
            String str4;
            if (elev > -0.83) {
                str4 = String.valueOf(Math.round(100.0 * appElev) / 100.0);
            }
            else {
                str4 = "";
            }
            if (str4.length() - str4.indexOf(".") == 2) {
                str4 = String.valueOf(str4) + "0";
            }
            while (str4.length() < 10) {
                str4 = " " + str4;
            }
            this.Data[n] = String.valueOf(str) + "  " + str2 + str4 + "  " + str3;
            double LHA = 15.0 * this.comp.LM_Sidereal_Time(Jd, -this.longitude) - RA;
            if (LHA < 0.0) {
                LHA += 360.0;
            }
            LHA %= 360.0;
            String str5 = String.valueOf(Math.round(100.0 * LHA) / 100.0);
            if (LHA < 10.0) {
                str5 = " " + str5;
            }
            if (LHA < 100.0) {
            ...
            final int n2 = n;
            data[n2] = String.valueOf(data[n2]) + "    " + str5;
            if (interval == 15) {
                if (elev > -6.0) {
                    for (int k = 1; k <= 3; ++k) {
                        ++n;
                        final double Jd2 = Jd + k * 0.25 / 24.0;
                        if (i < 10) {
                            str = "0" + i + ":" + k * 15;
                        }
                        else {
                        ...
                        final double T2 = (Jd2 - 2451545.0) / 36525.0;
                        RA = this.RightAscension(T2);
                        dec = this.DELTA;
                        elev = this.comp.sun_elev(Jd2, this.latitude, -this.longitude, dec, RA / 15.0);
                        az = this.comp.azimuth(Jd2, RA / 15.0, dec, this.latitude, this.longitude);
                        ...
                        str3 = String.valueOf(Math.round(100.0 * az) / 100.0);
                        if (str3.length() - str3.indexOf(".") == 2) {
                            str3 = String.valueOf(str3) + "0";
                        }
                        while (str3.length() < 8) {
                        ...
                        appElev = elev + this.comp.refract(elev);
                        if (elev > -0.83) {
                            str4 = String.valueOf(Math.round(100.0 * appElev) / 100.0);
                        }
                        else {
                        ...
                        this.Data[n] = String.valueOf(str) + "  " + str2 + str4 + "  " + str3;
                        LHA = 15.0 * this.comp.LM_Sidereal_Time(Jd2, -this.longitude) - RA;
                        if (LHA < 0.0) {
                            LHA += 360.0;
                        }
                        LHA %= 360.0;
                        str5 = String.valueOf(Math.round(100.0 * LHA) / 100.0);
                        if (LHA < 10.0) {
                            str5 = " " + str5;
                        }
                        if (LHA < 100.0) {
                        ...
                        final int n3 = n;
                        data2[n3] = String.valueOf(data2[n3]) + "    " + str5;
                    }
                }
            }
            else if (interval == 5 && elev > -6.0) {
                for (int k = 1; k <= 11; ++k) {
                    ++n;
                    final double Jd2 = Jd + k * 0.05 / 24.0;
                    if (i < 10) {
                        str = "0" + i + ":";
                    }
                    else {
                    ...
                    if (k > 1) {
                        str = String.valueOf(str) + k * 5;
                    }
                    else {
                        str = String.valueOf(str) + "0" + k * 5;
                    }
                    final double T2 = (Jd2 - 2451545.0) / 36525.0;
                    RA = this.RightAscension(T2);
                    dec = this.DELTA;
                    ...
                    final int n4 = n;
                    data3[n4] = String.valueOf(data3[n4]) + "    " + str5;
                }
            }
            ++n;
        }
        ...
        str = String.valueOf(str) + this.gmtStr + " UT" + "\n";
        str = String.valueOf(str) + this.dat.toString() + "\n";
        str = String.valueOf(str) + "Mean obliquity of the ecliptic " + Math.round(1000.0 * this.comp.sunDecRA(this.JD(this.date, this.month + 1, this.year + 1900, 0.0), 4)) / 1000.0 + '°' + "\n";
        str = String.valueOf(str) + "True longitude:   " + Math.round(1000.0 * this.trueLong) / 1000.0 + " = " + this.comp.DMS(this.trueLong) + "\n";
        str = String.valueOf(str) + "True declination: " + Math.round(1000.0 * this.trueDelta) / 1000.0 + " = " + this.comp.DMS(this.trueDelta) + "\n";
        str = String.valueOf(str) + "Radius vector R:  " + Math.round(10000.0 * this.r) / 10000.0 + " AU" + "\n";
        str = String.valueOf(str) + "Equation of time: " + this.comp.MS(this.equation) + "\n";
        str = String.valueOf(str) + "Apparent altitude " + Math.round(10.0 * this.appAlt) / 10.0 + '°' + "\n" + "\n";
        str = String.valueOf(str) + "Local      True     Appar.  Azimuth    Local HA\n";
        str = String.valueOf(str) + "Time       Alt.       Alt.\n";
    ...
    public void paint(final Graphics g) {
        ...
        final double T = (Jd - 2451545.0) / 36525.0;
        final double RA = this.RightAscension(T);
        final double dec = this.DELTA;
        final double L = this.LAMBDA;
        this.trueLong = this.THETA;
        this.trueDelta = this.trueDELTA;
        this.r = this.R;
        final double elev = this.comp.sun_elev(Jd, this.latitude, -this.longitude, dec, RA / 15.0);
        final double az = this.comp.azimuth(Jd, RA / 15.0, dec, this.latitude, this.longitude);
        this.appAlt = elev + this.comp.refract(elev);
        this.equation = this.eot(this.date, this.month + 1, this.year + 1900, UT);
        double trans = 12.0 - this.equation / 60.0 - this.longitude / 15.0;
        trans += this.timezoneoffset;
        final double transElev = this.transElevation(this.JD(this.date, this.month + 1, this.year + 1900, trans - this.timezoneoffset));
        ...
        g.drawString("Date, UT   " + this.gmtStr, 50, 115);
        ...
        g.drawString("Longitude  " + Math.round(1000.0 * L) / 1000.0 + '°' + " = " + this.comp.DMS(L), 50, 160);
        g.drawString("Declin.    " + Math.round(1000.0 * dec) / 1000.0 + '°' + " = " + this.comp.DMS(dec), 50, 175);
        g.drawString("Right Asc. " + Math.round(1000.0 * RA) / 1000.0 + '°' + " = " + this.comp.hms(RA), 50, 190);
        g.drawString(this.str = String.valueOf(this.latStr) + '°' + this.nsStr + ",  " + this.longStr + '°' + this.ewStr, 315, 115);
        g.drawString("Local Sid. Time  " + Math.round(15000.0 * LST) / 1000.0 + '°' + " = " + this.comp.hms(15.0 * LST), 315, 130);
        if (H > 180.0) {
            g.drawString("Loc. Hour Angle  " + Math.round(100.0 * H) / 100.0 + '°' + " = " + this.comp.makeTimeString(H / 15.0 - 12.0), 315, 145);
        }
        else {
            g.drawString("Loc. Hour Angle  " + Math.round(100.0 * H) / 100.0 + '°' + " = " + this.comp.makeTimeString(12.0 + H / 15.0), 315, 145);
        }
        g.drawString("True Elevation   " + Math.round(10.0 * elev) / 10.0 + '°', 315, 160);
        g.drawString("Azimuth          " + Math.round(10.0 * az) / 10.0 + '°', 315, 175);
        g.drawString("Max. Elev.       " + Math.round(10.0 * transElev) / 10.0 + '°' + " at " + this.comp.HMS(15.0 * trans), 315, 190);
        final String dgr = " deg";
        final String[] text = { "Date, UT   " + this.gmtStr, "Jul. Day   " + Math.round(100000.0 * Jd) / 100000.0, "T2000      " + Math.round(F * T) / F, "Oliqu. eps " + Math.round(1000.0 * this.comp.sunDecRA(Jd, 4)) / 1000.0 + dgr, "Long. Sun  " + Math.round(100.0 * L) / 100.0 + dgr, "Declin.    " + Math.round(100.0 * dec) / 100.0 + dgr, "Right Asc. " + Math.round(100.0 * RA) / 100.0 + dgr, "Local Sid. Time  " + Math.round(15000.0 * LST) / 1000.0 + dgr, "Loc. Hour Angle  " + Math.round(100.0 * H) / 100.0 + dgr, "Azimuth          " + Math.round(10.0 * az) / 10.0 + dgr, "Max. Elev.       " + Math.round(10.0 * transElev) / 10.0 + dgr + " at " + this.comp.HMS(15.0 * trans) };
        ...
        g.drawString(String.valueOf(this.versStr) + "  (c) 2006-2010 J.Giesen - www.GeoAstro.de", 130, this.size().height - 5);
    ...
    public boolean action(final Event event, final Object eventobject) {
        ...
        if (event.target instanceof Choice) {
            if (event.target == this.writeChoice) {
                if (this.writeChoice.getSelectedItem().equals("60 Min")) {
                ...
                if (this.writeChoice.getSelectedItem().equals("15 Min")) {
                    this.writeChoice.select(0);
                    final boolean dem = this.online || this.demo;
                    this.doData(15);
                    final Frame df = new dataFrame(this.year + 1900 + " " + this.monthArray[this.month] + " " + this.date, this.versStr, this.Data, this.nData, dem);
                ...
                if (this.writeChoice.getSelectedItem().equals("5 Min")) {
                    this.writeChoice.select(0);
                    final boolean dem = this.online || this.demo;
                    this.doData(5);
                    final Frame df = new dataFrame(this.year + 1900 + " " + this.monthArray[this.month] + " " + this.date, this.versStr, this.Data, this.nData, dem);
    ...
    public elevaz0985() {
        this.versStr = "Elevation & Azimuth  v. 0.985 ";
        this.demo = true;
        this.online = false;
        this.usrLat = 0.0;
        this.usrLong = 0.0;
