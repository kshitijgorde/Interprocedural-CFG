...
public class mseripple extends Applet implements Runnable
{
    Thread t;
    boolean suspended;
    String infile;
    ...
    public int blue(final int n, final int n2) {
        return mseripple.data[n * this.width + n2] & 0xFF;
    }
    
    private void buildImage() {
        this.photo = this.createImage(new MemoryImageSource(this.width, this.height, mseripple.dataNew, 0, this.width));
    }
    
    private void grabPixels() {
        final PixelGrabber pixelGrabber = new PixelGrabber(this.photo, 0, 0, this.width, this.height, mseripple.data, 0, this.width);
        try {
    ...
    public int green(final int n, final int n2) {
        return (mseripple.data[n * this.width + n2] & 0xFF00) >> 8;
    }
    ...
    public void init() {
        ...
        this.offsetstep = new Integer(this.getParameter("offsetstep"));
        this.freqstep = new Float(this.getParameter("freqstep"));
        this.loops = new Integer(this.getParameter("loops"));
        ...
        mseripple.data = new int[this.height * this.width];
        mseripple.dataNew = new int[this.height * this.width];
        this.grabPixels();
        this.restoreData();
        this.buildImage();
    ...
    private void program() {
        ...
        this.freqControl -= this.freqstep;
        this.Offset += this.offsetstep;
        if (this.times >= this.loops) {
            this.times = 0;
            this.maxDist = 0.0f;
        }
        if (this.times == 0) {
            ...
            final int n3 = -n;
            final int n4 = -n2;
            final float maxDist = (float)Math.sqrt(n3 * n3 + n4 * n4);
            if (maxDist > this.maxDist) {
                this.maxDist = maxDist;
            }
            final int n5 = this.width - 1 - n;
            final int n6 = -n2;
            final float maxDist2 = (float)Math.sqrt(n5 * n5 + n6 * n6);
            if (maxDist2 > this.maxDist) {
                this.maxDist = maxDist2;
            }
            final int n7 = -n;
            final int n8 = this.height - 1 - n2;
            final float maxDist3 = (float)Math.sqrt(n7 * n7 + n8 * n8);
            if (maxDist3 > this.maxDist) {
                this.maxDist = maxDist3;
            }
            final int n9 = this.width - 1 - n;
            final int n10 = this.height - 1 - n2;
            final float maxDist4 = (float)Math.sqrt(n9 * n9 + n10 * n10);
            if (maxDist4 > this.maxDist) {
                this.maxDist = maxDist4;
            }
        }
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                final int n11 = j - n;
                final int n12 = i - n2;
                final float n13 = (float)Math.sqrt(n11 * n11 + n12 * n12);
                float n14;
                float n15;
                if (n13 < 1.0f) {
                    n14 = j;
                    ...
                    final float n16 = this.maxDist - n13;
                    final float n17 = (float)(n13 + this.Offset * n16 * Math.sin(n16 * n16 * 3.14159f / 180.0f / this.freqControl) / this.maxDist);
                    n14 = n11 * n17 / n13 + n;
                    n15 = n12 * n17 / n13 + n2;
                }
                if (n14 > 0.0f && n14 < this.width - 1 && n15 > 0.0f && n15 < this.height - 1 && n13 < this.radius) {
                    final float n18 = n14 - (int)n14;
                    final float n19 = n15 - (int)n15;
                    this.setRGB(i, j, (int)((1.0f - n18) * (1.0f - n19) * this.red((int)n15, (int)n14) + (1.0f - n18) * n19 * this.red((int)n15 + 1, (int)n14) + n18 * (1.0f - n19) * this.red((int)n15, (int)n14 + 1) + n18 * n19 * this.red((int)n15 + 1, (int)n14 + 1)), (int)((1.0f - n18) * (1.0f - n19) * this.green((int)n15, (int)n14) + (1.0f - n18) * n19 * this.green((int)n15 + 1, (int)n14) + n18 * (1.0f - n19) * this.green((int)n15, (int)n14 + 1) + n18 * n19 * this.green((int)n15 + 1, (int)n14 + 1)), (int)((1.0f - n18) * (1.0f - n19) * this.blue((int)n15, (int)n14) + (1.0f - n18) * n19 * this.blue((int)n15 + 1, (int)n14) + n18 * (1.0f - n19) * this.blue((int)n15, (int)n14 + 1) + n18 * n19 * this.blue((int)n15 + 1, (int)n14 + 1)));
                }
                else {
                    mseripple.dataNew[i * this.width + j] = mseripple.data[i * this.width + j];
                }
            }
        }
        this.buildImage();
    ...
    public int red(final int n, final int n2) {
        return (mseripple.data[n * this.width + n2] & 0xFF0000) >> 16;
    }
    ...
    private void scroll(final int n, final int n2) {
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                mseripple.dataNew[(i + n2 + this.height) % this.height * this.width + (j + n + this.width) % this.width] = mseripple.data[i * this.width + j];
            }
        }
        this.buildImage();
    }
    ...
    public void setRGB(final int n, final int n2, final int n3, final int n4, final int n5) {
        mseripple.dataNew[n * this.width + n2] = new Color(n3, n4, n5).getRGB();
    }
    ...
    private void zoom(final int n) {
        ...
        for (int i = 0; i < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                mseripple.dataNew[i * this.width + j] = mseripple.data[((i - n3) * 2 / n + this.yc[this.id] + this.height) % this.height * this.width + ((j - n2) * 2 / n + this.xc[this.id] + this.width) % this.width];
            }
        }
        this.buildImage();
    }
