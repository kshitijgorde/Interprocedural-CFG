...
final class b extends ay implements aA
{
    ...
    public final boolean handleEvent(final Event q) {
        ...
        final int q3 = this.q.q();
        final int w2;
        final au au = ((w2 = this.q.w(q.x)) >= 0) ? this.q.q(w2) : null;
        bO arg;
        if (q2 >= 0 && q2 < q3) {
            arg = be.q(this.q).elementAt(q2);
        }
        else {
            arg = null;
        }
        switch (q.id) {
            ...
            case 501: {
                if (q.metaDown()) {
                    ...
                    if (be.q(this.w) && q2 < be.w(this.w).size() && arg != null && arg.q instanceof bV) {
                        if (be.q(this.w).q((Object)arg.q)) {
                            be.q(this.w).w(arg.q);
                        }
                        else {
                            be.q(this.w).q((bV)arg.q);
                        }
                    }
                    this.repaint();
                    this.requestFocus();
    ...
    public b(final be w, final be q) {
        this.w = w;
        this.q = false;
        this.q = -1;
        this.q = q;
