...
class TGLabelOp implements ImageObserver, TGGraphicsOp
{
    ...
    public Rectangle doIt(final Image image) {
        ...
        final int imageX = this.where.imageX(width);
        final int height = image.getHeight(this);
        if (height < 0) {
            return null;
        }
        final int imageY = this.where.imageY(height);
        ...
        final int n2 = imageY - (height2 - fontMetrics.getDescent());
        graphics.setClip(n, n2, stringWidth, height2);
        graphics.drawString(this.text, imageX, imageY);
        graphics.dispose();
