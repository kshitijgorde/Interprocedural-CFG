...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable
{
    ...
    public Period normalizedStandard(final PeriodType periodType) {
        Period period = new Period(this.getMillis() + this.getSeconds() * 1000L + this.getMinutes() * 60000L + this.getHours() * 3600000L + this.getDays() * 86400000L + this.getWeeks() * 604800000L, DateTimeUtils.getPeriodType(periodType), ISOChronology.getInstanceUTC());
        final int years = this.getYears();
        final int months = this.getMonths();
        if (years != 0 || months != 0) {
            final int safeAdd = FieldUtils.safeAdd(years, months / 12);
            final int n = months % 12;
            if (safeAdd != 0) {
                period = period.withYears(safeAdd);
            }
            if (n != 0) {
                period = period.withMonths(n);
            }
        }
        return period;
    }
