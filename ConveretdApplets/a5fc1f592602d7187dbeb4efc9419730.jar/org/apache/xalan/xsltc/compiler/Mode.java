...
final class Mode implements Constants
{
    ...
    private void peepHoleOptimization(final MethodGenerator methodGen) {
        ...
        while (iter2.hasNext()) {
            ...
            try {
                if (match2[0].hasTargeters() || match2[1].hasTargeters() || match2[2].hasTargeters()) {
                    continue;
                }
                final Instruction iload = match2[0].getInstruction();
                final Instruction aload = match2[1].getInstruction();
                il.insert(match2[0], aload);
                il.insert(match2[0], iload);
                il.delete(match2[0], match2[2]);
            }
        ...
        while (iter3.hasNext()) {
            ...
            try {
                if (match3[0].hasTargeters() || match3[1].hasTargeters()) {
                    continue;
                }
                il.insert(match3[1], new DUP());
                il.delete(match3[1]);
            }
