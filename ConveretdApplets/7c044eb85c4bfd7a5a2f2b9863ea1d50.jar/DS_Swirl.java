...
public class DS_Swirl extends Applet implements Runnable
{
    Thread _fld0;
    int _fld1;
    Font _fld2;
    ...
    void _mth1b() {
        ...
        this._fld2e <<= 1;
        String parameter2 = this.getParameter("radius");
        if (parameter2 == null) {
            parameter2 = "128";
        }
        this._fld2f = Integer.valueOf(parameter2);
        this._fld2f = ((this._fld2f >= 1) ? ((this._fld2f <= 800) ? this._fld2f : 800) : 1);
        String parameter3 = this.getParameter("angle");
        if (parameter3 == null) {
            parameter3 = "360";
        }
        this._fld0f = Integer.valueOf(parameter3);
        this._fld0f = ((this._fld0f >= 90) ? ((this._fld0f <= 720) ? this._fld0f : 720) : 90);
        String parameter4 = this.getParameter("pause");
        if (parameter4 == null) {
            parameter4 = "30";
        }
        this._fld1f = Integer.valueOf(parameter4);
        this._fld1f = ((this._fld1f >= 0) ? ((this._fld1f <= 60) ? this._fld1f : 60) : 0);
        ...
        if (parameter6 != null) {
            this._fld6f = parameter6;
            final String parameter7 = this.getParameter("reglink");
            if (parameter7 != null) {
                try {
                    this._fld7f = new URL("http://" + parameter7);
                }
                catch (MalformedURLException ex) {
                    this._fld7f = null;
                }
    ...
    void _mth2b() {
        this._fld9e = 0;
        this._fld1f *= this._fld2e;
        this._fld4f = (this._fld1f << 1) + (this._fld0f << 1);
        this._fld5f = this._fld4f >> 1;
        this._fld3f = new int[this._fld4f];
        for (int i = 0; i < this._fld1f; ++i) {
            this._fld3f[i] = 0;
        }
        for (int j = this._fld1f; j < this._fld0f + this._fld1f; ++j) {
            this._fld3f[j] = j - this._fld1f;
        }
        for (int k = 0; k < this._fld0f; ++k) {
            this._fld3f[this._fld1f + this._fld0f + k] = this._fld0f - k;
        }
        for (int l = 0; l < this._fld1f; ++l) {
            this._fld3f[this._fld1f + this._fld0f + this._fld0f + l] = 0;
        }
        this._fld5e = new double[this._fld3 * this._fld5];
        this._fld6e = new double[this._fld3 * this._fld5];
        this._fld7e = new double[this._fld3 * this._fld5];
        this._fld8e = new double[this._fld3 * this._fld5];
        final double n = this._fld3 >> 1;
        final double n2 = this._fld5 >> 1;
        this._fld4e = this._fld2f;
        for (int n3 = 0; n3 < this._fld5; ++n3) {
            for (int n4 = 0; n4 < this._fld3; ++n4) {
                final int n5 = n3 * this._fld3 + n4;
                this._fld5e[n5] = n4 - n;
                this._fld6e[n5] = n3 - n2;
                this._fld7e[n5] = this._fld5e[n5] * this._fld5e[n5] + this._fld6e[n5] * this._fld6e[n5];
                this._fld8e[n5] = 1.0 - Math.sqrt(this._fld7e[n5]) / this._fld4e;
                final double[] fld8e = this._fld8e;
                final int n6 = n5;
                fld8e[n6] *= this._fld8e[n5];
            }
        }
    }
    ...
    void _mth3b() {
        if (this._fld9e > 0) {
            if (this._fld9e < this._fld5f) {
                this._fld9e -= this._fld2e;
                return;
            }
            this._fld9e += this._fld2e;
        }
    ...
    void _mth4b() {
        ...
        this._fld9e %= this._fld4f;
        this._fld3e = this._fld3f[this._fld9e];
        this._mth5b();
        this._fld9 = this.createImage(this._fld8);
    }
    
    void _mth5b() {
        final int n = (this._fld3 >> 1) + this._fld3;
        final int n2 = (this._fld5 >> 1) + this._fld5;
        final double n3 = this._fld4e * this._fld4e;
        final double n4 = this._fld3e * 2.0 * 3.141592653589793 / 360.0;
        for (int n5 = this._fld3 * this._fld5, i = 0; i < n5; ++i) {
            if (this._fld7e[i] >= n3) {
                this._fld7[i] = this._fld6[i];
            }
            else {
                final double sin = Math.sin(n4 * this._fld8e[i]);
                final double cos = Math.cos(n4 * this._fld8e[i]);
                final double n6 = this._fld5e[i];
                final double n7 = this._fld6e[i];
                this._fld7[i] = this._fld6[((int)(sin * n6 + cos * n7) + n2) % this._fld5 * this._fld3 + ((int)(cos * n6 - sin * n7) + n) % this._fld3];
            }
        }
    }
    ...
    void _mth7b() {
        ...
        if (--fld9c > 0) {
            ...
            for (int i = 0; i < this._fld9c; ++i) {
                ...
                this._fld5g[i] = this._mth0b("overtextcol" + String.valueOf(i + 1), new Color(16777215));
                this._fld6g[i] = this._mth0b("overtextcols" + String.valueOf(i + 1), new Color(0));
