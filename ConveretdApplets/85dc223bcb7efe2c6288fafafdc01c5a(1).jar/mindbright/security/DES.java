...
public final class DES extends Cipher
{
    ...
    protected int IV0;
    protected int IV1;
    static final int[][] des_skb;
    static final int[][] des_SPtrans;
    ...
    public synchronized void encrypt(final byte[] src, final int srcOff, final byte[] dest, final int destOff, final int len) {
        ...
        for (int end = srcOff + len, si = srcOff, di = destOff; si < end; si += 8, di += 8) {
            ...
            iv2 ^= ((src[si + 4] & 0xFF) | (src[si + 5] & 0xFF) << 8 | (src[si + 6] & 0xFF) << 16 | (src[si + 7] & 0xFF) << 24);
            ...
            dest[di + 4] = (byte)(iv2 & 0xFF);
            dest[di + 5] = (byte)(iv2 >>> 8 & 0xFF);
            dest[di + 6] = (byte)(iv2 >>> 16 & 0xFF);
            dest[di + 7] = (byte)(iv2 >>> 24 & 0xFF);
        }
        this.IV0 = iv0;
        this.IV1 = iv2;
    }
    ...
    public synchronized void decrypt(final byte[] src, final int srcOff, final byte[] dest, final int destOff, final int len) {
        ...
        int iv2 = this.IV1;
        for (int end = srcOff + len, si = srcOff, di = destOff; si < end; si += 8, di += 8) {
            final int d0 = (src[si] & 0xFF) | (src[si + 1] & 0xFF) << 8 | (src[si + 2] & 0xFF) << 16 | (src[si + 3] & 0xFF) << 24;
            final int d2 = (src[si + 4] & 0xFF) | (src[si + 5] & 0xFF) << 8 | (src[si + 6] & 0xFF) << 16 | (src[si + 7] & 0xFF) << 24;
            this.decrypt(d0, d2, out);
            iv0 ^= out[0];
            iv2 ^= out[1];
            dest[di] = (byte)(iv0 & 0xFF);
            dest[di + 1] = (byte)(iv0 >>> 8 & 0xFF);
    ...
    public void setKey(final byte[] key) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            c &= 0xFFFFFFF;
            d &= 0xFFFFFFF;
            int s = DES.des_skb[0][c & 0x3F] | DES.des_skb[1][(c >>> 6 & 0x3) | (c >>> 7 & 0x3C)] | DES.des_skb[2][(c >>> 13 & 0xF) | (c >>> 14 & 0x30)] | DES.des_skb[3][(c >>> 20 & 0x1) | (c >>> 21 & 0x6) | (c >>> 22 & 0x38)];
            t = (DES.des_skb[4][d & 0x3F] | DES.des_skb[5][(d >>> 7 & 0x3) | (d >>> 8 & 0x3C)] | DES.des_skb[6][d >>> 15 & 0x3F] | DES.des_skb[7][(d >>> 21 & 0xF) | (d >>> 22 & 0x30)]);
            this.key_schedule[i * 2] = (t << 16 | (s & 0xFFFF));
            s = (s >>> 16 | (t & 0xFFFF0000));
            this.key_schedule[i * 2 + 1] = (s << 4 | s >>> 28);
        }
    }
    
    public void encrypt(int l, int r, final int[] out) {
        int t = 0;
        int u = 0;
        t = ((r >>> 4 ^ l) & 0xF0F0F0F);
        l ^= t;
        ...
        for (int i = 0; i < 32; i += 4) {
            u = (r ^ this.key_schedule[i]);
            t = (r ^ this.key_schedule[i + 1]);
            t = (t >>> 4) + (t << 28);
            l ^= (DES.des_SPtrans[1][t & 0x3F] | DES.des_SPtrans[3][t >>> 8 & 0x3F] | DES.des_SPtrans[5][t >>> 16 & 0x3F] | DES.des_SPtrans[7][t >>> 24 & 0x3F] | DES.des_SPtrans[0][u & 0x3F] | DES.des_SPtrans[2][u >>> 8 & 0x3F] | DES.des_SPtrans[4][u >>> 16 & 0x3F] | DES.des_SPtrans[6][u >>> 24 & 0x3F]);
            u = (l ^ this.key_schedule[i + 2]);
            t = (l ^ this.key_schedule[i + 3]);
            t = (t >>> 4) + (t << 28);
            r ^= (DES.des_SPtrans[1][t & 0x3F] | DES.des_SPtrans[3][t >>> 8 & 0x3F] | DES.des_SPtrans[5][t >>> 16 & 0x3F] | DES.des_SPtrans[7][t >>> 24 & 0x3F] | DES.des_SPtrans[0][u & 0x3F] | DES.des_SPtrans[2][u >>> 8 & 0x3F] | DES.des_SPtrans[4][u >>> 16 & 0x3F] | DES.des_SPtrans[6][u >>> 24 & 0x3F]);
        }
        l = (l >>> 1 | l << 31);
        r = (r >>> 1 | r << 31);
        t = ((r >>> 1 ^ l) & 0x55555555);
    ...
    public void decrypt(int l, int r, final int[] out) {
        ...
        for (int i = 30; i > 0; i -= 4) {
            int u = r ^ this.key_schedule[i];
            t = (r ^ this.key_schedule[i + 1]);
            t = (t >>> 4) + (t << 28);
            l ^= (DES.des_SPtrans[1][t & 0x3F] | DES.des_SPtrans[3][t >>> 8 & 0x3F] | DES.des_SPtrans[5][t >>> 16 & 0x3F] | DES.des_SPtrans[7][t >>> 24 & 0x3F] | DES.des_SPtrans[0][u & 0x3F] | DES.des_SPtrans[2][u >>> 8 & 0x3F] | DES.des_SPtrans[4][u >>> 16 & 0x3F] | DES.des_SPtrans[6][u >>> 24 & 0x3F]);
            u = (l ^ this.key_schedule[i - 2]);
            t = (l ^ this.key_schedule[i - 1]);
            t = (t >>> 4) + (t << 28);
            r ^= (DES.des_SPtrans[1][t & 0x3F] | DES.des_SPtrans[3][t >>> 8 & 0x3F] | DES.des_SPtrans[5][t >>> 16 & 0x3F] | DES.des_SPtrans[7][t >>> 24 & 0x3F] | DES.des_SPtrans[0][u & 0x3F] | DES.des_SPtrans[2][u >>> 8 & 0x3F] | DES.des_SPtrans[4][u >>> 16 & 0x3F] | DES.des_SPtrans[6][u >>> 24 & 0x3F]);
        }
        l = (l >>> 1 | l << 31);
