...
class SPLOFArray extends WorldArray
{
    public float maxhop;
    ...
    public SPLOFArray(final int n, final int n2, final MofPanel p3) {
        super(n, n2);
    ...
    public void Compute() {
        ...
        for (int i = 0; i < this.Width(); ++i) {
            ...
            for (int j = 0; j < this.Length(); ++j) {
                ...
                final int n8 = (int)(getValue / (2.0f * WorldArray.acos(SPLOFArray.r0 / (n7 + SPLOFArray.r0)))) + 1;
                final float n9 = getValue / (2.0f * n8);
                final float n10 = this.m9_1 / WorldArray.cos(WorldArray.fmin(WorldArray.fmax(WorldArray.atan(WorldArray.sin(n9) / ((SPLOFArray.r0 + n7) / SPLOFArray.r0 - WorldArray.cos(n9))), this.min_phi), this.max_phi));
                final float n11 = this.sex_1 - this.sex_2 * WorldArray.pow(WorldArray.cos(WorldArray.fmin(WorldArray.fmax(WorldArray.atan((WorldArray.cos(n9) - SPLOFArray.r0 / (n7 + SPLOFArray.r0)) / WorldArray.sin(n9)), this.min_takeoff), this.max_takeoff)), this.sex_3);
                for (int l = 1; l <= n8; ++l) {
                ...
                if (WorldArray.sygn(getValue6) != WorldArray.sygn(getValue5) && WorldArray.fabs(getValue6) >= this.te_minlat && WorldArray.fabs(getValue6) <= this.te_maxlat && WorldArray.fabs(getValue5) >= this.te_minlat && WorldArray.fabs(getValue5) <= this.te_maxlat && ((getValue3 > 0.0f && getValue2 > SPLOFArray.pi - this.te_azim && getValue2 < SPLOFArray.pi + this.te_azim) || (getValue3 < 0.0f && (getValue2 < this.te_azim || getValue2 > SPLOFArray.twopi - this.te_azim)))) {
                    fmin *= this.te_1;
                }
                final float fmin3 = WorldArray.fmin(WorldArray.fmax(fmin * n10, 1.0), 50.0);
                final float n13 = n * WorldArray.pow(WorldArray.fabs(n11), this.sex_4);
                float fmin4 = WorldArray.fmin(WorldArray.fmax(this.clof_1 + this.clof_2 * n13 + this.clof_3 * WorldArray.pow(n13, this.clof_4), 1.0), 50.0);
                if (fmin3 < fmin4 && this.blackout_flag) {
                    fmin4 = 50.0f;
                }
