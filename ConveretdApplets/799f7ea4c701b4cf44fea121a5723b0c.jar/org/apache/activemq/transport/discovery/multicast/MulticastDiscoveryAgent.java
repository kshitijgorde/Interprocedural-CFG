    ...
    class RemoteBrokerData
    {
        ...
        public synchronized void updateHeartBeat() {
            ...
            if (!this.failed && this.failureCount > 0 && this.lastHeartBeat - this.recoveryTime > 60000L) {
                if (MulticastDiscoveryAgent.LOG.isDebugEnabled()) {
                    MulticastDiscoveryAgent.LOG.debug("I now think that the " + this.service + " service has recovered.");
                }
                this.failureCount = 0;
        ...
        public synchronized boolean markFailed() {
            if (!this.failed) {
                ...
                if (MulticastDiscoveryAgent.LOG.isDebugEnabled()) {
                    MulticastDiscoveryAgent.LOG.debug("Remote failure of " + this.service + " while still receiving multicast advertisements.  Advertising events will be suppressed for " + reconnectDelay + " ms, the current failure count is: " + this.failureCount);
                }
        ...
        public synchronized boolean doRecovery() {
            ...
            if (MulticastDiscoveryAgent.this.maxReconnectAttempts > 0 && this.failureCount > MulticastDiscoveryAgent.this.maxReconnectAttempts) {
                if (MulticastDiscoveryAgent.LOG.isDebugEnabled()) {
                    MulticastDiscoveryAgent.LOG.debug("Max reconnect attempts of the " + this.service + " service has been reached.");
                }
                return false;
            }
            ...
            if (MulticastDiscoveryAgent.LOG.isDebugEnabled()) {
                MulticastDiscoveryAgent.LOG.debug("Resuming event advertisement of the " + this.service + " service.");
            }
            this.failed = false;
            return true;
        }
