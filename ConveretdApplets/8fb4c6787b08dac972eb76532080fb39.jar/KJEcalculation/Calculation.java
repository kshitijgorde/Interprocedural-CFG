...
public abstract class Calculation
{
    ...
    public String JavaScriptReport(final String s, final String s2, final String s3) throws CalculationException {
        ...
        this.recalculate();
        this.bWithSchedule = false;
        final String replace = replace("**GRAPH3**", s3, replace("**GRAPH2**", s2, replace("**GRAPH**", s, replace("**GRAPH**", String.valueOf(this.sChrome) + " " + s, replace("&amp;NBSP;", "&nbsp;", replace("QQ_HEADER_QQ", this.MHI, replace("CURRENCY_LABEL", this.sCurrency, this.formatReport(replace("&QCPQ;", "&copy;", replace("&NBSP;", "&nbsp;", replace("**Q**", "\"", replace("&QQ", "&#", replace("<START>", "<HTML><HEAD>", this._sMessage)))))))))))));
        if (replace.indexOf("**REPEATING GROUP**") == -1) {
            return replace;
        }
        return replace("**REPEATING GROUP**", this.getRepeating(), replace);
    }
