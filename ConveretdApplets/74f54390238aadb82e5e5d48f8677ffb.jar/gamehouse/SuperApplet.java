...
public class SuperApplet extends Applet implements Runnable
{
    ...
    private void CA(final int i) {
        ...
        this.C7.setFatalMessage(this.m_props.getString("STRING_LABEL_ERRORCON", "Error connecting to server.\nYour session may have expired.\n\nPlease close your browser and log in again."));
    }
    ...
    private void CC(final String s, final Vector vector, final MediaTracker mediatracker) {
        try {
            ...
            System.out.println("ld compound:" + ioexception.toString());
        }
    }
    
    public boolean action(final Event event, final Object obj) {
        if (obj instanceof String) {
            if (((String)obj).equals("casino")) {
                this.goCasino();
                return true;
            }
    ...
    public void init() {
        (this.m_props = new Properties()).init(this);
        this.m_helper = new ZylomHelper(this, this.m_props);
        String s = null;
        String s2 = null;
        String s3 = null;
        URL url = null;
        ...
        try {
            ...
            String s4 = this.getParameter("GAMEID");
            if (s4 != null) {
                i = Integer.parseInt(s4);
            }
            s3 = this.getParameter("DATAFILE");
            if ((s4 = this.getParameter("CANVAS")) != null && (this.D2 = InkUtil.getGameCanvasType(this, s4)) != 0) {
                s2 = s4;
            }
            s4 = this.getParameter("POINTMODE");
            if (s4 != null) {
                this.CC = (!s4.equalsIgnoreCase("true") && !s4.equalsIgnoreCase("yes") && !s4.equalsIgnoreCase("1"));
            }
            this.D1 = this.getParameter("XTRACOPY");
            this.CE = this.getParameter("ADURL");
            this.CF = this.getParameter("ADIMAGE");
            s4 = this.getParameter("ADSECS");
            if (s4 != null) {
                this.D0 = Integer.parseInt(s4);
            }
            Color color = this.C6("BGCOLOR", Color.black);
        ...
        this.CD = InkUtil.getUser(this, s, this.D2);
        if (this.D2 == 0) {
            s2 = null;
        }
        if (s2 == null) {
        ...
        try {
            final Class class1 = Class.forName(s2);
            final GameCanvas gamecanvas = class1.newInstance();
            gamecanvas.InitCanvas(url, s3, i);
            this.goCanvas(gamecanvas);
        }
    ...
    public void run() {
        ...
        try {
            if (!gamecanvas.isLoaded()) {
                ...
                gamecanvas.setMessage(this.m_props.getString("STRING_LABEL_LOADING", "Loading..."), 10);
                Image image = null;
                if (this.CF != null) {
                    image = this.C7(this.CF);
                    gamecanvas.setLoadingImage(image);
                    if (this.CE != null) {
                        gamecanvas.handCursor();
                    }
                }
                this.repaint();
                ...
                gamecanvas.setMessage(this.m_props.getString("STRING_LABEL_LOADING", "Loading Images..."), 20);
                ...
                for (int j = 0; j < i; ++j) {
                    ...
                    if (as[j] != null && as[j].length() > 0) {
                        ...
                        s1 = this.m_props.getString("STRING_FOLDER_IMAGES", "images/en-US") + s1;
                ...
                this.C8 = vector.size();
                this.CA(0);
                gamecanvas.setMessage(this.m_props.getString("STRING_LABEL_CONNECTING", "Connecting..."), 0);
                if (!gamecanvas.initGame()) {
                    this.CB = true;
                    gamecanvas.setFatalMessage(this.m_props.getString("STRING_LABEL_ERRORINIT", "Error initializing game. Please reload."));
                    return;
                }
                gamecanvas.setMessage(this.m_props.getString("STRING_LABEL_LOADIMG", "Loading Images..."), 0);
                mediatracker.waitForAll();
                if (this.C6) {
                    return;
                }
                ...
                gamecanvas.setMessage(this.m_props.getString("STRING_LABEL_LOADSND", "Loading Sounds..."), 0);
                ...
                gamecanvas.setMessage(this.m_props.getString("STRING_LABEL_INIT", "Initializing..."), 100);
            ...
            gamecanvas.setMessage(this.m_props.getString("STRING_LABEL_ABORTED", "Aborted loading"), 0);
            return;
        }
        catch (Throwable throwable) {
            gamecanvas.setFatalMessage(this.m_props.getString("STRING_LABEL_UNEXPECTED", "An unexpected error occured while loading game.\n"));
