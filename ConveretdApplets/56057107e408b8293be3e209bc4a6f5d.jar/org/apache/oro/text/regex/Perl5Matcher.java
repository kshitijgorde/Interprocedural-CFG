...
public final class Perl5Matcher implements PatternMatcher
{
    ...
    private void __pushState(final int n) {
        ...
        array[0] = this.__expSize;
        array[1] = this.__lastParen;
        array[2] = this.__inputOffset;
        for (int i = this.__expSize; i > n; --i, n2 -= 3) {
            array[n2] = this.__endMatchOffsets[i];
            array[n2 + 1] = this.__beginMatchOffsets[i];
            array[n2 + 2] = i;
        }
    ...
    private boolean __matchUnicodeClass(final char c, final char[] array, int n, final char c2) {
        boolean b = c2 == '#';
        while (array[n] != '\0') {
            if (array[n] == '%') {
                ++n;
                ...
                switch (array[n++]) {
                    ...
                    case '*': {
                        ...
                        if (this.__caseInsensitive && Character.isUpperCase(c)) {
                            return b;
                        }
                        continue;
                    }
                    case '-': {
                        ...
                        if (this.__caseInsensitive && Character.isLowerCase(c)) {
                            return b;
                        }
                        continue;
                    }
