...
public abstract class Calculation
{
    ...
    public String JavaScriptReport(final String s, final String s2, final String s3) throws CalculationException {
        this.bWithSchedule = true;
        this.recalculate();
        this.bWithSchedule = false;
        final String replace = replace("**GRAPH3**", s3, replace("**GRAPH2**", s2, replace("**GRAPH**", s, replace("&amp;NBSP;", "&nbsp;", replace("QQ_HEADER_QQ", this.MHI, replace("CURRENCY_LABEL", this.sCurrency, this.formatReport(replace("&QCPQ;", "&copy;", replace("&NBSP;", "&nbsp;", replace("**Q**", "\"", replace("&QQ", "&#", replace("<START>", "<HTML><HEAD>", this._sMessage))))))))))));
        this.sChrome = replace("qqPAGE_TITLEqq", this._sTitle, this.sChrome);
        final String replace2 = replace("</body", String.valueOf(this.sChrome) + " </body", replace("</BODY", String.valueOf(this.sChrome) + " </BODY", replace));
        if (replace2.indexOf("**REPEATING GROUP**") == -1) {
            return replace2;
        }
        return replace("**REPEATING GROUP**", this.getRepeating(), replace2);
    }
    ...
    public static String getMessage(final URL url, final String s) {
        final StringBuffer sb = new StringBuffer();
        try {
            final InputStream openStream = url.openStream();
            String line;
            while ((line = new BufferedReader((s == null) ? new InputStreamReader(openStream) : new InputStreamReader(openStream, s)).readLine()) != null) {
