...
public final class T extends bt implements aO
{
    protected final synchronized void a(final bv bv, final int n, final Graphics graphics) {
        try {
            ...
            if (bv.k != null && n4 + 24 >= n2 && n4 <= n3) {
                graphics.drawImage(bv.k, 6, n4, super.a);
            }
            ...
            if (n5 >= n2 && n5 - height <= n3) {
                ...
                graphics.setColor(new Color(bv.aA));
                graphics.setFont(a);
            ...
            if (bv.aC != -1) {
                graphics.setColor(new Color(bv.aC));
            }
            else if (bv.z) {
    ...
    private void a(final bv bv, final String s, final Graphics graphics, int n, final int n2, final Color color) {
        try {
            ...
            for (int i = 0; i < s.length(); ++i) {
                if (bv.j.containsKey(String.valueOf(i + bv.aD))) {
                    if (string.length() > 0) {
                        n += a(graphics, string, n, n2);
                    }
                    string = "";
                    final Object value = bv.j.get(String.valueOf(i + bv.aD));
