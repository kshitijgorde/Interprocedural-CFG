...
public class FindIt2Frame extends Applet implements MouseListener, MouseMotionListener
{
    ...
    private void endGame() {
        final HighScoreTool highScoreTool = new HighScoreTool(this);
        highScoreTool.setScore(this.getScore());
    ...
    public void init() {
        ...
        try {
            this.input = new URL(this.getCodeBase(), "data.txt").openStream();
        }
        catch (Exception ex) {
        ...
        if (this.registered) {
            try {
                this.raUrl = new URL(this.getParameter("URL"));
            }
            catch (Exception ex2) {}
    ...
    private void loadGraphics(final int n) {
        switch (n) {
            ...
            case 2: {
                this.logo = this.loadImage("logo.gif");
    ...
    public void mouseClicked(final MouseEvent mouseEvent) {
        ...
        if (!this.registered && x > 50 && x < 150 && y > 360 && y < 385) {
            this.pauseGame();
            this.getAppletContext().showDocument(this.raUrl, "_ra");
        }
    }
    ...
    public void paint(final Graphics graphics) {
        ...
        if (this.loadCount < this.totalFiles) {
            ...
            if (!this.registered) {
                ...
                this.frameBuffer.drawString("Registered to " + this.companyname, 60, 200);
            }
            this.frameBuffer.setColor(this.bgColor);
    ...
    public void readFile() {
        try {
            ...
            if (!this.registered && this.numberOfLevels > 20) {
