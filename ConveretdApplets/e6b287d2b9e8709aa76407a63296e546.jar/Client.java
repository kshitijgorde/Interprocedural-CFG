...
public class Client extends Applet implements Runnable, AppletStub, MouseListener, MouseMotionListener
{
    ...
    public void init() {
        ...
        System.out.println("\nAddonChat v8.60.00.28 [en, ico:" + this.c + "]");
        System.out.println("Copyright © 2001-2010, AddonInteractive. All Rights Reserved.");
        ...
        try {
            ...
            System.out.println("This JRE does not support Swing applets. [Fatal]");
            this.a(1);
        }
        try {
    ...
    public void paint(final Graphics graphics) {
        ...
        switch (this.a()) {
            ...
            case 1: {
                ...
                graphics.drawString("This applet requires a newer version of Java(tm)", (width - graphics.getFontMetrics().stringWidth("This applet requires a newer version of Java(tm)")) / 2, height / 2 + size / 2 - size - 40);
            ...
            case 2: {
                ...
                graphics.fillRect(0, 0, width, height);
                graphics.setColor(Color.black);
                graphics.drawString("We encountered a problem while attempting to load the applet.", (width - graphics.getFontMetrics().stringWidth("We encountered a problem while attempting to load the applet.")) / 2, height / 2 + size / 2 - size);
                graphics.drawString("Please reload this applet and try again. ", (width - graphics.getFontMetrics().stringWidth("Please reload this applet and try again. ")) / 2, height / 2 + size / 2 + size);
                break;
            }
            default: {
                graphics.setColor(Color.white);
    ...
    public void run() {
        try {
            ...
            System.out.println("Loading applet [" + array[0] + "]");
    ...
    public void mouseClicked(final MouseEvent mouseEvent) {
        ...
        if (x >= this.g && x <= this.h && y >= this.i && y <= this.j) {
            try {
                this.getAppletContext().showDocument(new URL("http://java.sun.com/getjava/"), "getjava");
            }
            catch (Exception ex) {}
        }
    }
