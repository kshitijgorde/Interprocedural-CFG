...
public class Legend
{
    ...
    private static final int TOPBOTTOM_BORDER_WIDTH = 3;
    private static final int BOX_SIDE_LENGTH = 6;
    public int LINE_SPACE;
    private static final int BOX_TO_LABEL_SPACE = 5;
    ...
    private void drawCatagoryLegend(final Graphics graphics, final int n, final int n2) {
        int n3 = n2 + 3;
        final Color[] colorList = this._gGraph.getColorList();
        for (int i = 0; i < this._gGraph._sGraphCatagories.length; ++i) {
            if (this._iShape == 1) {
                graphics.setColor(colorList[i]);
                graphics.fillRect(n + this._iColorBoxOffset, n3 + this._iColorBoxYOffset - 1, 6, 6);
                graphics.setColor(this._gGraph.getForeground());
                graphics.drawRect(n + this._iColorBoxOffset, n3 + this._iColorBoxYOffset - 1, 6, 6);
            }
            else {
                graphics.setColor(colorList[i]);
                graphics.fillOval(n + this._iColorBoxOffset, n3 + this._iColorBoxYOffset - 1, 7, 7);
                graphics.setColor(this._gGraph.getForeground());
            }
    ...
    public void paint(final Graphics graphics, int n, int n2, int n3, int n4, final int n5, final int n6, final int n7, final int n8) {
        ...
        if (this._bShow) {
            ...
            graphics.setFont(this._fFont);
            graphics.setColor(this._gGraph.getBackground());
    ...
    public void setSize(final Graphics graphics) {
        ...
        if (this._bShow) {
            this._iColorBoxOffset = 5;
            this._iLabelOffset = 17;
            this._iWidth = this._iLabelOffset + this._iMaxLegendLength + 4;
            this._iHeight = 3 + (this._iFontHeight + this.LINE_SPACE) * ((this._legendType == 2) ? ((this._gGraph._sGraphCatagories == null) ? 0 : this._gGraph._sGraphCatagories.length) : this._gGraph.getDataSeriesCount()) - this.LINE_SPACE - this._iFontDescent + 3;
            this._iColorBoxYOffset = (int)Math.ceil((this._iFontHeight - this._iFontDescent - 6) / 2.0f) + 1;
            switch (this._iOrientation) {
                case 3: {
