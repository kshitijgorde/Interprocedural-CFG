...
public abstract class Compiler implements Registers
{
    ...
    public static void main(final String[] array) throws IOException {
        ...
        final Seekable.File file = new Seekable.File(source);
        Writer writer = null;
        ...
        if (s4 == null || s4.equals("class")) {
            if (s != null) {
                ...
                compiler = new ClassFileCompiler(file, s3, outputStream);
            }
            else if (s2 != null) {
                ...
                compiler = new ClassFileCompiler(file, s3, file2);
            }
            else {
            ...
            compiler = new JavaSourceCompiler(file, s3, writer);
        }
        else {
    ...
    public Compiler(final Seekable seekable, final String fullClassName) throws IOException {
    ...
    public void go() throws Exn, IOException {
        ...
        if (this.pruneCases) {
            ...
            for (int i = 0; i < this.elf.sheaders.length; ++i) {
                final ELF.SHeader sHeader = this.elf.sheaders[i];
                ...
                if (sHeader.addr != 0) {
                    if (name.equals(".data") || name.equals(".sdata") || name.equals(".rodata") || name.equals(".ctors") || name.equals(".dtors")) {
                        this.findBranchesInData(new DataInputStream(sHeader.getInputStream()), sHeader.size, this.jumpableAddresses, sectionWithName.addr, sectionWithName.addr + sectionWithName.size);
                    }
                }
            }
        ...
        for (int j = 0; j < this.elf.sheaders.length; ++j) {
            ...
            if ((this.elf.sheaders[j].flags & 0x2) != 0x0 && !name2.equals(".text") && !name2.equals(".data") && !name2.equals(".sdata") && !name2.equals(".rodata") && !name2.equals(".ctors") && !name2.equals(".dtors") && !name2.equals(".bss") && !name2.equals(".sbss")) {
                throw new Exn("Unknown section: " + name2);
            }
        }
        this._go();
    }
    
    private void findBranchesInSymtab(final ELF.Symtab symtab, final Hashtable hashtable) {
        final ELF.Symbol[] symbols = symtab.symbols;
        int n = 0;
        for (int i = 0; i < symbols.length; ++i) {
            final ELF.Symbol symbol = symbols[i];
    ...
    private void findBranchesInText(final int n, final DataInputStream dataInputStream, final int n2, final Hashtable hashtable) throws IOException {
        final int n3 = n2 / 4;
        int n4 = n;
        int n5 = 0;
    ...
    private void findBranchesInData(final DataInputStream dataInputStream, final int n, final Hashtable hashtable, final int n2, final int n3) throws IOException {
        final int n4 = n / 4;
        int n5 = 0;
        for (int i = 0; i < n4; ++i) {
            final int int1 = dataInputStream.readInt();
    ...
    private Option getOption(String lowerCase) {
        lowerCase = lowerCase.toLowerCase();
        try {
            for (int i = 0; i < Compiler.options.length; i += 2) {
                if (Compiler.options[i].toLowerCase().equals(lowerCase)) {
                    return new Option(Compiler.options[i]);
                }
            }
            return null;
        }
    ...
    public void parseOptions(final String s) {
        ...
        while (stringTokenizer.hasMoreElements()) {
            ...
            final Option option = this.getOption(s2);
            if (option == null) {
    ...
    private class Option
    {
        ...
        public Option(final String s) throws NoSuchFieldException {
        ...
        public void set(final Object o) {
            ...
            try {
                this.field.set(Compiler.this, o);
            }
