...
final class CallTemplate extends Instruction
{
    ...
    public Template getCalleeTemplate() {
        final Stylesheet stylesheet = this.getXSLTC().getStylesheet();
        final Vector templates = stylesheet.getAllValidTemplates();
        for (int size = templates.size(), i = 0; i < size; ++i) {
            final Template t = templates.elementAt(i);
