        ...
        if (headers.containsKey("content-length") || transformation.equals(Stomp.Transformations.JMS_BYTE.toString())) {
            ...
            try {
                ...
                switch (Stomp.Transformations.getValue(transformation)) {
                    case JMS_OBJECT_XML: {
                        final HierarchicalStreamReader in = (HierarchicalStreamReader)new XppReader((Reader)new StringReader(text));
                        msg = this.createObjectMessage(in);
                        break;
                    }
                    case JMS_OBJECT_JSON: {
                        final HierarchicalStreamReader in = new JettisonMappedXmlDriver().createReader(new StringReader(text));
                        msg = this.createObjectMessage(in);
                        break;
                    }
                    case JMS_MAP_XML: {
