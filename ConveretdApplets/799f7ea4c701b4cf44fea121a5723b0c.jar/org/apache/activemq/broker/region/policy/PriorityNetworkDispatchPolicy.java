                    ...
                    while (it.hasNext()) {
                        ...
                        if (this.matches(candidate, info)) {
                            if (this.hasLowerPriority(candidate, info)) {
                                ...
                                if (!PriorityNetworkDispatchPolicy.LOG.isDebugEnabled()) {
                                    continue;
                                }
                                PriorityNetworkDispatchPolicy.LOG.debug("ignoring lower priority: " + candidate + "[" + candidate.getConsumerInfo().getNetworkConsumerIds() + ", " + candidate.getConsumerInfo().getNetworkConsumerIds() + "] in favour of: " + sub + "[" + sub.getConsumerInfo().getNetworkConsumerIds() + ", " + sub.getConsumerInfo().getNetworkConsumerIds() + "]");
                            }
                        }
                    }
