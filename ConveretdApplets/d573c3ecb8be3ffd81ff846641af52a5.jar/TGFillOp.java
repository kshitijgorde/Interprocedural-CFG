...
class TGFillOp implements ImageObserver, TGGraphicsOp
{
    ...
    public Rectangle doIt(final Image sourceImage) {
        ...
        final int canvasX = this.point.canvasX(this.sourceWidth);
        if (canvasX < 0 || canvasX >= this.sourceWidth) {
            return null;
        }
        ...
        final int canvasY = this.point.canvasY(this.sourceHeight);
        if (canvasY < 0 || canvasY >= this.sourceHeight) {
            return null;
        }
        this.pixels = new int[this.sourceWidth * this.sourceHeight];
        this.yCenter = this.sourceHeight / 2;
        this.subPixLeftX = canvasX - 10;
        if (this.subPixLeftX < 0) {
            this.subPixLeftX = 0;
        }
        this.subPixTopY = canvasY - 10;
        ...
        final int n = this.pixels[canvasY * this.sourceWidth + canvasX] & 0xFFFFFF;
        final int n2 = this.color.getRGB() & 0xFFFFFF;
        if (n == n2) {
            return null;
        }
        final int n3 = canvasX;
        this.minFloodX = n3;
        this.maxFloodX = n3;
        final int n4 = canvasY;
        this.minFloodY = n4;
        this.maxFloodY = n4;
        this.floodFill(canvasX, canvasY, this.sourceWidth, this.sourceHeight, n, n2);
