...
public class TFTPClient extends TFTP
{
    ...
    public void sendFile(final String filename, final int mode, InputStream input, InetAddress host, final int port) throws IOException {
        ...
        int lastBlock = dataLength = (hostPort = (bytesRead = 0));
        int block = 0;
        final boolean lastAckWait = false;
        if (mode == 0) {
                ...
                do {
                    ...
                    switch (received.getType()) {
                        ...
                        case 4: {
                            ...
                            int offset;
                            for (dataLength = 512, offset = 4; dataLength > 0 && (bytesRead = input.read(this._sendBuffer, offset, dataLength)) > 0; offset += bytesRead, dataLength -= bytesRead) {}
