...
public class Parser2
{
    ...
    private boolean maybeDoctypeDecl() throws IOException, SAXException {
        ...
        if (this.maybeWhitespace() && (externalSubset = this.maybeExternalID()) != null) {
            this.lexicalHandler.startDTD(this.rootElementName, externalSubset.publicId, externalSubset.systemId);
            this.maybeWhitespace();
        }
        else {
        ...
        while (true) {
            while (!this.in.peekc('>')) {
                if (this.in.peekc('/')) {
                    ...
                    if (this.namespaces) {
                        final String[] parts = this.processName(name.name, false);
                ...
                if (this.namespaces) {
                    this.processAttributeNS(attQName2, type, value, defaultValue, true, false);
                }
                else {
    ...
    private void processAttributeNS(final String attQName, final String type, final String value, final String defaultValue, final boolean isSpecified, final boolean isDefaulting) throws SAXException {
        if (attQName.startsWith("xmlns")) {
    ...
    private String[] processName(final String qName, final boolean isAttribute) throws SAXException {
        ...
        if (parts == null) {
            ...
            this.error("P-084", new Object[] { qName });
        }
        return parts;
    }
    ...
    private boolean defaultAttributes(final ElementDecl element) throws SAXException {
        ...
        while (e.hasMoreElements()) {
            ...
            if (this.namespaces) {
                this.processAttributeNS(declAttName, info.type, defaultValue, defaultValue, false, true);
            }
            else {
    ...
    private boolean maybeAttlistDecl() throws IOException, SAXException {
        ...
        while (!this.peek(">")) {
            ...
            if (!this.ignoreDeclarations && element.attributes.get(a.name) == null) {
                ...
                this.declHandler.attributeDecl(element.name, a.name, a.type, a.valueDefault, a.defaultValue);
            }
            this.maybeWhitespace();
        }
        if (this.isValidating && start != this.in) {
