...
public class VariableInput extends TextField implements InputObject, Tieable, Value
{
    ...
    protected double convertInput(final String s) {
        ...
        if (this.errorMessage == null && (n < this.minValue || n > this.maxValue)) {
            ...
            if (this.minValue > -1.7976931348623157E308 && this.maxValue < Double.MAX_VALUE) {
                this.errorMessage = String.valueOf(this.errorMessage) + "between " + NumUtils.realToString(this.minValue) + " and " + NumUtils.realToString(this.maxValue);
            }
            else if (this.minValue > -1.7976931348623157E308) {
                this.errorMessage = String.valueOf(this.errorMessage) + "greater than or equal to " + NumUtils.realToString(this.minValue);
            }
            else {
                this.errorMessage = String.valueOf(this.errorMessage) + "less than or equal to " + NumUtils.realToString(this.maxValue);
            }
            if (this.throwErrors) {
                this.requestFocus();
            }
    ...
    public void processKeyEvent(final KeyEvent keyEvent) {
        if (keyEvent.getID() == 401) {
            ...
            boolean b = (keyChar != '\0' && Character.isDigit(keyChar)) || keyChar == '-' || keyChar == '+' || keyCode == 127 || keyCode == 8;
            if (this.inputStyle != 2) {
                b = (b || keyChar == '.' || keyChar == 'e' || keyChar == 'E');
            }
            if (this.inputStyle == 0) {
                b = (b || Character.isLetter(keyChar) || keyChar == '(' || keyChar == ')' || keyChar == '*' || keyChar == '/' || keyChar == '^' || keyChar == ':' || keyChar == '?' || keyChar == '|' || keyChar == '&' || keyChar == '~' || keyChar == '=' || keyChar == '<' || keyChar == '>' || keyChar == '!' || keyCode == 32);
            }
    ...
    private class VI extends Variable
    {
        ...
        public void setVal(final double val) {
            ...
            if (!VariableInput.this.hasChanged && ((Double.isNaN(val) && Double.isNaN(val2)) || val == val2)) {
                return;
            }
