...
class IntArray
{
    ...
    public BigInteger toBigInteger() {
        ...
        final byte[] array2 = new byte[4 * (usedLength - 1) + n2];
        for (int j = 0; j < n2; ++j) {
            array2[j] = array[j];
        }
        for (int k = usedLength - 2; k >= 0; --k) {
            for (int l = 3; l >= 0; --l) {
                array2[n2++] = (byte)(this.m_ints[k] >>> 8 * l);
            }
        }
    ...
    public IntArray shiftLeft(final int n) {
        final int usedLength = this.getUsedLength();
        if (usedLength == 0) {
            return this;
        }
        ...
        for (int i = 1; i < usedLength; ++i) {
            array[i] = (this.m_ints[i] << n | this.m_ints[i - 1] >>> n2);
        }
        array[usedLength] = this.m_ints[usedLength - 1] >>> n2;
