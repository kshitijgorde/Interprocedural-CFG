...
public class DESAlgorithm
{
    ...
    private boolean des_check_key;
    private int[] ll;
    private static final byte[][] weak_keys;
    private static final byte[] odd_parity;
    private static final boolean[] shifts2;
    private static final int[][] des_skb;
    private static final int[][] des_SPtrans;
    
    public DESAlgorithm(final boolean des_check_key) {
        this.des_check_key = true;
        this.ll = new int[2];
        this.des_check_key = des_check_key;
    }
    
    public final void des_set_odd_parity(final byte[] array) {
        for (int i = 0; i < 8; ++i) {
            array[i] = DESAlgorithm.odd_parity[array[i] & 0xFF];
        }
    }
    
    public final boolean check_parity(final byte[] array) {
        for (int i = 0; i < 8; ++i) {
            if (array[i] != DESAlgorithm.odd_parity[array[i] & 0xFF]) {
                return false;
            }
        }
        return true;
    ...
    public final boolean des_is_weak_key(final byte[] array) {
        final int n = 0;
        if (n >= DESAlgorithm.weak_keys.length) {
            return false;
        }
        for (int i = 0; i < 8; ++i) {
            if (DESAlgorithm.weak_keys[n][i] != array[i]) {}
        }
        return true;
    }
    
    private final int Get32bits(final byte[] array, final int n) {
        return ((array[n + 3] & 0xFF) << 24) + ((array[n + 2] & 0xFF) << 16) + ((array[n + 1] & 0xFF) << 8) + (array[n] & 0xFF);
    }
    
    public final void des_set_key(final byte[] array, final int[] array2) throws SecurityException {
        if (this.des_check_key) {
            if (!this.check_parity(array)) {
                throw new SecurityException("des_set_key attempted with incorrect parity");
            }
            if (this.des_is_weak_key(array)) {
                throw new SecurityException("des_set_key attempted with weak key");
            }
        }
        final int n = 0;
        int n2 = 0;
        final int get32bits = this.Get32bits(array, n);
        final int get32bits2 = this.Get32bits(array, n + 4);
        final int n3 = (get32bits2 >>> 4 ^ get32bits) & 0xF0F0F0F;
        final int n4 = get32bits ^ n3;
        final int n5 = get32bits2 ^ n3 << 4;
        final int n6 = (n4 << 18 ^ n4) & 0xCCCC0000;
        final int n7 = n4 ^ n6 ^ n6 >>> 18;
        final int n8 = (n5 << 18 ^ n5) & 0xCCCC0000;
        final int n9 = n5 ^ n8 ^ n8 >>> 18;
        final int n10 = (n9 >>> 1 ^ n7) & 0x55555555;
        final int n11 = n7 ^ n10;
        final int n12 = n9 ^ n10 << 1;
        final int n13 = (n11 >>> 8 ^ n12) & 0xFF00FF;
        final int n14 = n12 ^ n13;
        final int n15 = n11 ^ n13 << 8;
        final int n16 = (n14 >>> 1 ^ n15) & 0x55555555;
        final int n17 = n15 ^ n16;
        final int n18 = n14 ^ n16 << 1;
        int n19 = (n18 & 0xFF) << 16 | (n18 & 0xFF00) | (n18 & 0xFF0000) >>> 16 | (n17 & 0xF0000000) >>> 4;
        int n20 = n17 & 0xFFFFFFF;
        for (int i = 0; i < 16; ++i) {
            int n21;
            int n22;
            if (DESAlgorithm.shifts2[i]) {
                n21 = (n20 >>> 2 | n20 << 26);
                n22 = (n19 >>> 2 | n19 << 26);
            }
            else {
