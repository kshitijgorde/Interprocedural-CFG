...
public class PGraphics extends PImage implements PConstants
{
    ...
    public void vertex(final float[] array) {
        this.vertexCheck();
        System.arraycopy(array, this.curveVertexCount = 0, this.vertices[this.vertexCount], 0, 36);
        ++this.vertexCount;
    }
    ...
    public void arc(final float n, final float n2, final float n3, final float n4, float n5, float n6) {
        float n7 = n;
        float n8 = n2;
        float n9 = n3;
        float n10 = n4;
    ...
    public void sphere(final float n) {
        ...
        for (int j = 2; j < this.sphereDetailV; ++j) {
            ...
            for (int k = 0; k < this.sphereDetailU; ++k) {
                ...
                this.vertex(this.sphereX[n4], this.sphereY[n4], this.sphereZ[n4++]);
            ...
            this.vertex(this.sphereX[n6], this.sphereY[n6], this.sphereZ[n6]);
    ...
    public void textMode(final int textMode) {
        if (textMode == 37 || textMode == 39) {
            showWarning("Since Processing beta, textMode() is now textAlign().");
            return;
        }
        if (this.textModeCheck(textMode)) {
            this.textMode = textMode;
        }
        else {
            String value = String.valueOf(textMode);
            switch (textMode) {
                case 256: {
                    value = "SCREEN";
                    break;
    ...
    protected void textCharImpl(final char c, final float n, final float n2) {
        ...
        if (this.textMode == 4) {
            final float n3 = this.textFont.height[index] / this.textFont.fheight;
            final float n4 = this.textFont.width[index] / this.textFont.fwidth;
            final float n5 = this.textFont.leftExtent[index] / this.textFont.fwidth;
            final float n6 = this.textFont.topExtent[index] / this.textFont.fheight;
            final float n7 = n + n5 * this.textSize;
            final float n8 = n2 - n6 * this.textSize;
            this.textCharModelImpl(pImage, n7, n8, n7 + n4 * this.textSize, n8 + n3 * this.textSize, this.textFont.width[index], this.textFont.height[index]);
        }
        else if (this.textMode == 256) {
            this.textCharScreenImpl(pImage, (int)n + this.textFont.leftExtent[index], (int)n2 - this.textFont.topExtent[index], this.textFont.width[index], this.textFont.height[index]);
        }
    }
    ...
    public static int lerpColor(final int n, final int n2, final float n3, final int n4) {
        if (n4 == 1) {
            final float n5 = n >> 24 & 0xFF;
            final float n6 = n >> 16 & 0xFF;
            final float n7 = n >> 8 & 0xFF;
            final float n8 = n & 0xFF;
    ...
    public static void showWarning(final String s) {
        if (PGraphics.warnings == null) {
            PGraphics.warnings = new HashMap<String, Object>();
        }
        if (!PGraphics.warnings.containsKey(s)) {
            System.err.println(s);
            PGraphics.warnings.put(s, new Object());
        }
    }
    
    protected static void showDepthWarning(final String s) {
        showWarning(s + "() can only be used with a renderer that " + "supports 3D, such as P3D or OPENGL.");
    }
    
    protected static void showDepthWarningXYZ(final String s) {
        showWarning(s + "() with x, y, and z coordinates " + "can only be used with a renderer that " + "supports 3D, such as P3D or OPENGL. " + "Use a version without a z-coordinate instead.");
    }
    
    protected static void showMethodWarning(final String s) {
        showWarning(s + "() is not available with this renderer.");
    }
    
    protected static void showVariationWarning(final String s) {
        showWarning(s + " is not available with this renderer.");
    }
    
    protected static void showMissingWarning(final String s) {
        showWarning(s + "(), or this particular variation of it, " + "is not available with this renderer.");
    }
    
    public static void showException(final String s) {
        throw new RuntimeException(s);
    }
    
    protected static void showTextFontException(final String s) {
        throw new RuntimeException("Use textFont() before " + s + "()");
    }
