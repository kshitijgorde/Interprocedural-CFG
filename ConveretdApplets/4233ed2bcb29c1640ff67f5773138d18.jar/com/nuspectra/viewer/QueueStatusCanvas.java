...
class QueueStatusCanvas extends Canvas implements Runnable, SessionListener
{
    boolean needRepaint;
    ...
    protected QueueStatusCanvas(final NuApplet applet) {
    ...
    public void cameraChanged(final int p, final int t, final int z, final int backlight) {
        ++this.ptzUpdates;
        new StringBuffer("{").append(p / 100).append(".").append(Math.abs(p % 100)).append(",").append(t / 100).append(".").append(Math.abs(t % 100)).append(",").append(z / 100).append(".").append(Math.abs(z % 100)).append("}").toString();
        this.needRepaint = true;
    }
