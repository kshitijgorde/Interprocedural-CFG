...
public class FontSizeBindingRule implements IBindingRule
{
    ...
    public FontSizeBindingRule(final String fontTag) {
    ...
    private static final class Listener extends ExpressionListener
    {
        ...
        public void notifyAdd(final IExpression expression, final IContext context, final List<IModelObject> list) {
            ...
            this.xidget.getFeature(ITextWidgetFeature.class).setFontSize(Xlate.get(this.node, 10.0));
        }
        ...
        public void notifyChange(final IExpression expression, final IContext context, final String s, final String s2) {
            try {
                this.xidget.getFeature(ITextWidgetFeature.class).setFontSize(Double.parseDouble(s));
            }
            catch (Exception ex) {}
        }
        
        @Override
        public void notifyChange(final IExpression expression, final IContext context, final double fontSize, final double n) {
            this.xidget.getFeature(ITextWidgetFeature.class).setFontSize(fontSize);
        }
