...
public class LayoutGridXAction extends AbstractLayoutAction
{
    ...
    protected void layout(final IContext context, final IXidget xidget, final List<IXidget> list, final int n) {
        ...
        for (int i = 0; i < n5; ++i, n4 += n3) {
            layoutFeature.attachContainer(list.get(i), ILayoutFeature.Side.right, n4, null, -n2, b);
        }
        for (int j = 1; j <= n5; ++j) {
            layoutFeature.attachPeer(list.get(j), ILayoutFeature.Side.left, list.get(j - 1), ILayoutFeature.Side.right, n);
        }
        layoutFeature.attachContainer(list.get(0), ILayoutFeature.Side.left, 0);
        layoutFeature.attachContainer(list.get(n5), ILayoutFeature.Side.right, 0);
    }
}
