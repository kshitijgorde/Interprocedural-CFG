...
public class a7 extends f
{
    ...
    static {
        a7.di = new char[] { 'd', 'a', 'z', 'z', 'l', 'e', 'c', 'o', 'l', 'o', 'r', '\0' };
        a7.dx = new char[] { 'f', 'l', 'a', 'r', 'e', 'c', 'o', 'l', 'o', 'r', '\0' };
        a7.dz = new char[] { 's', 'i', 'z', 'e', '\0' };
    }
    ...
    public void int(final bh bh) {
        ...
        for (int i = 0; i < bh.do; ++i) {
            ...
            else if (bh.try[i].toLowerCase().compareTo("dazzlecolor") == 0) {
                this.dH = true;
                final int a = an.a(bh.new[i]);
                this.dM = (a >> 16 & 0xFF);
                this.dL = (a >> 8 & 0xFF);
                this.dF = (a & 0xFF);
            }
            else if (bh.try[i].toLowerCase().compareTo("flarecolor") == 0) {
                this.ds |= 0x1;
                final int a2 = an.a(bh.new[i]);
                this.dk = (a2 >> 16 & 0xFF);
                this.dT = (a2 >> 8 & 0xFF);
                this.dO = (a2 & 0xFF);
            }
            else if (bh.try[i].toLowerCase().compareTo("size") == 0) {
                this.dP = new Float(bh.new[i]);
                this.ds |= 0x2;
            }
            else if (bh.try[i].toLowerCase().compareTo("file") == 0) {
                this.dA = super.b.B.a(bh.new[i], super.long, false, false, false);
                this.ds |= 0x4;
            }
        }
        this.a();
    ...
    public boolean a(final long n) {
        ...
        if (this.ds == 7 && this.dA != null && this.dA.b) {
            ...
            for (bh bh = do2.if; bh != null; bh = bh.for) {
                if (bh.a.toLowerCase().compareTo("flare") == 0) {
                    this.m();
                    for (int j = 0; j < bh.do; ++j) {
                        if (bh.try[j].toLowerCase().compareTo("map") == 0) {
                            this.dq[this.dN] = new Integer(bh.new[j]);
                        }
                        else if (bh.try[j].toLowerCase().compareTo("color") == 0) {
                            this.dh[this.dN] = an.a(bh.new[j]);
                        }
                        else if (bh.try[j].toLowerCase().compareTo("position") == 0) {
                            this.dm[this.dN] = (int)(new Float(bh.new[j]) * 655.35);
                        }
                        else if (bh.try[j].toLowerCase().compareTo("size") == 0) {
                            this.dK[this.dN] = new Float(bh.new[j]);
                        }
                    }
                    ++this.dN;
                }
    ...
    public void a(final char[] array, final bi bi) {
        ...
        else if (i.do(array, a7.di) == 0) {
            if (bi.char == 2) {
                if ((bi.case & 0xFFFFFFL) == 0x0L) {
                    this.dH = false;
                }
                else {
                    this.dH = true;
                    this.dM = ((int)(bi.case >> 16) & 0xFF);
                    this.dL = ((int)(bi.case >> 8) & 0xFF);
                    this.dF = ((int)bi.case & 0xFF);
                }
            }
        }
        else if (i.do(array, a7.dx) == 0) {
            if (bi.char == 2) {
                if ((bi.case & 0xFFFFFFL) == 0x0L) {
                    this.ds = 0;
                }
                    ...
                    this.ds = 7;
                    this.dk = ((int)(bi.case >> 16) & 0xFF);
                    this.dT = ((int)(bi.case >> 8) & 0xFF);
                    this.dO = ((int)bi.case & 0xFF);
                }
            }
        }
        else if (i.do(array, an.M) == 0) {
            if (bi.char == 2 || bi.char == 3) {
                super.cE = (float)((bi.char == 2) ? bi.case : bi.else);
            }
        }
        else if (i.do(array, an.byte) == 0 && (bi.char == 2 || bi.char == 3)) {
            super.cx = (float)((bi.char == 2) ? bi.case : bi.else);
        }
    }
