...
public class NativeLibrary
{
    ...
    Function getFunction(final String name, final Method method) {
        ...
        for (int i = 0; i < etypes.length; ++i) {
            if (((NativeLibrary.class$com$sun$jna$LastErrorException == null) ? (NativeLibrary.class$com$sun$jna$LastErrorException = class$("com.sun.jna.LastErrorException")) : NativeLibrary.class$com$sun$jna$LastErrorException).isAssignableFrom(etypes[i])) {
                flags |= 0x4;
            }
        }
    ...
    public void dispose() {
        synchronized (NativeLibrary.libraries) {
            NativeLibrary.libraries.remove(this.getName() + this.options);
            final File file = this.getFile();
            if (file != null) {
                NativeLibrary.libraries.remove(file.getAbsolutePath() + this.options);
                NativeLibrary.libraries.remove(file.getName() + this.options);
            }
        }
        synchronized (this) {
            if (this.handle != 0L) {
    ...
    private static native long open(final String p0);
    
    private static native void close(final long p0);
    
    private static native long findSymbol(final long p0, final String p1);
