...
public class GrappaRB extends Applet implements Runnable
{
    Thread Faden;
    ...
    public void run() {
        ...
        final int[] array7 = new int[360];
        final int[] array8 = new int[360];
        final int[] array9 = new int[360];
        for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < 360; ++j) {
            if (j < 60) {
                array7[j] = 255;
                array8[j] = (int)(j * 255.0 / 60.0);
                array9[j] = 0;
            }
            else if (j < 120) {
                array7[j] = (int)((120 - j) * 255.0 / 60.0);
                array8[j] = 255;
                array9[j] = 0;
            }
            else if (j < 180) {
                array7[j] = 0;
                array8[j] = 255;
                array9[j] = (int)((j - 120) * 255.0 / 60.0);
            }
            else if (j < 240) {
                array7[j] = 0;
                array8[j] = (int)((240 - j) * 255.0 / 60.0);
                array9[j] = 255;
            }
            else if (j < 300) {
                array7[j] = (int)((j - 240) * 255.0 / 60.0);
                array8[j] = 0;
                array9[j] = 255;
            }
            else {
        ...
        while (this.busy) {
            for (int k = 0; k < 10; ++k) {
                for (int l = 0; l < n; ++l) {
                    final double n3 = this.mouseX - array[l];
                    final double n4 = this.mouseY - array2[l];
                    final double[] array10 = array3;
                    final int n5 = l;
                    array10[n5] += n2 * n3 * array5[l];
                    final double[] array11 = array4;
                    final int n6 = l;
                    array11[n6] += n2 * n4 * array5[l];
                    final double[] array12 = array3;
                    final int n7 = l;
                    array12[n7] *= 0.98 + 0.015 * array6[l];
                    final double[] array13 = array4;
                    final int n8 = l;
                    array13[n8] *= 0.98 + 0.015 * array6[l];
                    final double[] array14 = array;
                    final int n9 = l;
                    array14[n9] += n2 * array3[l];
                    final double[] array15 = array2;
                    final int n10 = l;
                    array15[n10] += n2 * array4[l];
                    final int n11 = (int)(360.0 + 57.29577951308232 * Math.atan2(array3[l], array4[l])) % 360;
                    if (n11 < 0 || n11 > 359) {
                        System.out.println("w=" + n11);
                    }
                    this.SS.drawPixel(array[l], array2[l], array7[n11], array8[n11], array9[n11], 0.15);
                }
            }
    ...
    class SmiSchirm_c
    {
        ...
        public void fill(final int n, final int n2, final int n3) {
            GrappaRB.this.osg.setColor(new Color(n, n2, n3));
            GrappaRB.this.osg.fillRect(0, 0, GrappaRB.this.AppletW, GrappaRB.this.AppletH);
            this.y = 0;
            while (this.y < this.h) {
                this.x = 0;
                while (this.x < this.w) {
        ...
        public void update() {
            ...
            while (this.y < this.h) {
                this.x = 0;
                while (this.x < this.w) {
                    GrappaRB.this.osg.setColor(new Color(this.Rot[this.x][this.y] / 256, this.Gruen[this.x][this.y] / 256, this.Blau[this.x][this.y] / 256));
                    GrappaRB.this.osg.fillRect(this.x, this.y, 1, 1);
                    ++this.x;
                }
                ++this.y;
            }
        ...
        public void drawPixel(final double n, final double n2, final int n3, final int n4, final int n5, final double n6) {
            if (n > 0.0 && n < GrappaRB.this.AppletW - 1 && n2 > 0.0 && n2 < GrappaRB.this.AppletH - 1) {
                final int n7 = (int)n;
                final int n8 = n7 + 1;
                final int n9 = (int)n2;
                final int n10 = n9 + 1;
                ...
                if (this.realtime) {
                    GrappaRB.this.osg.setColor(new Color(this.Rot[n7][n9] / 256, this.Gruen[n7][n9] / 256, this.Blau[n7][n9] / 256));
                    GrappaRB.this.osg.drawLine(n7, n9, n7, n9);
                    GrappaRB.this.osg.setColor(new Color(this.Rot[n8][n9] / 256, this.Gruen[n8][n9] / 256, this.Blau[n8][n9] / 256));
                    GrappaRB.this.osg.drawLine(n8, n9, n8, n9);
                    GrappaRB.this.osg.setColor(new Color(this.Rot[n7][n10] / 256, this.Gruen[n7][n10] / 256, this.Blau[n7][n10] / 256));
                    GrappaRB.this.osg.drawLine(n7, n10, n7, n10);
                    GrappaRB.this.osg.setColor(new Color(this.Rot[n8][n10] / 256, this.Gruen[n8][n10] / 256, this.Blau[n8][n10] / 256));
                    GrappaRB.this.osg.drawLine(n8, n10, n8, n10);
                }
            }
        }
    }
    ...
    class MouseEventHandler extends MouseAdapter
    {
        public void mousePressed(final MouseEvent mouseEvent) {
            GrappaRB.this.mouseX = mouseEvent.getX();
            GrappaRB.this.mouseY = mouseEvent.getY();
            GrappaRB.this.mousePressed = true;
            if (GrappaRB.this.mouseY < 33 && GrappaRB.this.mouseX < 100) {
                GrappaRB.this.mousePressed = false;
                try {
                    GrappaRB.this.getAppletContext().showDocument(new URL("http://www.eigelb.at"), "_blank");
                }
