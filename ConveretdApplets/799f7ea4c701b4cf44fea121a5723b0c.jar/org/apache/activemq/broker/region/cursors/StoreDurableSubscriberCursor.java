...
public class StoreDurableSubscriberCursor extends AbstractPendingMessageCursor
{
    ...
    private int cacheCurrentLowestPriority;
    ...
    public StoreDurableSubscriberCursor(final Broker broker, final String clientId, final String subscriberName, final int maxBatchSize, final DurableTopicSubscription subscription) {
        ...
        this.cacheCurrentLowestPriority = -1;
    ...
    public synchronized void addMessageLast(final MessageReference node) throws Exception {
        if (node != null) {
            ...
            if (msg.isPersistent()) {
                final Destination dest = msg.getRegionDestination();
                final TopicStorePrefetch tsp = this.topics.get(dest);
                if (tsp != null) {
                    final int priority = msg.getPriority();
                    if (this.isStarted() && this.prioritizedMessages && this.immediatePriorityDispatch && !tsp.isCacheEnabled()) {
                        if (priority > tsp.getCurrentLowestPriority()) {
                            if (StoreDurableSubscriberCursor.LOG.isTraceEnabled()) {
                                StoreDurableSubscriberCursor.LOG.trace("enabling cache for cursor on high priority message " + priority + ", current lowest: " + tsp.getCurrentLowestPriority());
                            }
                            tsp.setCacheEnabled(true);
                            this.cacheCurrentLowestPriority = tsp.getCurrentLowestPriority();
                        }
                    }
                    else if (this.cacheCurrentLowestPriority != -1 && priority <= this.cacheCurrentLowestPriority) {
                        if (StoreDurableSubscriberCursor.LOG.isTraceEnabled()) {
                            StoreDurableSubscriberCursor.LOG.trace("disabling/clearing cache for cursor on lower priority message " + priority + ", tsp current lowest: " + tsp.getCurrentLowestPriority() + " cache lowest: " + this.cacheCurrentLowestPriority);
                        }
