...
public class ModSlayer
{
    ...
    final int get_track(final ModTrackInfo modTrackInfo, final byte[] array, int n) {
        final int n2 = array[n] & 0xF0;
        final int portto = (array[n++] & 0xF) << 8 | (array[n++] & 0xFF);
        final byte b = (byte)(array[n] & 0xF);
        int n3 = n2 | (array[n++] & 0xF0) >> 4;
        int oldsampofs = array[n++];
        modTrackInfo.effect = 0;
        if (n3 != 0) {
            --n3;
            final ModInstrument modInstrument = this.mod.insts[n3];
        ...
        if (portto != 0) {
            ...
            if (b != 3 && b != 5) {
                final int n4 = portto;
                modTrackInfo.period = n4;
                modTrackInfo.start_period = n4;
            ...
            if (b != 0 || oldsampofs != 0) {
                switch (b) {
                    case 0: {
                        int n5 = 12;
                        while (true) {
                            while (modTrackInfo.period < ModSlayer.period_set[n5]) {
                                if (++n5 >= 48) {
                                    modTrackInfo.arp[0] = ModSlayer.period_set[n5];
                                    modTrackInfo.arp[1] = ModSlayer.period_set[n5 + (oldsampofs & 0xF)];
                                    modTrackInfo.arp[2] = ModSlayer.period_set[n5 + ((oldsampofs & 0xF0) >> 4)];
                                    modTrackInfo.arpindex = 0;
                    ...
                    case 14: {
                        final int n6 = oldsampofs & 0xF0;
                        final int n7 = oldsampofs & 0xF;
                        switch (n6) {
                            case 1: {
                                modTrackInfo.period += n7;
