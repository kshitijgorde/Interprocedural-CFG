...
public class Interpreter extends UnixRuntime implements Cloneable
{
                        ...
                        switch (n2) {
                            case 0: {
                                switch (n10) {
                                    ...
                                    default: {
                                        throw new ExecutionException("Illegal instruction 0/" + n10);
                                    }
                                }
                                break;
                            }
                            ...
                            case 17: {
                                ...
                                switch (n3) {
                                    ...
                                    case 2: {
                                        if (n7 != 31) {
                                            throw new ExecutionException("FCR " + n7 + " unavailable");
                                        }
                                    ...
                                    case 16: {
                                        switch (n10) {
                                            ...
                                            default: {
                                                throw new ExecutionException("Invalid Instruction 17/" + n3 + "/" + n10 + " at " + this.sourceLine(pc));
                                            }
                                        }
                                        break;
                                    }
                                    ...
                                    default: {
                                        throw new ExecutionException("Invalid Instruction 17/" + n3);
                                    }
                                }
                                break;
                            }
                            ...
                            case 33: {
                                ...
                                switch (n22 & 0x3) {
                                    ...
                                    default: {
                                        throw new ReadFaultException(n22);
                                    }
                                }
                                if ((n23 & 0x8000) != 0x0) {
                                    n23 |= 0xFFFF0000;
                            ...
                            case 37: {
                                ...
                                switch (n27 & 0x3) {
                                    ...
                                    default: {
                                        throw new ReadFaultException(n27);
                                    }
                                }
                                break;
                            }
                            ...
                            case 41: {
                                ...
                                switch (n30 & 0x3) {
                                    ...
                                    default: {
                                        throw new WriteFaultException(n30);
                                    }
                                }
                                try {
                            ...
                            default: {
                                throw new ExecutionException("Invalid Instruction: " + n2);
                            }
                        }
                    }
                    pc = pc2;
    ...
    private void loadImage(final Seekable seekable) throws IOException {
        ...
        for (int i = 0; i < pheaders.length; ++i) {
            final ELF.PHeader pHeader = pheaders[i];
            if (pHeader.type == 1) {
                final int memsz = pHeader.memsz;
                final int filesz = pHeader.filesz;
                if (memsz != 0) {
                    if (memsz < 0) {
                        throw new IOException("pheader size too large");
                    }
                    int vaddr = pHeader.vaddr;
                    if (vaddr == 0) {
                    ...
                    if (filesz != 0) {
                        ...
                        final DataInputStream dataInputStream = new DataInputStream(pHeader.getInputStream());
                        do {
    ...
    protected void setCPUState(final CPUState cpuState) {
        for (int i = 1; i < 32; ++i) {
            this.registers[i] = cpuState.r[i];
        }
        for (int j = 0; j < 32; ++j) {
            this.fpregs[j] = cpuState.f[j];
        }
        this.hi = cpuState.hi;
        this.lo = cpuState.lo;
    ...
    protected void getCPUState(final CPUState cpuState) {
        for (int i = 1; i < 32; ++i) {
            cpuState.r[i] = this.registers[i];
        }
        for (int j = 0; j < 32; ++j) {
            cpuState.f[j] = this.fpregs[j];
        }
        cpuState.hi = this.hi;
        cpuState.lo = this.lo;
    ...
    public Interpreter(final Seekable seekable) throws IOException {
        super(4096, 65536);
    ...
    public Interpreter(final String image) throws IOException {
        this(new Seekable.File(image, false));
        this.image = image;
    }
    
    public Interpreter(final InputStream inputStream) throws IOException {
    ...
    {
        public void run() {
            final int access$000 = Interpreter.this.pc;
            if (Interpreter.this.getState() == 0) {
                System.err.print("\nCPU Executing " + Runtime.toHex(access$000) + ": " + Interpreter.this.sourceLine(access$000) + "\n");
            }
        }
    }
}
