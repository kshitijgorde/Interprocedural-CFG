...
public class ax extends Canvas implements aO
{
    ...
    private void b(final Graphics graphics) {
        if (graphics != null && this.isShowing()) {
            ...
            if (this.m != null) {
                ...
                if (this.e) {
                    graphics.setColor(new Color((!this.d || !this.h) ? this.ag : 16777215));
                }
                else {
    ...
    public ax(final String s, final int n, int c, final boolean b, final String s2, final String s3) {
        ...
        if (b) {
            if (s2 != null) {
                try {
                    ax.ah = Integer.parseInt(s2, 16);
                    ax.X = true;
                }
                catch (Exception ex) {}
            }
            else {
                ax.ah = Integer.parseInt("DDDDDD", 16);
            }
            if (s3 != null) {
                try {
                    ax.ai = Integer.parseInt(s3, 16);
                    ax.X = true;
                }
                catch (Exception ex2) {}
            }
        ...
        this.W = ax.X;
        if (this.W) {
            this.ag = new Color(ax.ai).getRGB();
            final int rgb = new Color(ax.ah).getRGB();
