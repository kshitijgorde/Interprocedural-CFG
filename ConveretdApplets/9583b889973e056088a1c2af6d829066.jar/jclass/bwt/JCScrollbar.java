...
public class JCScrollbar extends JCContainer implements JCAdjustable, JCActionListener
{
    ...
    public JCScrollbar(final int orientation, final Applet applet, final String s) {
        ...
        if (s == null) {
            this.setName("scrollbar" + JCScrollbar.nameCounter++);
        }
        this.setLayout(null);
    ...
    void setValue(int check, int n, final boolean b) {
        switch (n) {
            case 0: {
                n = 2;
                break;
        ...
        Vector<JCAdjustmentListener> vector = null;
        synchronized (this) {
            check = this.check(check);
            if (check == this.value) {
                // monitorexit(this)
            ...
            this.value = check;
            vector = (Vector<JCAdjustmentListener>)this.adjustmentListeners.clone();
        }
        this.moveThumb();
        if (!b || vector.size() == 0) {
            return;
        ...
        for (int i = 0; i < vector.size(); ++i) {
            vector.elementAt(i).adjustmentValueChanged(jcAdjustmentEvent);
        }
    }
    ...
    public void setValues(int min, int min2, final int min3, int max) {
        boolean b = false;
        synchronized (this) {
            if (max < min3) {
                max = min3;
    ...
    int cvtThumbSizeToPixels(final int n) {
        ...
        int n3 = Math.min(this.trough_size, Math.max(n2, 4));
    ...
    public void layout() {
        ...
        int max = 0;
        synchronized (this) {
            max = Math.max(this.minor, 16);
            if (max * 2 > this.major) {
                max = this.major / 2;
            }
            this.trough_size = Math.max(6, this.major) - 2 * max;
