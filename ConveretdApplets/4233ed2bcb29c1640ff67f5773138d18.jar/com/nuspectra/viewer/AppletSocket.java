...
public class AppletSocket
{
    ...
    public boolean connect(final URL url) throws IOException {
        ...
        if (this.debug) {
            Debug.println("calling openConnection.");
        }
        this.socket = (HttpURLConnection)url.openConnection();
    ...
    private boolean getStatus() throws IOException {
        ...
        if (this.status == null) {
            this.status = this.socket.getResponseMessage();
            if (this.status == null) {
    ...
    private byte[] getContentGuessLength(final int maxBytes) throws IOException {
        ...
        IO.copy((InputStream)this.getDataInputStream(), (OutputStream)buf, (long)maxBytes);
    ...
    public void setAuthorization(final String auth) throws Exception {
