...
public class PTVFile
{
    ...
    protected void extractHeaderData(final byte[] array) {
        ...
        if ((this.bitFlags & 0x1) != 0x0) {
            ...
            int length = int2;
            if (this.ptv.getCodeBase().toString().length() < length - 2) {
                length = this.ptv.getCodeBase().toString().length();
            }
            this.currentDomainString = this.ptv.getCodeBase().toString().substring(0, length - 2);
