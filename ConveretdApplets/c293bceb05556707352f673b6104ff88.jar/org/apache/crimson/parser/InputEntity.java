...
final class InputEntity implements Locator
{
    ...
    boolean isXmlDeclOrTextDeclPrefix() throws IOException, SAXException {
        final String match = "<?xml";
        final int matchLen = match.length();
        final int prefixLen = matchLen + 1;
        ...
        int i;
        for (i = 0; i < matchLen && this.start + i < this.finish; ++i) {
            if (this.buf[this.start + i] != match.charAt(i)) {
                return false;
            }
        }
        if (i >= matchLen) {
            return XmlChars.isSpace(this.buf[i]);
        }
        if (this.reader == null || this.isClosed) {
            return false;
        }
