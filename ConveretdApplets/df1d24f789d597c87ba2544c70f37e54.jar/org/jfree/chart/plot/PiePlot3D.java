...
public class PiePlot3D extends PiePlot implements Serializable
{
    ...
    public void draw(final Graphics2D g2, Rectangle2D plotArea, final PlotState parentState, final PlotRenderingInfo info) {
        ...
        for (int categoryIndex = 0; categoryIndex < categoryCount; ++categoryIndex) {
            ...
            if (arc != null) {
                final Paint paint = this.getSectionPaint(categoryIndex);
                outlinePaint = this.getSectionOutlinePaint(categoryIndex);
                final Stroke outlineStroke = this.getSectionOutlineStroke(categoryIndex);
                g2.setPaint(paint);
                g2.fill(arc);
        ...
        for (final Arc2D segment : arcList) {
            if (segment != null) {
                final Paint paint = this.getSectionPaint(cat);
                outlinePaint = this.getSectionOutlinePaint(cat);
                final Stroke outlineStroke = this.getSectionOutlineStroke(cat);
        ...
        for (int sectionIndex = 0; sectionIndex < categoryCount; ++sectionIndex) {
            ...
            if (arc != null) {
                ...
                final Paint paint = this.getSectionPaint(sectionIndex);
                outlinePaint = this.getSectionOutlinePaint(sectionIndex);
                final Stroke outlineStroke = this.getSectionOutlineStroke(sectionIndex);
                g2.setPaint(paint);
                g2.fill(upperArc);
                g2.setStroke(outlineStroke);
                g2.setPaint(outlinePaint);
                ...
                if (info != null) {
                    ...
                    if (entities != null) {
                        if (this.getToolTipGenerator() == null) {
                            this.setToolTipGenerator(new StandardPieItemLabelGenerator());
                        }
                        final String tip = this.getToolTipGenerator().generateToolTip(dataset, currentKey2);
                        String url = null;
