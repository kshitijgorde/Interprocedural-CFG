...
public class Brealizer
{
    ...
    public static char[] encode(final byte[] array, final int n) {
        ...
        int n9;
        int n10;
        for (int i = 0, n3 = 0; i < n; n4 = (array[i++] & 0xFF), n5 = ((i < n) ? (array[i++] & 0xFF) : 0), n6 = ((i < n) ? (array[i++] & 0xFF) : 0), n7 = n4 >>> 2, n8 = ((n4 & 0x3) << 4 | n5 >>> 4), n9 = ((n5 & 0xF) << 2 | n6 >>> 6), n10 = (n6 & 0x3F), array2[n3++] = Brealizer.map1[n7], array2[n3++] = Brealizer.map1[n8], array2[n3] = ((n3 < n2) ? Brealizer.map1[n9] : '='), ++n3, array2[n3] = ((n3 < n2) ? Brealizer.map1[n10] : '='), ++n3) {}
        return array2;
    }
    ...
    public static byte[] decode(final char[] array) {
        ...
        while (i < length) {
            ...
            final byte b = Brealizer.map2[c];
            final byte b2 = Brealizer.map2[c2];
            final byte b3 = Brealizer.map2[c3];
            final byte b4 = Brealizer.map2[c4];
    ...
    static {
        ...
        Brealizer.map1[n++] = '+';
        Brealizer.map1[n++] = '/';
        Brealizer.map2 = new byte[128];
        for (int i = 0; i < Brealizer.map2.length; ++i) {
            Brealizer.map2[i] = -1;
        }
        for (int j = 0; j < 64; ++j) {
            Brealizer.map2[Brealizer.map1[j]] = (byte)j;
        }
    }
}
