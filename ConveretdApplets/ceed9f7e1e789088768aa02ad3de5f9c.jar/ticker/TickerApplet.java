...
public class TickerApplet extends Applet
{
    ...
    public void init() {
        this.initParametres();
        this.database = new TitresDataBase(this.getDocumentBase(), this.getCodeBase(), Parametres.getParamStr("cgi") + "?genre=ticker", Parametres.getParamInt("cgi_frequence_ms"));
        this.def = new Defile(this.database);
    ...
    Hashtable loadINI() {
        ...
        try {
            final DataInputStream dataInputStream = new DataInputStream(new URL(this.getCodeBase(), "ticker/config.ini").openStream());
            String line;
