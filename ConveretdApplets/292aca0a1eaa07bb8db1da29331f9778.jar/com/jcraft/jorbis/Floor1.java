...
class Floor1 extends FuncFloor
{
    ...
    void pack(final Object i, final Buffer opb) {
        ...
        final int rangebits = ilog2(maxposit);
        int j = 0;
        int k = 0;
    ...
    private static void render_line(int x0, final int x1, final int y0, final int y1, final float[] d) {
        final int dy = y1 - y0;
        final int adx = x1 - x0;
        int ady = Math.abs(dy);
        ...
        d[x0] *= Floor1.FLOOR_fromdB_LOOKUP[y2];
        while (++x0 < x1) {
            err += ady;
            if (err >= adx) {
                err -= adx;
